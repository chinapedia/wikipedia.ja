...
公比  は （ は任意）という形で取り出すことができ、 以外の全ての数を取りうる（とくに  の場合は公差が  の[等差数列](https://ja.wikipedia.org/wiki/等差数列 "wikilink")でもある）。等比数列の各項は初項  と公比  を用いて \(a, ar, ar^2, ar^3, ar^4, \ldots\) と表せる。すなわち  番目の項 （一般項）は \(a_n := a r^{n-1}\) で求まる。 一般に第  項  と第  項  の関係は \(a_n = a_m r^{n-m}\)　である。
...
## 関連項目
  ...
  - [等差数列](https://ja.wikipedia.org/wiki/等差数列 "wikilink")

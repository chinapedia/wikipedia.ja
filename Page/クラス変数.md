...
**クラス変数** () または、**静的メンバ変数** ()、**静的フィールド** () とは、同一[クラス](../Page/クラス_\(コンピュータ\).md "wikilink")・[派生クラスで共有される](../Page/サブクラス_\(計算機科学\).md "wikilink")[変数である](https://ja.wikipedia.org/wiki/変数_\(プログラミング\) "wikilink")。そのため、関連クラスに渡って共通に使用できる[グローバル変数](../Page/グローバル変数.md "wikilink")（大域変数）であるともいえる。

これに対して、インスタンスごとに確保される変数のことを、[インスタンス変数](https://ja.wikipedia.org/wiki/インスタンス変数 "wikilink")あるいは単に[フィールド (計算機科学)](../Page/フィールド_\(計算機科学\).md "wikilink")、[データメンバ](https://ja.wikipedia.org/wiki/データメンバ "wikilink")、[メンバ変数](https://ja.wikipedia.org/wiki/メンバ変数 "wikilink")などと呼ぶ。
...
なお、Rubyにおいては[クラス自身もClassクラスのインスタンスであるので](https://ja.wikipedia.org/wiki/メタクラス "wikilink")、そのインスタンス変数（クラスインスタンス変数）も存在する。「クラスに属する変数」という意味で両者は共通するが、クラスインスタンス変数はインスタンスメソッドから直接使うことができないなど、機能としては別である。

## Java

本節では、[Java](https://ja.wikipedia.org/wiki/Java "wikilink")におけるクラス変数の例を記す。

Javaにおいては、static修飾子を使うことでクラス変数を宣言できる。下記の例において、2行目で宣言されているfooはクラス変数であり、3行目で宣言されているbarは[インスタンス変数](https://ja.wikipedia.org/wiki/インスタンス変数 "wikilink")である。冒頭でも説明されているように、クラス変数は同一[クラスと](../Page/クラス_\(コンピュータ\).md "wikilink")[派生クラスで共有される](../Page/サブクラス_\(計算機科学\).md "wikilink")。ただし、クラス変数のアクセスレベルが`private`の場合、あるいはアクセスレベルが[パッケージプライベート](../Page/パッケージ_\(Java\).md "wikilink")（デフォルト）で、派生クラスがパッケージ外に ある場合は、派生クラスから参照することはできない。
...
2.  クラスの状態の保持
      - あるクラスのインスタンス全てから共有可能な「クラスの状態」を表す変数の定義に使用できる。
          - クラスの状態の保持の代表例として、[Singleton パターンでシングルトンオブジェクトを記憶する変数が挙げられる](https://ja.wikipedia.org/wiki/Singleton_パターン "wikilink")。

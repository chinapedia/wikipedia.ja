...
**ソート** () は、[データ](https://ja.wikipedia.org/wiki/データ "wikilink")の集合を一定の規則に従って並べること。。はその原義から**分類**という訳語が充てられていた\[1\]。

主に[配列](https://ja.wikipedia.org/wiki/配列 "wikilink")や[連結リスト](../Page/連結リスト.md "wikilink")のような、[リストデータ構造に分類されるコレクション](https://ja.wikipedia.org/wiki/リスト_\(抽象データ型\) "wikilink")（[コンテナ](https://ja.wikipedia.org/wiki/コンテナ_\(データ型\) "wikilink")）に格納されている要素データを、[全順序](https://ja.wikipedia.org/wiki/全順序 "wikilink")関係によって並べ替えることを指す。また、単に「ソート」といった場合、値の小さい方から大きい方へ順に並べる**昇順**（しょうじゅん、）を指すことが多い。その反対に値を大きい方から小さい方へ順に並べることを**降順**（こうじゅん、）という。
...
効率的なソートは、ソート済みのデータを必要とする他のアルゴリズム（[探索](https://ja.wikipedia.org/wiki/探索 "wikilink")や[マージ](https://ja.wikipedia.org/wiki/マージ "wikilink")）の最適化にとっても重要である。また、ソートされたデータの方が人間にとっても読みやすい。形式的には、その出力は以下の2つの条件を満たさなければならない。

1.  設定された順序（昇順または降順）に対して、逆になるような順序の出力があってはならない。
2.  出力は入力の並べ替えでなければならない。

[情報工学](../Page/情報工学.md "wikilink")や[計算機科学](../Page/計算機科学.md "wikilink")の黎明期から、ソートは単純な問題でありながら効率的に解くことが難しく、そのためもあって盛んに研究されてきた。例えば[バブルソート](https://ja.wikipedia.org/wiki/バブルソート "wikilink")については、早くも1956年には解析が行われている\[2\]。が、も新たなソートアルゴリズムが発明されている（例えば、[図書館ソート](https://ja.wikipedia.org/wiki/図書館ソート "wikilink")が最初に発表されたのは2004年である）。すでに考案されているソートにも様々なアルゴリズムがあり、またアルゴリズムという概念を学習するのに最適なので、情報工学や計算機科学での入門的題材として広く親しまれている。例えば、[分割統治法](https://ja.wikipedia.org/wiki/分割統治法 "wikilink")、[データ構造](../Page/データ構造.md "wikilink")、[乱択アルゴリズム](https://ja.wikipedia.org/wiki/乱択アルゴリズム "wikilink")、[計算量解析](https://ja.wikipedia.org/wiki/計算複雑性理論 "wikilink")、[O記法](https://ja.wikipedia.org/wiki/ランダウの記号 "wikilink")、[時間と空間のトレードオフ](https://ja.wikipedia.org/wiki/時間と空間のトレードオフ "wikilink")、下限などが含まれる。
...
同じ値に関して、ソート前の順序がソート後も維持されているソートを[安定ソート](https://ja.wikipedia.org/wiki/安定ソート "wikilink")という。
...
ソートされるデータの格納領域を変更して処理を進めていく[In-placeのソートを](https://ja.wikipedia.org/wiki/In-placeアルゴリズム "wikilink")**内部ソート**という。[クイックソート](https://ja.wikipedia.org/wiki/クイックソート "wikilink")のような再帰を利用するアルゴリズムは、再帰のために O(log *n*) の領域を必要とすることからIn-placeであるか否かは議論が分かれるところであるが、これも内部ソートに含めるのが一般的である。このことから内部ソートは、ソートされるデータの格納域以外には O(1) か O(log *n*) の領域しか必要としない。
...
入力されるリストの項目数 *n* に基づいた[計算量による分類](https://ja.wikipedia.org/wiki/計算複雑性理論 "wikilink")。典型的なソートアルゴリズムでは、最善で [O](https://ja.wikipedia.org/wiki/ランダウの記号 "wikilink")(*n* log *n*) 、最悪で O(*n*<sup>2</sup>) である。理想は O(*n*) である。
...
[配列](https://ja.wikipedia.org/wiki/配列 "wikilink")に格納された*n*個のデータをソートする場合について、各アルゴリズムの性能を示す。 計算時間の表記に用いている記号 O（オー）については、[ランダウの記号](https://ja.wikipedia.org/wiki/ランダウの記号 "wikilink")を参照。
...
<tbody>
<tr class="odd">
<td><p><a href="https://ja.wikipedia.org/wiki/バブルソート" title="wikilink">バブルソート</a></p></td>
<td><p>—</p></td>
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/シェーカーソート" title="wikilink">シェーカーソート</a></p></td>
<td><p>—</p></td>
<td><p><span class="math inline"><em>n</em><sup>2</sup></span></p></td>
<td><p><span class="math inline">1</span></p></td>
<td><p>○</p></td>
...
<tr class="odd">
<td><p><a href="https://ja.wikipedia.org/wiki/選択ソート" title="wikilink">選択ソート</a></p></td>
<td><p><span class="math inline"><em>n</em><sup>2</sup></span></p></td>
<td><p><span class="math inline"><em>n</em><sup>2</sup></span></p></td>
<td><p><span class="math inline">1</span></p></td>
<td><p>×</p></td>
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/挿入ソート" title="wikilink">挿入ソート</a></p></td>
...
<tr class="odd">
<td><p><a href="https://ja.wikipedia.org/wiki/シェルソート" title="wikilink">シェルソート</a></p></td>
<td><p>—</p></td>
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/マージソート" title="wikilink">マージソート</a></p></td>
<td><p><span class="math inline"><em>n</em>log <em>n</em></span></p></td>
<td><p><span class="math inline"><em>n</em>log <em>n</em></span></p></td>
<td><p><span class="math inline"><em>n</em></span></p></td>
<td><p>○</p></td>
...
<tr class="odd">
<td><p><a href="https://ja.wikipedia.org/wiki/In-placeアルゴリズム" title="wikilink">In-place</a> <a href="https://ja.wikipedia.org/wiki/マージソート" title="wikilink">マージソート</a></p></td>
<td><p><span class="math inline"><em>n</em>log<sup>2</sup><em>n</em></span></p></td>
<td><p><span class="math inline"><em>n</em>log<sup>2</sup><em>n</em></span></p></td>
<td><p><span class="math inline">1</span></p></td>
<td><p>○</p></td>
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/ヒープソート" title="wikilink">ヒープソート</a></p></td>
<td><p><span class="math inline"><em>n</em>log <em>n</em></span></p></td>
<td><p><span class="math inline"><em>n</em>log <em>n</em></span></p></td>
<td><p><span class="math inline">1</span></p></td>
<td><p>×</p></td>
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/クイックソート" title="wikilink">クイックソート</a></p></td>
<td><p><span class="math inline"><em>n</em>log <em>n</em></span></p></td>
<td><p><span class="math inline"><em>n</em><sup>2</sup></span></p></td>
<td><p><span class="math inline"><em>n</em></span></p></td>
<td><p>×</p></td>
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/奇偶転置ソート" title="wikilink">奇偶転置ソート</a></p></td>
<td><p><span class="math inline"><em>n</em><sup>2</sup></span></p></td>
<td><p><span class="math inline"><em>n</em><sup>2</sup></span></p></td>
<td><p><span class="math inline">1</span></p></td>
<td><p>○</p></td>
...
<tr class="odd">
<td><p><a href="https://ja.wikipedia.org/wiki/シェアソート" title="wikilink">シェアソート</a></p></td>
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/バケットソート" title="wikilink">バケットソート</a></p></td>
...
<tr class="odd">
<td><p><a href="https://ja.wikipedia.org/wiki/バケットソート" title="wikilink">分布数えソート</a></p></td>
<td><p><span class="math inline"><em>n</em> + 2<sup><em>k</em></sup></span></p></td>
<td><p><span class="math inline"><em>n</em> + 2<sup><em>k</em></sup></span></p></td>
<td><p><span class="math inline"><em>n</em> + 2<sup><em>k</em></sup></span></p></td>
<td><p>○</p></td>
...
<tr class="even">
<td><p>LSD <a href="https://ja.wikipedia.org/wiki/基数ソート" title="wikilink">基数ソート</a></p></td>
...
<tr class="odd">
<td><p>MSD <a href="https://ja.wikipedia.org/wiki/基数ソート" title="wikilink">基数ソート</a></p></td>
<td><p><span class="math inline"><em>n</em><em>k</em>/<em>s</em></span></p></td>
...
<tbody>
<tr class="odd">
<td><p><a href="https://ja.wikipedia.org/wiki/ボゴソート" title="wikilink">ボゴソート</a></p></td>
...
例えば、再帰型の[クイックソート](https://ja.wikipedia.org/wiki/クイックソート "wikilink")は主記憶上では性能が良いが、ソート対象の配列が主記憶に収まらない場合はスワップが頻繁に発生して、性能が極端に低下する。したがって、そのような場合は比較回数が多くても他のアルゴリズムを使った方がよい。
...
別の技法として、2つのアルゴリズムを組み合わせて、それぞれの利点を利用する方法がある。例えば、配列をチャンクに分割して個々のチャンクが主記憶上でソートできる大きさにする。チャンク内のソートはメモリ上で効率的に動作するソートアルゴリズムを使い、その結果を[マージソート](https://ja.wikipedia.org/wiki/マージソート "wikilink")でマージする。これは、元の配列を単純にマージソートでソートするよりも効率が悪いが、全体をクイックソートでソートするよりもメモリ使用量が少なくてすむ。
...
## 参考文献
  ...
  - [D. E. Knuth](../Page/ドナルド・クヌース.md "wikilink"), *[The Art of Computer Programming](https://ja.wikipedia.org/wiki/The_Art_of_Computer_Programming "wikilink"), Volume 3: Sorting and Searching*.
...
### ソートアルゴリズムの視覚化
  ...
  - [graphical demonstration](http://vision.bc.edu/~dmartin/teaching/sorting/anim-html/all.html) - [ボストンカレッジ](https://ja.wikipedia.org/wiki/ボストンカレッジ "wikilink")。8種類のソートアルゴリズムに4種類の典型的な初期条件を与えたときのソートの様子をグラフィカルに表示している。
  - [graphical demonstration](http://www.ida.liu.se/~TDDB28/mtrl/demo/sorting/index.sv.shtml) - [リンショーピング大学](https://ja.wikipedia.org/wiki/リンショーピング大学 "wikilink")
...
1.  [パンチカード](https://ja.wikipedia.org/wiki/パンチカード "wikilink")システムの時代には、下位の桁から順に**分類**しては併合することがまさしく整列であった（O(*n*) の[バケットソート](https://ja.wikipedia.org/wiki/バケットソート "wikilink")になる）。

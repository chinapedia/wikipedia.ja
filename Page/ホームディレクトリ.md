...
| url = <http://refspecs.linuxfoundation.org/FHS_3.0/fhs/ch03s08.html> | title = 3.8. /home : User home directories (optional) | accessdate = 2018-08-31 | publisher = LSB Workgroup, [The Linux Foundation](../Page/Linux_Foundation.md "wikilink") }}</ref>，最も標準的なホームディレクトリの位置である。
  ...
  - `/Users/username` - [macOS](https://ja.wikipedia.org/wiki/macOS "wikilink")、[NEXTSTEP](https://ja.wikipedia.org/wiki/NEXTSTEP "wikilink")、[OPENSTEP](https://ja.wikipedia.org/wiki/OPENSTEP "wikilink")などで使用される。
  - `/usr/username` - 初期の[UNIX](../Page/UNIX.md "wikilink")で使用された\[1\]。`/usr` はユーザファイルシステムのことである。

[Unix系](../Page/Unix系.md "wikilink")OSにおける[スーパーユーザである](https://ja.wikipedia.org/wiki/スーパーユーザー "wikilink") root のホームディレクトリは、他のユーザと違い`/`や、`/root`などである。これらのユーザのホームディレクトリは、ルートファイルシステムに置かれる。まずあり得ないが、root のホームディレクトリが、他のスライス（パーティション）や、[NFS上にあった場合](https://ja.wikipedia.org/wiki/Network_File_System "wikilink")、rootがログインできなくなってしまう。

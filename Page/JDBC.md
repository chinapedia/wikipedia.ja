...
たとえば、初期の[オラクル社の](../Page/オラクル_\(企業\).md "wikilink")  () 用JDBCドライバは、当時の JDBC API が [BLOB](https://ja.wikipedia.org/wiki/バイナリ・ラージ・オブジェクト "wikilink")、[CLOBに対応していなかったため](https://ja.wikipedia.org/wiki/キャラクタ・ラージ・オブジェクト "wikilink")、独自に機能拡張をしてBLOBとCLOBに対応していた。

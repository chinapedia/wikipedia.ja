...
\[\sum_{n=-\infty}^{\infty}(-1)^nq^{2n^2}z^{2n}=\prod_{m=1}^{\infty}(1-q^{4m})(1-q^{4m-2}z^{2})(1-q^{4m-2}z^{-2})\] [オイラーの五角数定理](https://ja.wikipedia.org/wiki/オイラーの五角数定理 "wikilink")により
...
### 和文
  ...
  - , 平成29年度 (第39回) 数学入門公開講座テキスト ([京都大学数理解析研究所](https://ja.wikipedia.org/wiki/京都大学数理解析研究所 "wikilink")，平成29年7月31日～8月3日開催)

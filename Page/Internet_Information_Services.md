...
**Microsoft Internet Information Services** (IIS) は、[Microsoft Windowsの標準](https://ja.wikipedia.org/wiki/Microsoft_Windows "wikilink")[Webサーバ](../Page/Webサーバ.md "wikilink")ー（[アプリケーションサーバ](https://ja.wikipedia.org/wiki/アプリケーションサーバ "wikilink")ー）[サービス](https://ja.wikipedia.org/wiki/サービス "wikilink")である。[HTTP](../Page/Hypertext_Transfer_Protocol.md "wikilink")/[HTTPS](https://ja.wikipedia.org/wiki/HTTPS "wikilink")、[FTP](../Page/File_Transfer_Protocol.md "wikilink")、[SMTP](../Page/Simple_Mail_Transfer_Protocol.md "wikilink")、[NNTP等の基本的なプロトコルはサポートしている](../Page/Network_News_Transfer_Protocol.md "wikilink")。クライアント版に付属するIISでは機能制限が行われている\[1\]。
...
インストールした時点でIISの仕事は始まっており、指定されたフォルダにhtmlテキストを保存し、設定することでwebページの公開は可能である。またサーバ版の場合、[Windows Server Update Servicesや](https://ja.wikipedia.org/wiki/Windows_Server_Update_Services "wikilink")、[Microsoft Exchange Server等のアプリケーションと関連付ける事で](https://ja.wikipedia.org/wiki/Microsoft_Exchange_Server "wikilink")、サーバアプリケーションをブラウザ越しに、よりグラフィカルに設定させることが出来るため、ある意味[マイクロソフト](../Page/マイクロソフト.md "wikilink")を象徴するコンポーネントといえる。
...
## バージョン
  ...
  - IIS 6.0
    [Windows Server 2003](https://ja.wikipedia.org/wiki/Microsoft_Windows_Server_2003 "wikilink") と Windows XP Professional x64 Edition に付属するバージョンである。
    安全性の向上として IIS 5.0 では [Windows Server](https://ja.wikipedia.org/wiki/Windows_Server "wikilink") のインストール完了直後から IIS が機能していたが、このバージョンから明示的にインストールしない限り無効化されている。また、全面的にソースコードが書き直された。
  ...
  - IIS 7.0
    [Windows Vista](https://ja.wikipedia.org/wiki/Microsoft_Windows_Vista "wikilink") と [Windows Server 2008](https://ja.wikipedia.org/wiki/Microsoft_Windows_Server_2008 "wikilink") に付属するバージョンである。Windows Vista は Windows Vista Service Pack 1 で Windows Server 2008 と同じ [FastCGI](https://ja.wikipedia.org/wiki/FastCGI "wikilink") に標準で対応したものに変更された。
    IIS の機能のモジュール化が行われた。
    Server Core インストールで IIS の機能を有効化させることができるが、静的 Web サイトのみ対応し、ASP.NET 等の動的 Web サイトを構築できない。
  - IIS 7.5
    [Windows 7](https://ja.wikipedia.org/wiki/Microsoft_Windows_7 "wikilink") と [Windows Server 2008 R2](https://ja.wikipedia.org/wiki/Microsoft_Windows_Server_2008_R2 "wikilink") に付属するバージョンである。
  ...
  - IIS 8.0
    ...
    [NUMA](https://ja.wikipedia.org/wiki/NUMA "wikilink") ハードウェアのサポートが行われることによりサーバー資源の効率化が可能となった。アプリケーション プールのサーバー資源の利用調整機能の強化が行われ、複数の Web サイトを単一のアプリケーション プールで展開した際のパフォーマンス低下の防止が可能となった。
    ワーカープロセスの[サンドボックス化などの安全性の向上が行われた](https://ja.wikipedia.org/wiki/サンドボックス_\(セキュリティ\) "wikilink")。
  - IIS 8.5
...
IIS は当初、多くのそして重大な[セキュリティホール](https://ja.wikipedia.org/wiki/セキュリティホール "wikilink")が頻繁に発見された。過去には[Code Redや](https://ja.wikipedia.org/wiki/Code_Red "wikilink")[Nimda](https://ja.wikipedia.org/wiki/Nimda "wikilink")といった[ワームの蔓延により大規模な障害を引き起こした](https://ja.wikipedia.org/wiki/ワーム_\(コンピュータ\) "wikilink")。特にWindows 2000では標準で組み込まれるため被害を大きくした。
...
1.
2.  [NTFSの](https://ja.wikipedia.org/wiki/NT_File_System "wikilink")[ACLなど](https://ja.wikipedia.org/wiki/アクセス制御リスト "wikilink")、[Windows NT系システムでのパーミッション管理について一定の知識を要する](https://ja.wikipedia.org/wiki/Windows_NT系 "wikilink")。
3.
4.
5.
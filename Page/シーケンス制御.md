...
自動[洗濯機](../Page/洗濯機.md "wikilink")や[エアコン](https://ja.wikipedia.org/wiki/エアコン "wikilink")などの[家電](https://ja.wikipedia.org/wiki/家電 "wikilink")品には、それぞれの機器1台につき1個の専用のプログラムが内蔵され、ワンチップの[マイクロコントローラ](https://ja.wikipedia.org/wiki/マイクロコントローラ "wikilink")によって制御されている。これにより機器の操作対象や環境の変化に応じた自動運転ができる。温度・時間・回数など設定する機能をプログラムとして組み込むことによりユーザーの意思どおりの制御ができる。一般にマイコン制御と呼ばれる。

...
[Print_Server_FPSU2BF.jpg](https://ja.wikipedia.org/wiki/File:Print_Server_FPSU2BF.jpg "fig:Print_Server_FPSU2BF.jpg") **プリントサーバ**は、[コンピュータネットワーク](../Page/コンピュータネットワーク.md "wikilink")上に配置されたある[プリンター](https://ja.wikipedia.org/wiki/プリンター "wikilink")を、複数のクライアントコンピュータから利用する際に用いられる、制御用コンピュータである。プリントサーバは複数の[クライアントから同時に印刷要求があっても](../Page/クライアント_\(コンピュータ\).md "wikilink")、これを適切に処理し、プリンターが順序よく印刷をこなすための管理を行う。

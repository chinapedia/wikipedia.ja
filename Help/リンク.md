...
**ウィキリンク**（または**内部リンク**）は、日本語版ウィキペディア内にある別のページへリンクします。ウィキリンクを表記するには、リンクする先のページ名を2連の角括弧 `[[|]]` で囲みます。例えば、「`[[あい|あい]]`」と表記すると、「[あい](https://ja.wikipedia.org/wiki/あい "wikilink")」と表示され、ページ「あい」にリンクされます。
  ...
  - パイプ付きリンク
    ウィキリンクでは、[バーティカルバー](https://ja.wikipedia.org/wiki/バーティカルバー "wikilink")（`|`」、パイプ記号）を用いることで、リンクが付けられた文字列（リンクラベル）とリンク先のページ名を異なる文字列で表示することが可能です。例えば、「`[[あい|うえ]]`」と表記すると、「[うえ](https://ja.wikipedia.org/wiki/あい "wikilink")」と表示され、ページ「あい」にリンクされます。このリンク表記は「[**パイプ付きリンク**](https://ja.wikipedia.org/wiki/Help:パイプ付きリンク "wikilink")」と呼ばれています。
  - 赤リンク
...
[MediaWiki:Common.cssなどで指定をすれば](https://ja.wikipedia.org/wiki/MediaWiki:Common.css "wikilink")、PDFファイルへのリンクに[ファイル:Icons-mini-file_acrobat.gif](https://ja.wikipedia.org/wiki/ファイル:Icons-mini-file_acrobat.gif "wikilink")などの固有のアイコンをつけることができます。現在の設定では `https://a.pdf` は <https://a.pdf> を返します。
...
セクションの冒頭以外の任意の位置にアンカーをおくこともできます。それにはテンプレートを使うか、HTMLコードを使います。

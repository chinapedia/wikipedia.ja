...
OSEKにおけるISR（interrupt service routine[割り込みサービスルーチン](https://ja.wikipedia.org/wiki/割り込みサービスルーチン "wikilink")）には、OSのサービスを使用しない(つまりOSのクリティカルセクションか否かにかかわらず割り込みが受け付けられる)ISR1と、OSのサービスであるシステムコールを呼び出すことが出来る(つまりOSのクリティカルセクション以外でのみ割り込みが受け付けられる)ISR2がある。また、割り込みの禁止・許可を制御する機能も有している。OSEKでは、エンジン・モータのような角速度制御しているソフトウェアを割り込みで設計することを想定している。

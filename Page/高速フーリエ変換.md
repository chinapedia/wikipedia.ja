...
**高速フーリエ変換**（こうそくフーリエへんかん、）は、[離散フーリエ変換](https://ja.wikipedia.org/wiki/離散フーリエ変換 "wikilink")（）を計算機上で高速に計算する[アルゴリズム](../Page/アルゴリズム.md "wikilink")である。高速フーリエ変換の逆変換を逆高速フーリエ変換（）と呼ぶ。

## 歴史

高速フーリエ変換といえば一般的には1965年、 (J. W. Cooley) と[ジョン・テューキー](https://ja.wikipedia.org/wiki/ジョン・テューキー "wikilink") (J. W. Tukey) が発見した\[1\] とされているを呼ぶ\[2\]。同時期に[高橋秀俊](https://ja.wikipedia.org/wiki/高橋秀俊 "wikilink")がクーリーとテューキーとは全く独立にフーリエ変換を高速で行うためのアルゴリズムを考案していた\[3\]。しかし、[1805年](https://ja.wikipedia.org/wiki/1805年 "wikilink")頃に既に[ガウスが同様のアルゴリズムを独自に発見していた](https://ja.wikipedia.org/wiki/カール・フリードリヒ・ガウス "wikilink")\[4\]（本ページの外部リンク先に同じ文章PDFへのリンクがある）。ガウスの論文以降、地球物理学や気候や潮位解析などの分野などで測定値に対する調和解析は行われていたので、計算上の工夫を必要とする応用分野で受け継がれていたようである（たとえば、Robart L. Nowack: "Development of the FFT and Applications in Geophysics", in Proceedings of the Cornelius Lanczos International Centenary Conference,SIAM, ISBN 978-0898713398 (1994), pp.395--397、の中では Danielson and Lanczos（1942年）などの先行例をあげている。和書でも沼倉三郎：「測定値計算法」、森北出版、（1956年）には，人が計算を行う場合に一般の合成数Nに対してではないがある程度の大きさの合成数Nに対してどのように計算すればよいかについての説明をみることができる）。
...
これを直接計算したときの[時間計算量は](https://ja.wikipedia.org/wiki/計算複雑性理論 "wikilink")、[ランダウの記号](https://ja.wikipedia.org/wiki/ランダウの記号 "wikilink")を用いて表現すると  である。
...
[離散フーリエ変換](https://ja.wikipedia.org/wiki/離散フーリエ変換 "wikilink")を
...
このため、 の離散フーリエ逆変換を求めるには、
  ...
  -
    (1) [複素共役](https://ja.wikipedia.org/wiki/複素共役 "wikilink")を取り、 を求める、
...
[分割統治法](https://ja.wikipedia.org/wiki/分割統治法 "wikilink")を使ったアルゴリズムで、 のサイズの変換を、より小さいサイズである ,  のサイズの変換に分割していくことで高速化を図っている。
...
かつては実数の入力データに対するフーリエ係数の計算は (discrete Hartley transform, DHT) を用いればさらに効率化できると思われていた。しかしその後に、最適化された[離散フーリエ変換](https://ja.wikipedia.org/wiki/離散フーリエ変換 "wikilink") (discrete Fourier transform, DFT) アルゴリズムの方が、離散ハートリー変換アルゴリズムに比べて必要な演算回数が少ないことが判明した。また、ブルーン (Bruun) FFT アルゴリズムは実数入力に対して有利であると最初は云われていたが現在ではそうではない。

また、[偶奇の対称性を持つ実入力の場合にはさらに最適化ができて](https://ja.wikipedia.org/wiki/偶関数と奇関数 "wikilink")、DFTは[DCTや](https://ja.wikipedia.org/wiki/離散コサイン変換 "wikilink")となり、時間とメモリーに関してほぼ2倍の高速化が得られるから、そのような場合にはDFTのアルゴリズムを適用するのではなく、DCTやDSTを直接適用した計算によりフーリエ係数を求める方が良い。

## 応用

  - [スペクトラムアナライザ](https://ja.wikipedia.org/wiki/スペクトラムアナライザ "wikilink")
  - [OFDM変復調器](https://ja.wikipedia.org/wiki/直交周波数分割多重方式 "wikilink")

<!-- end list -->

  -
    [OFDM](https://ja.wikipedia.org/wiki/直交周波数分割多重方式 "wikilink")（[日本](https://ja.wikipedia.org/wiki/日本 "wikilink")および[欧州](https://ja.wikipedia.org/wiki/欧州 "wikilink")で[地上デジタルテレビジョン放送](https://ja.wikipedia.org/wiki/地上デジタルテレビジョン放送 "wikilink")や[ADSL](https://ja.wikipedia.org/wiki/ADSL "wikilink")等に用いられる[変調方式](https://ja.wikipedia.org/wiki/変調方式 "wikilink")）の実装は、[LSI化されたFFTおよびIFFT](https://ja.wikipedia.org/wiki/集積回路 "wikilink")（逆変換）をそれぞれ復調器および変調器を用いて行われている。
...
<!-- end list -->
  ...
  - [コンピュータ断層撮影](https://ja.wikipedia.org/wiki/コンピュータ断層撮影 "wikilink") (CT)、[核磁気共鳴画像法](https://ja.wikipedia.org/wiki/核磁気共鳴画像法 "wikilink") (MRI) 等
...
<!-- end list -->
  ...
  - 多[倍精度の乗除算](https://ja.wikipedia.org/wiki/倍精度浮動小数点数 "wikilink")
  - [自動列車停止装置](https://ja.wikipedia.org/wiki/自動列車停止装置 "wikilink") (例: [JR西日本の最新型車両](https://ja.wikipedia.org/wiki/西日本旅客鉄道 "wikilink")。[地上子](https://ja.wikipedia.org/wiki/地上子 "wikilink")が[発振](https://ja.wikipedia.org/wiki/発振 "wikilink")する周波数の検出に、高速フーリエ変換が用いられている)
  - [FFTアナライザ](https://ja.wikipedia.org/wiki/FFTアナライザ "wikilink")

<!-- end list -->

  -
    周波数の分布を調べるために使用される。以前はハードウェアで信号を処理していたが、近年は[CPU](https://ja.wikipedia.org/wiki/CPU "wikilink")の性能が向上した為[ソフトウェア](../Page/ソフトウェア.md "wikilink")で処理される。[ノートパソコン](../Page/ノートパソコン.md "wikilink")と[USB](https://ja.wikipedia.org/wiki/USB "wikilink")で接続して使用するもの\[7\] や、近年は[デジタルオシロスコープ](https://ja.wikipedia.org/wiki/デジタルオシロスコープ "wikilink")にFFTの機能を内蔵している物もある。
...
<!-- end list -->
  ...
  -
    [FX型デジタル分光相関器](https://ja.wikipedia.org/wiki/FX型デジタル分光相関器 "wikilink")等を使用して[星間分子](https://ja.wikipedia.org/wiki/星間分子 "wikilink")のスペクトルを解析する\[8\]\[9\]。
...
## 関連記事
  ...
  - [離散フーリエ変換](https://ja.wikipedia.org/wiki/離散フーリエ変換 "wikilink") (DFT)
  ...
  - [時系列](https://ja.wikipedia.org/wiki/時系列 "wikilink")

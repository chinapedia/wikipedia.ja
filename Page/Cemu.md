...
なお、[Intel](https://ja.wikipedia.org/wiki/インテル "wikilink") CPUは公式にはサポートされていない。

2020年1月現在、[Linux](../Page/Linux.md "wikilink")及び[macOS](https://ja.wikipedia.org/wiki/macOS "wikilink")版は存在しない。
...
現在、WiiU GamePad、WiiU PRO コントローラ、およびWiiクラシックコントローラ/クラシックコントローラPROがエミュレートされている。また、Wiiリモコンも使用できる（ネイティブサポートを含む）。そして入力デバイスとしてのキーボード入力+ USBコントローラーがサポートされている。GamePadタッチ入力は、マウスの左クリックで制御でき、ジャイロ機能は制限付きでエミュレートされ、マウスの右ボタン及びホイールで制御可能である。\[11\]

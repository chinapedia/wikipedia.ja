...
**ローカル変数**（**局所変数**、）とは、[プログラムの一部分でしか利用できない](../Page/プログラム_\(コンピュータ\).md "wikilink")[変数のことである](https://ja.wikipedia.org/wiki/変数_\(プログラミング\) "wikilink")。一般的に[グローバル変数](../Page/グローバル変数.md "wikilink")（大域変数）と対比される。ローカル変数の定義は[プログラミング言語](../Page/プログラミング言語.md "wikilink")によって異なるので、詳細な説明は言語別の項に譲る。

## C言語

[C言語](../Page/C言語.md "wikilink")および[C++](../Page/C++.md "wikilink")のローカル変数は、[関数内の同じ](https://ja.wikipedia.org/wiki/サブルーチン "wikilink")「[ブロック](../Page/ブロック_\(プログラミング\).md "wikilink")」という領域内からのみ参照可能な変数である。[C99](../Page/C99.md "wikilink")よりも前の規格ではブロックの先頭部分でのみ定義可能だったが、C99からはC++同様に任意の位置で定義可能である。

ある関数fにおけるローカル変数aの[スコープ](https://ja.wikipedia.org/wiki/スコープ "wikilink") (scope) すなわち可視性は、その関数f内において、その変数aが宣言された場所から、その変数aが定義された[ブロックを抜けるまでである](../Page/ブロック_\(プログラミング\).md "wikilink")。スコープ外からは参照することができない。ローカル変数aを定義したブロック内で別の関数gを呼び出すと、関数gからはその変数aが見えなくなるが、メモリ上には依然として存在しており、関数gが終了して関数fに戻り、変数aのスコープに入ると再び見えるようになる。
...
[Java](https://ja.wikipedia.org/wiki/Java "wikilink")における[変数は大別して](https://ja.wikipedia.org/wiki/変数_\(プログラミング\) "wikilink")4種類ある\[5\]。

...
WAVフォーマットでは、データ長が32ビット符号なし[整数型](../Page/整数型.md "wikilink")で記述されているため、4GBを超えるファイルを作成できない。この制限を越えるため、データ長を[64ビット](https://ja.wikipedia.org/wiki/64ビット "wikilink")符号なし整数型で記述するWave64 (.w64) というフォーマットも存在する。

...
## 補足
  ...
  - envを用いたトリックはPATH[環境変数](https://ja.wikipedia.org/wiki/環境変数 "wikilink")に依存する。親プロセスが独自にPATHを設定していた場合、想定外の動作をする可能性がある。`ruby` インタプリタの場合は `-x` オプションを利用した以下のようなシェルスクリプトに見せかけるトリックを使用したほうが良い。

...
指数関数を含む方程式の多くは、W関数を用いることで解くことができる。主な方針は、[未知数](https://ja.wikipedia.org/wiki/未知数 "wikilink")を含む項を方程式の左辺（あるいは右辺）に寄せ、W関数で解を表現できる \(xe^x\)の形にすることである。例えば、方程式 \(2^t = 5t\) を解くには、両辺に \(-\frac{\ln2\cdot 2^{-t}}{5}\)を掛け、\(-\frac{\ln2}{5} = -t \ln2 \cdot e^{-t\ln 2}\)を得る。

ここで、両辺のW関数をとれば、\(W\left(-\frac{\ln2}{5}\right) = -t \ln 2\)、即ち \(t=-\frac {W \left (-\frac{\ln{2}}{5}\right)}{\ln{2}} = 0.235\dots, 4.488\dots\)となる。

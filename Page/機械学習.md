...
機械学習では、[センサ](../Page/センサ.md "wikilink")や[データベース](../Page/データベース.md "wikilink")などに由来するサンプルデータを入力して解析を行い、そのデータから有用な規則、ルール、知識表現、判断基準などを抽出し、[アルゴリズム](../Page/アルゴリズム.md "wikilink")を発展させる。

そのアルゴリズムは、まずそのデータ（訓練例と呼ぶ）を生成した潜在的機構の特徴（[確率分布](../Page/確率分布.md "wikilink")）を捉え、複雑な関係を識別・定量化する。次に学習・識別したパターンを用いて新たなデータについて予測・決定を行う。データは、観測された変数群のとる関係の具体例と見ることができる\[1\]。

1つの根本的な課題は、全ての取りうるデータを訓練例（学習データ）に含めるのは（多くの実用的な関心事の場合）データ数が大きすぎて現実的でないという点である。したがってアルゴリズムは限られた訓練例を一般化して、新たなデータ入力から有用な出力を生成しなければならない\[2\]。

機械学習は[検索エンジン](../Page/検索エンジン.md "wikilink")、[医療診断](https://ja.wikipedia.org/wiki/医療診断 "wikilink")、[スパムメールの検出](../Page/スパム_\(メール\).md "wikilink")、金融市場の予測、[DNA配列の分類](../Page/デオキシリボ核酸.md "wikilink")、[音声認識](../Page/音声認識.md "wikilink")や[光学文字認識](../Page/光学文字認識.md "wikilink")などの[パターン認識](../Page/パターン認識.md "wikilink")、ゲーム戦略、[ロボット](../Page/ロボット.md "wikilink")、など幅広い分野で用いられている。応用分野の特性に応じた様々な学習手法が提案されている\[3\]。

## 定義

[アーサー・サミュエル](https://ja.wikipedia.org/wiki/アーサー・サミュエル "wikilink")は1959年に一般紙に対するインタビューとして、機械学習を「明示的に[プログラミング](../Page/プログラミング.md "wikilink")することなく、コンピュータに学ぶ能力を与えようとする研究分野\[4\]」と定義した。

は、よく引用されるさらに厳格な定義として「コンピュータプログラムが、ある種のタスクTと評価尺度Pにおいて、経験Eから学習するとは、タスクTにおけるその性能をPによって評価した際に、経験Eによってそれが改善されている場合である」とした\[5\]。
...
機械学習システムによっては、人間の[直観](https://ja.wikipedia.org/wiki/直観 "wikilink")によるデータ解析の必要性を排除しようとしているが、人間と機械の協調的相互作用を取り入れたものもある。しかし、そもそもシステムのデータ表現方法やデータの特徴を探る機構は、人間が設計したものであり、人間の直観を完全に排除することはできない。
...
機械学習アルゴリズムとその性能についての分析は、[理論計算機科学](../Page/理論計算機科学.md "wikilink")の一分野であり、と呼ばれている。訓練例は有限であるのに対して、未来は不確かであるため、学習理論は一般にアルゴリズムの性能を保証できない。その代わりに、性能の確率的範囲を与える。 によるなど統計的学習理論という表現もある。\[7\]
...
統計的機械学習とは、機械学習のうちデータの**確率的な生成規則**を学習するもの<ref>安田 宗樹. "統計的機械学習理論と ボルツマン機械学習" 山形大学

<https://www.r-ccs.riken.jp/labs/cms/workshop/20170322/presentation/yasuda.pdf></ref>を指す。
...
統計的機械学習の例としては、[ニューラルネットワーク](../Page/ニューラルネットワーク.md "wikilink")における生成モデル、例えば自己回帰型生成ネット、変分オートエンコーダ（VAE）、[敵対的生成ネットワーク](https://ja.wikipedia.org/wiki/敵対的生成ネットワーク "wikilink")（GAN）などが挙げられる。これらのモデル（＝母集団）から実際にサンプリングすれば画像や音声といったデータが生成できるため、特にニューラルネットワークの分野では2010年代後半に非常によく研究され大きな成果をあげている（WaveNet、VQ-VAE-2、BigGANなど）。
...
## 技法
  ...
  - [決定木](https://ja.wikipedia.org/wiki/決定木 "wikilink")学習
    [決定木](https://ja.wikipedia.org/wiki/決定木 "wikilink")をとして使用した学習であり、アイテムについての観測をそのアイテムの目標値についての結論とマッピングする。具体例として[ID3](../Page/ID3.md "wikilink")や[Random forestがある](https://ja.wikipedia.org/wiki/Random_forest "wikilink")。
  -
    大規模データベースにおける変数間の興味深い関係を発見するための技法。
  - [ニューラルネットワーク](../Page/ニューラルネットワーク.md "wikilink") (NN)
    人工ニューラルネットワーク (ANN) とも呼ばれ、生物の[神経](https://ja.wikipedia.org/wiki/神経 "wikilink")ネットワークの構造と機能を模倣するという観点から生まれた学習アルゴリズムである。[人工神経](../Page/人工神経.md "wikilink")を相互接続したもので計算を構造化し、[コネクショニズム](https://ja.wikipedia.org/wiki/コネクショニズム "wikilink")的計算技法で情報を処理する。現代的ニューラルネットワークは[非線形な](../Page/非線形システム論.md "wikilink")[統計的](../Page/統計学.md "wikilink")[データモデリング](https://ja.wikipedia.org/wiki/データモデリング "wikilink")ツールである。入力と出力の間の複雑な関係をモデル化するのに使われ、データの[パターン認識](../Page/パターン認識.md "wikilink")や観測された変数間の未知の[同時分布](https://ja.wikipedia.org/wiki/同時分布 "wikilink")における統計的構造を捉えるなどの用途がある。
  - [遺伝的プログラミング](https://ja.wikipedia.org/wiki/遺伝的プログラミング "wikilink") (GP)
...
<!-- end list -->
  ...
  - [製造業](../Page/製造業.md "wikilink")\[9\]

2006年、オンラインDVDレンタル会社[ネットフリックス](https://ja.wikipedia.org/wiki/ネットフリックス "wikilink")は、同社の[レコメンダシステム](https://ja.wikipedia.org/wiki/レコメンダシステム "wikilink")より10%以上高性能な（ユーザーの好みをより正確に予測する）プログラムを捜す競技会 [Netflix Prize](https://ja.wikipedia.org/wiki/:en:Netflix_Prize "wikilink") を開催した。この競技会は数年かけて行われ、AT\&T Labs のチームが「プラグマティック・ケイオス」\[10\]という機械学習プログラムで2009年に優勝し100万ドルを獲得した\[11\]。
...
データロボット社\[12\]による複数の手法を並列計算させて比較する方法がある\[13\]。 また、Pythonによるソースコードの公開が広がっており、docker環境でどこでも機械学習を実行できるようになってきた\[14\]。
...
2.
3.  それらの手法は、*Machine Learning* や *IEEE Transactions on Pattern Analysis and Machine Intelligence* などの学術雑誌などで発表されることが多い。
4.
5.  Definition: A computer program is said to **learn** from experience *E* with respect to some class of tasks *T* and performance measure *P*, if its performance at tasks in *T*, as measured by *P*, improves with experience *E*. Mitchell, T. (1997). *Machine Learning*, McGraw Hill. ISBN 0-07-042807-7, p.2.
...
8.
9.  製造業における機械学習 <https://qiita.com/kaizen_nagoya/items/fbe846de16f74bea1d6f>
10.
11. ["BelKor Home Page"](http://www2.research.att.com/~volinsky/netflix/) research.att.com
12.
13. DataRobot: <https://www.datarobot.com>
14. なぜdockerで機械学習するか 書籍・ソース一覧作成中 (目標100) <https://qiita.com/kaizen_nagoya/items/ddd12477544bf5ba85e2>
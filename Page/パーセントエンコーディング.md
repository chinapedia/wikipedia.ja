...
URL Standardでは、URLのパス部分の構文解析の際、以下 (path percent-encode set) に該当する文字であれば、[UTF-8](../Page/UTF-8.md "wikilink")で符号化のうえパーセントエンコードする旨を規定している ([URL Standard 4.4 URL parsing, path state](https://url.spec.whatwg.org/#path-state))。パーセントエンコードとは、バイトの並びについて各バイトを「%XX」（XXは[十六進法](../Page/十六進法.md "wikilink")）という文字列への変換である。
  ...
  - U+0023 [番号記号](https://ja.wikipedia.org/wiki/番号記号 "wikilink") `#`
  - U+003C 小なり `<`
  - U+003E 大なり `>`
  - U+0060 バッククオート `` ` ``
  - U+007B 開き波括弧 `{`
...
[HTTPの](../Page/Hypertext_Transfer_Protocol.md "wikilink")[POSTメソッドで](https://ja.wikipedia.org/wiki/Hypertext_Transfer_Protocol#メソッド "wikilink")[Webフォームの文字列を送信する場合に](https://ja.wikipedia.org/wiki/フォーム_\(ウェブ\) "wikilink")、文字列はエンコードして送信される。その際の符号化方法は[MIMEの](https://ja.wikipedia.org/wiki/Multipurpose_Internet_Mail_Extensions "wikilink")[Content-Typeがapplication](https://ja.wikipedia.org/wiki/メディアタイプ "wikilink")/x-www-form-urlencodedで指定される。この符号化方法を**URLエンコード**と言うことがある。

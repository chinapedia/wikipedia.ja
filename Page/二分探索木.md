...
平衡（左右のバランスがとれている状態）している状態では木の高さは [O(log<sub>2</sub> N)](https://ja.wikipedia.org/wiki/O記法 "wikilink") となる。ただし最悪の場合は、事実上の [線形リスト](https://ja.wikipedia.org/wiki/線形リスト "wikilink") になり、木の高さは O(N) となる。木の形は挿入時のデータ出現順序に依存し、特にソート済みのデータを与えると線形リストになる点は注意を要する。データの出現順序によって大きく性能が劣化しないように、挿入・削除の際に木の平衡を取り直す処理を追加した二分探索木は[平衡二分探索木](../Page/平衡二分探索木.md "wikilink")と呼ばれる。

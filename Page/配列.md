...
この記事では、[コンピュータ・プログラムにおいて](https://ja.wikipedia.org/wiki/プログラム_\(コンピュータ\) "wikilink")**配列**（はいれつ、）と呼ばれている[データ構造](../Page/データ構造.md "wikilink")および[データ型](https://ja.wikipedia.org/wiki/データ型 "wikilink")について説明する。[計算科学](https://ja.wikipedia.org/wiki/計算科学 "wikilink")方面では[ベクトル](https://ja.wikipedia.org/wiki/ベクトル "wikilink")という場合もある。また、[リストも参照](https://ja.wikipedia.org/wiki/リスト_\(抽象データ型\) "wikilink")。一般に、添え字で個々の要素を区別する。

## 配列とは

複数の要素（値）の集合を格納・管理するのに用いられるデータ構造が配列である。数学の[ベクトル](https://ja.wikipedia.org/wiki/ベクトル "wikilink")および[行列](https://ja.wikipedia.org/wiki/行列 "wikilink")に近い概念であり、実際にベクトルおよび行列をプログラム上で表現する場合に配列が使われることが多い。同様に複数要素の集合を管理するデータ構造（コレクションあるいはコンテナ）には[連結リスト](../Page/連結リスト.md "wikilink")や[ハッシュテーブル](../Page/ハッシュテーブル.md "wikilink")などがあるが、通常はメモリアドレス上での連続性の違いなどから配列とは区別される。1次元の配列は特に線形配列 (linear array) とも呼ばれる。
...
いずれにせよ、C/C++ではmallocあるいはnewによって[ヒープ領域](https://ja.wikipedia.org/wiki/ヒープ領域 "wikilink")から確保したメモリは明示的に解放する必要があり、解放を忘れると[メモリリーク](https://ja.wikipedia.org/wiki/メモリリーク "wikilink")の原因となる。プログラミングの煩雑さを解消するため、[C++](../Page/C++.md "wikilink")では[コンストラクタ](https://ja.wikipedia.org/wiki/コンストラクタ "wikilink")・[デストラクタ](https://ja.wikipedia.org/wiki/デストラクタ "wikilink")を使ったメモリ寿命管理手法 ([RAII](https://ja.wikipedia.org/wiki/RAII "wikilink")) が使われることが多い。[Java](https://ja.wikipedia.org/wiki/Java "wikilink")などの後発の言語では[ガベージコレクション](../Page/ガベージコレクション.md "wikilink")による自動解放を導入していることが多く、また配列の確保に関して静的確保・動的確保といった区別をしない（配列の確保はすべて動的確保である）ことが多い。
...
「配列」という語は、抽象データ型というよりも、添え字をオフセットとしてメモリのアドレスにマップすることで、*Ο*(1) でアクセスできる具象あるいは実装を特に指す場合がある（その意味では、次節の[連想配列](https://ja.wikipedia.org/wiki/連想配列 "wikilink")などは「配列」ではない、ということになる）。
...
添え字は一般に通例0か1始まりの (非負) 整数である。一方、文字列など他の[データ型](https://ja.wikipedia.org/wiki/データ型 "wikilink")を添え字のように使用できる配列を[連想配列](https://ja.wikipedia.org/wiki/連想配列 "wikilink")という。
...
要素数によって自動的にサイズが拡張する配列を、動的配列 (dynamic array) あるいは[可変長配列](https://ja.wikipedia.org/wiki/可変長配列 "wikilink") (variable-length array) と呼ぶ。メモリが許す限り、要素の末尾追加や途中挿入がいくらでもできる。ライブラリで提供されるもの（[C++](../Page/C++.md "wikilink")のstd::vector、[Java](https://ja.wikipedia.org/wiki/Java "wikilink")のjava.util.ArrayList、[.NETのSystem](https://ja.wikipedia.org/wiki/.NET_Framework "wikilink").Collections.ArrayListやSystem.Collections.Generic.List\[1\]など）と、言語に組み込まれているもの（[Perl](../Page/Perl.md "wikilink")や[Dなど](https://ja.wikipedia.org/wiki/D言語 "wikilink")）がある。またPerlなど、言語によっては、最初に配列を生成する際に指定されたサイズからはみ出してアクセス（範囲外アクセス）しても、自動的に拡大されるような配列を持っているものもある。

逆に決まった要素数しか格納できない配列を、静的配列 (static array) あるいは固定長配列 (fixed length array) と呼ぶ。

なおC言語では、下記のように、実行時に（整数定数式ではない）要素数を指定してスタック上に自動変数として確保することのできる静的配列を[可変長配列](https://ja.wikipedia.org/wiki/可変長配列 "wikilink") (variable-length array) と呼んでいる\[2\]。[GCCに拡張として実装されていたが](../Page/GNUコンパイラコレクション.md "wikilink")、[C99](../Page/C99.md "wikilink")以降で標準化された。この可変長配列は後から要素を追加したりすることはできない。
...
要素のアドレスを指定するための参照の展開は、ジャグ配列では次元の数だけ必要なのに対し、矩形配列では1回で済む。また配列の領域を確保する際、ジャグ配列では次元ごとに領域確保を繰り返す必要があるのに対し、矩形配列では1回の[new演算子](https://ja.wikipedia.org/wiki/new演算子 "wikilink")の使用で領域が確保できる。ただし矩形配列は全要素が収まる連続領域を確保しなければならず、[疎行列](https://ja.wikipedia.org/wiki/疎行列 "wikilink")などのまばらな配列には空間的オーバーヘッドが大きくなってしまうことから向いていない。また、[.NET Frameworkの中間言語には](https://ja.wikipedia.org/wiki/共通中間言語 "wikilink")1次元配列の要素アクセスに関する専用命令が存在するため、矩形配列よりもジャグ配列のほうが速度性能面で有利になるケースも存在する\[5\]。

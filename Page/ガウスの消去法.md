...
行列に対して掃き出し法を行う為には、[行に関する基本変形を行列に可能な限り繰り返し行って行列の左下部分の成分を全て](https://ja.wikipedia.org/wiki/行列の基本変形 "wikilink") 0 にする。行に関する基本変形には、
...
## 注意
  ...
  - [疎行列](https://ja.wikipedia.org/wiki/疎行列 "wikilink")に対してガウスの消去法のステップを行うと[疎性](https://ja.wikipedia.org/wiki/疎性 "wikilink")を損なう。
  - 前進消去の段階において[対角化](https://ja.wikipedia.org/wiki/対角化 "wikilink")を目指して、後退代入を行わずに *x* を直接計算する方法は**ガウス・ジョルダンの消去法**（）と呼ぶ。アルゴリズムは単純になるが、[計算量は多くなる](https://ja.wikipedia.org/wiki/計算複雑性理論 "wikilink")（変数が多い場合、ほぼ 1.5 倍になる）。

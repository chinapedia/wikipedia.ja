...
**QVT**（Queries/Views/Transformations）とは、[Object Management Groupが定義した](https://ja.wikipedia.org/wiki/Object_Management_Group "wikilink")[モデル駆動型アーキテクチャ](../Page/モデル駆動型アーキテクチャ.md "wikilink")における[モデル変換](../Page/モデル変換.md "wikilink")の標準である。[Meta-Object Facility](../Page/Meta-Object_Facility.md "wikilink")(MOF)に関連する標準であることから、**MOF QVT** とも呼ばれる。

## 解説

モデル変換とは、メタモデル MMa に準拠したモデル Ma を[メタモデル](../Page/メタモデル.md "wikilink") MMb に準拠したモデル Mb に変換するプロセスである。MMa=MMb であれば、その変換を**内発的**（endogeneous）といい、そうでなければ**外発的**（exogeneous）という。モデル変換はモデル駆動型アーキテクチャ(MDA)において重要な役割を担う。このため、OMG は [MOF](../Page/Meta-Object_Facility.md "wikilink") **Query/Views/Transformations** の RFP（Request for proposal）を発行し、MDA 関連の推奨規格（[UML](../Page/統一モデリング言語.md "wikilink")、MOF、[OCLなど](https://ja.wikipedia.org/wiki/Object_Constraint_Language "wikilink")）に互換性のある標準を求めた。いくつかの企業や研究機関がこれに応じ、3年をかけて共同提案が策定され、標準として承認された。
...
また、*QVT/BlackBox* という機構で他の言語（[XSLT](../Page/XSL_Transformations.md "wikilink")、[XQuery](https://ja.wikipedia.org/wiki/XQuery "wikilink")）で表現された変換機能を呼び出すようになっており、これも仕様の重要な部分を占める。これは特に既存の QVT 以外のライブラリを統合する際に重要となる。
...
## 実装
  ...
  - **[M2M](https://ja.wikipedia.org/wiki/M2M_\(Eclipse\) "wikilink")** - OMG QVT 標準の Eclipse による実装。M2Mは、[ATL](https://ja.wikipedia.org/wiki/ATLAS_Transformation_Language "wikilink")、Compuware社による宣言型QVTの実装、Borlandによる命令型QVTの実装、から構成される。
...
宣言型QVT(QVT-Relations) :
  ...
  - [ModelMorf](http://www.tcs-trddc.com/ModelMorf/index.htm) - [タタ・コンサルタンシー・サービシズ](https://ja.wikipedia.org/wiki/タタ・コンサルタンシー・サービシズ "wikilink")の子会社 [TRDDC](http://www.tcs-trddc.com) による変換エンジン。独自仕様だが部分的に QVT 準拠。オープンソースではない。
...
QVTに似た言語 ：
  ...
  - **[ATL](https://ja.wikipedia.org/wiki/ATLAS_Transformation_Language "wikilink")** - QVT 風の変換言語とエンジン。多数のユーザーがおり、オープンソースの変換ライブラリも豊富。
...
3.  **時期尚早な標準化**。これまでの所、このような手法が産業界で大規模に使われた例がない。この時点で標準として膨大な推奨仕様を策定するのは無謀であるとの見方もある。策定を急いだのは、[W3C](../Page/World_Wide_Web_Consortium.md "wikilink") の XMLベースの標準である [XSLT](../Page/XSL_Transformations.md "wikilink") や [XQuery](https://ja.wikipedia.org/wiki/XQuery "wikilink") に対抗するためと言われている。
...
## 関連項目
  ...
  - [Object Constraint Language](https://ja.wikipedia.org/wiki/Object_Constraint_Language "wikilink") (OCL): モデル制約（クエリ）言語

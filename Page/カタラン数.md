...
カタラン数は様々な意味付けがなされている。以下に例を示す。
  ...
  - [二分木](https://ja.wikipedia.org/wiki/二分木 "wikilink"):[center](https://ja.wikipedia.org/wiki/ファイル:Catalan_number_binary_tree_example.png "wikilink")*C* は、*n* 個の分岐を持つ（*n* + 1 枚の葉を持つ）二分木の総数である。上記の図は C = 5 の場合に対応している。
    格子状の経路数え上げ
...
*n* が十分大きいとき、次の式でカタラン数を[近似](https://ja.wikipedia.org/wiki/近似 "wikilink")することができる（なおこれは[ウォリスの公式](https://ja.wikipedia.org/wiki/ウォリスの公式 "wikilink")から証明できる）：

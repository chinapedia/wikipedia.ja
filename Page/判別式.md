...
[数学](../Page/数学.md "wikilink")において、[多項式](../Page/多項式.md "wikilink")の**判別式**（はんべつしき、）とは、その多項式の根が重根を持つための[条件を与える](../Page/論理包含.md "wikilink")、係数の[多項式](../Page/多項式.md "wikilink")で最小のもののことである。通常は、大文字の  あるいは大文字の  で表記される。具体的には、以下の式で定義される：
  ...
  -
    ...
    :<math>D = {a_n}^{2n-2} \\textstyle \\prod\\limits_{i\<j} ( \\alpha_i - \\alpha_j )^2 =(-1)^{n(n-1)/2} {a_n}^{2n-2} \\prod\\limits_{i \\neq j} ( \\alpha_i - \\alpha_j )

</math> この定義式は、次の手順から、係数  の分数式である。
...
判別式  を係数  の多項式で表すには、[終結式](https://ja.wikipedia.org/wiki/終結式 "wikilink")（[シルヴェスター行列](https://ja.wikipedia.org/wiki/シルヴェスター行列 "wikilink")の[行列式](../Page/行列式.md "wikilink")）を用いるのが最も簡明である：
...
より高次の方程式に対しても、判別式は定義され、係数たちの多項式であるが、その式は非常に長くなる。[五次方程式](../Page/五次方程式.md "wikilink")の判別式は 59 の項を持ち\[1\]、の判別式は 246 の項を持ち\[2\]、項の個数は次数によって指数的に増加する。

四次までの[代数方程式](../Page/代数方程式.md "wikilink")に対しては、判別式は解の公式に現れるため、判別式の定義とは、解の公式の一部と誤解されがちであるが、解の公式が存在しない五次以上の代数方程式（[アーベル-ルフィニの定理](https://ja.wikipedia.org/wiki/アーベル-ルフィニの定理 "wikilink")）に対しても、判別式は定義される。

定義式から、判別式の値が  であるのは、重根（すなわち[重複度が](https://ja.wikipedia.org/wiki/重複度_\(数学\) "wikilink") 2以上の根）が存在することと[同値](../Page/同値.md "wikilink")である。

実数係数の代数方程式の実数解の個数は、二次方程式では、判別式の[符号により判別できるが](../Page/符号_\(数学\).md "wikilink")、三次以上では、判別式以外にも指標となる式が必要となる（詳しくは、[三次方程式\#解の様子](https://ja.wikipedia.org/wiki/三次方程式#解の様子 "wikilink")、[四次方程式\#解の様子](https://ja.wikipedia.org/wiki/四次方程式#解の様子 "wikilink")などを参照）。

用語 "discriminant" はイギリス人数学者[ジェームス・ジョセフ・シルベスター](https://ja.wikipedia.org/wiki/ジェームス・ジョセフ・シルベスター "wikilink") () によって[1851年](../Page/1851年.md "wikilink")に造り出された\[3\]。

判別式の概念は、係数[体が](https://ja.wikipedia.org/wiki/可換体 "wikilink")[複素数](../Page/複素数.md "wikilink")体に含まれない体である場合にも適用できる。係数たちが[整域](https://ja.wikipedia.org/wiki/整域 "wikilink")  に属していさえすれば定義され、この場合、判別式は  の元である。特に、整数係数多項式の判別式は常に整数である。この性質は[数論](../Page/数論.md "wikilink")において広く用いられる。
...
\[{a_n}^{2n-2} \textstyle \prod\limits_{i<j} ( \alpha_i - \alpha_j )^2\]
...
（注）
  ...
  - （注1）この行列式において、第1列は  で割り切れるため、右辺は  の  次[斉次多項式](https://ja.wikipedia.org/wiki/斉次多項式 "wikilink")である。
  - （注2）この行列式は  と  の[終結式](https://ja.wikipedia.org/wiki/終結式 "wikilink")であり、記号で \(\operatorname{Res} (f,f')\) と表される。
...
\[\textstyle \prod\limits_{i<j} ( \alpha_i - \alpha_j )^2\]

\[= (-1)^{n(n-1)/2} \textstyle \prod\limits_{i \neq j} ( \alpha_i - \alpha_j )\]

\[= (-1)^{n(n-1)/2} \textstyle \prod\limits_{i=1}^n \prod\limits_{j \neq i} ( \alpha_i - \alpha_j ) \quad \cdots \ (1)\]

\[f(x)= a_n \textstyle \prod\limits_{j=1}^n ( x- \alpha_j )\]

\[f'(x)= a_n \textstyle \sum\limits_{k=1}^n \prod\limits_{j \neq k} ( x- \alpha_j )\]

\[f'( \alpha_i )= a_n \textstyle \prod\limits_{j \neq i} ( \alpha_i - \alpha_j ) \quad \cdots \ (2)\]
  ...
  -
    ...
    (2) = (3) より、 に注意して
    \(\textstyle \prod\limits_{j \neq i} ( \alpha_i - \alpha_j ) = n \prod\limits_{j=1}^{n-1} ( \alpha_i - \beta_j ) \quad \cdots \ (4)\)
    (4) を (1) に代入すると、
    \(\textstyle \prod\limits_{i<j} ( \alpha_i - \alpha_j )^2 = (-1)^{n(n-1)/2} n^n \prod\limits_{i,j} ( \alpha_i - \beta_j ) \quad \cdots \ (5)\)
...
\[\frac{D}{4} = b'^2 - ac\] となり、実数解の個数を考える時に使用されることが多い。
...
より一般に、実数係数の  次[代数方程式](../Page/代数方程式.md "wikilink")に対して、
  ...
  - ：\(0 \leq k \leq \frac{n}{4}\) なるある整数  に対して、 組の共役虚数解と  個の実数解があり、全て異なる；

  - ：\(0 \leq k \leq \frac{n-2}{4}\) なるある整数  に対して、 組の共役虚数解のと  個の実数解があり、全て異なる；

  - 0}}：少なくとも 1 個の重解が存在する。実数係数でも、重根は実数とは限らない（虚数の場合もある）。
...
1.  , [Preview page 1](http://blms.oxfordjournals.org/cgi/pdf_extract/28/1/96)
2.  , [Chapter 1 page 26](http://books.google.com/books?id=rSs-pQNrO_YC&pg=PA26)
3.  J. J. Sylvester (1851) "On a remarkable discovery in the theory of canonical forms and of hyperdeterminants," *Philosophical Magazine*, 4th series, **2** : 391-410; Sylvester coins the word "discriminant" on [page 406](http://books.google.com/books?id=DBNDAQAAIAAJ&pg=PA406#v=onepage&q&f=false).
4.

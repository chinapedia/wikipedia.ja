...
**グローバル変数**（**大域変数**、）は、[コンピュータプログラミングにおいて全ての](https://ja.wikipedia.org/wiki/プログラミング_\(コンピュータ\) "wikilink")[スコープ](https://ja.wikipedia.org/wiki/スコープ "wikilink")からアクセスできる[変数のことである](https://ja.wikipedia.org/wiki/変数_\(プログラミング\) "wikilink")。グローバル変数の寿命は、プログラムの生存期間と同じである。対する語は、[ローカル変数](https://ja.wikipedia.org/wiki/ローカル変数 "wikilink")である。[スコープ](https://ja.wikipedia.org/wiki/スコープ "wikilink")も参照。
...
しかしグローバル変数が適する状況もある。例えば、システム起動時に一度だけファイルから読み出し、以降は様々な関数を通して継続的によく使われるような設定情報を格納する変数を、[関数に毎回](https://ja.wikipedia.org/wiki/サブルーチン "wikilink")[引数](../Page/引数.md "wikilink")として渡すことを避けるために使用できる。また、並列実行する[スレッドや](../Page/スレッド_\(コンピュータ\).md "wikilink")[シグナルハンドラのように](https://ja.wikipedia.org/wiki/シグナル_\(Unix\) "wikilink")、呼び出し側/被呼び出し側の関係を持たないコード同士でセクション間情報を受け渡すためにも広く使われる。利用可能なメモリ容量が少ない[組み込みシステム](../Page/組み込みシステム.md "wikilink")のような厳しい環境では、グローバル変数を使用せざるをえない場面もある。
...
各々のソースファイルが暗黙の名前空間（[無名名前空間](https://ja.wikipedia.org/wiki/無名名前空間 "wikilink")）を定義するような言語では、グローバルな名前空間を持つ言語に見られる問題の多くが解消されているが、一部の問題は適切な[カプセル化](https://ja.wikipedia.org/wiki/カプセル化 "wikilink")を行わなければ解決できない。（[ミューテックス](https://ja.wikipedia.org/wiki/ミューテックス "wikilink")のような）適切なロック無しにグローバル変数を使用しているコードは、保護メモリ内の読み出し専用値以外は[スレッドセーフ](https://ja.wikipedia.org/wiki/スレッドセーフ "wikilink")ではない。

[ハンガリアン記法](https://ja.wikipedia.org/wiki/ハンガリアン記法 "wikilink")では、グローバル変数を使用していることをソースコード上で明確化するために、"g_"の接頭辞を付ける。
...
[Java](https://ja.wikipedia.org/wiki/Java "wikilink")や[C\#のような後発のオブジェクト指向言語はグローバル変数を持たないが](../Page/C_Sharp.md "wikilink")、クラスの静的[フィールド](https://ja.wikipedia.org/wiki/フィールド_\(計算機科学\) "wikilink")（クラス変数）がグローバル変数に相当する。静的フィールドは一度だけ呼び出される静的イニシャライザまたは静的コンストラクタにより初期化処理を制御することができるが、グローバル変数と同じく非局在的な性質を内包しているため、多用するとコード品質の低下を招く。特に、再代入可能な静的フィールドを外部に公開することは避けるべきとされている\[1\]。
...
## 関連項目
  ...
  - [ローカル変数](https://ja.wikipedia.org/wiki/ローカル変数 "wikilink")
  - [静的変数](https://ja.wikipedia.org/wiki/静的変数 "wikilink")
  - [クラス変数](https://ja.wikipedia.org/wiki/クラス変数 "wikilink")
  - [Singleton パターン](https://ja.wikipedia.org/wiki/Singleton_パターン "wikilink")

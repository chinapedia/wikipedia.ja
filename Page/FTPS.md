...
SFTP ([SSH File Transfer Protocol](https://ja.wikipedia.org/wiki/SSH_File_Transfer_Protocol "wikilink")) はSSHの上でFTPとは別個のプロトコルにてファイル転送を実現するコマンドである。そのためSFTPはSSHのポートを使い既定の[Well-known Portは](https://ja.wikipedia.org/wiki/ポート番号 "wikilink")22/tcpであり、FTPSとは全くの別物である。

## 概要

FTPの認証で送信されるユーザ名と[パスワード](https://ja.wikipedia.org/wiki/パスワード "wikilink")の電文は、暗号化されていない状態（[クリアテキスト](https://ja.wikipedia.org/wiki/クリアテキスト "wikilink")）であるため、第三者に[盗聴](https://ja.wikipedia.org/wiki/盗聴 "wikilink")・侵入される危険性がある。FTPSはその危険性を回避するために制定された。
...
### 対応サーバー
  ...
  - [ProFTPD](https://ja.wikipedia.org/wiki/ProFTPD "wikilink")

...
[2005年](https://ja.wikipedia.org/wiki/2005年 "wikilink")11月時点で、"POJO"という用語は主に、いかなる（主要な）Javaオブジェクトモデル、規約、フレームワーク（たとえばEJB）にも従わないJavaオブジェクトを指して使われている。しかし、モデルやフレームワークが将来的に変化する可能性や、またこのような分類自体が文脈に依存するという事実を考慮すると、その定義は本質的に不安定で曖昧である。

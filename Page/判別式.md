...
より高次の方程式に対しても、判別式は定義され、係数たちの多項式であるが、その式は非常に長大なものになる。[五次方程式](../Page/五次方程式.md "wikilink")の判別式は 59 の項を持ち\[1\]、の判別式は 246 の項を持ち\[2\]、項の個数は次数によって指数的に増加する。
...
実数係数の代数方程式の実数解の個数は、二次方程式では、判別式の[符号が正か零か負かにより](../Page/符号_\(数学\).md "wikilink")2個、1個（重複度2）、0個と判別できるが、三次の場合にはそれぞれ3個、2個（片方は重複度2）あるいは1個（重複度3），1個となる。 このように三次以上では、判別式以外にも指標となる式が必要となる（詳しくは、[三次方程式\#解の様子](https://ja.wikipedia.org/wiki/三次方程式#解の様子 "wikilink")、[四次方程式\#解の様子](https://ja.wikipedia.org/wiki/四次方程式#解の様子 "wikilink")などを参照）。

"discriminant"（判別式） という用語は[1851年](../Page/1851年.md "wikilink")にイギリス人数学者[ジェームス・ジョセフ・シルベスター](https://ja.wikipedia.org/wiki/ジェームス・ジョセフ・シルベスター "wikilink") () によって造り出された\[3\]。

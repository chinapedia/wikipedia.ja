...
LR法はいわゆる[ボトムアップ構文解析](../Page/ボトムアップ構文解析.md "wikilink")を行う。つまり、**葉**から始めて最上位の構文要素にたどり着く。
...
LR法には次のような欠点がある：
  ...
  - [LL法](../Page/LL法.md "wikilink")に比べ、コンパイルエラー時のエラーメッセージを適切に出しにくい。

LR構文解析器を1から書くのは難しく、[パーサジェネレータ](../Page/パーサジェネレータ.md "wikilink")を使って生成するのが一般的である。構文解析表の生成手法の違いにより、[単純LR法](../Page/単純LR法.md "wikilink")(SLR)、[LALR法](../Page/LALR法.md "wikilink")、[正規LR法](https://ja.wikipedia.org/wiki/正規LR法 "wikilink")に分類される。後者ほど大規模な文法を扱える（LALR法はSLR法より強力で、正規LR法はLALR法より強力である）。[yacc](https://ja.wikipedia.org/wiki/yacc "wikilink")はLALR法による構文解析器を生成する。

## LR法の構成

[rightの構成](https://ja.wikipedia.org/wiki/ファイル:LR_Parser.png "wikilink")\]\] 表に基づいた[ボトムアップ構文解析](../Page/ボトムアップ構文解析.md "wikilink")器の構成を右図に示す。構文解析器には、「入力[バッファ](../Page/バッファ.md "wikilink")」、状態を保持するための「[スタック](../Page/スタック.md "wikilink")」、次に遷移すべき状態や現在の状態で読み込んだ[終端記号](https://ja.wikipedia.org/wiki/終端記号 "wikilink")や[非終端記号](https://ja.wikipedia.org/wiki/非終端記号 "wikilink")に適用すべき文法規則を示す「アクション表」と「GOTO表」を持つ。これらの動きを示すため、以下の簡単な文法を使って説明する:
...
このスタックは[有限オートマトン](../Page/有限オートマトン.md "wikilink")の動作履歴のようなものであり、現在は非終端記号 E を読んだ直後に終端記号 '+' を読んだ状態を示している。このオートマトンの[状態遷移表](../Page/状態遷移表.md "wikilink")はアクション表の shift 指定と GOTO表から構成される。
...
[SLR法や](../Page/単純LR法.md "wikilink")[LALR法](../Page/LALR法.md "wikilink")のようなより洗練された方式では行全体が同じ reduce アクションとはならないような表構築手続きを実施する。つまり、これらの構文解析では LR(0) よりも適用可能な文法の範囲が広い。
...
これらの例では、非終端記号 *A* の Follow-set（[LL法](../Page/LL法.md "wikilink")参照）を使用して、*A* の還元規則を使うかどうかを決定すればよい。つまり、入力の次の記号が *A* の Follow-set にある場合だけ、規則 *A* → *w* を適用する。この方式を[単純LR法](../Page/単純LR法.md "wikilink")と呼ぶ。

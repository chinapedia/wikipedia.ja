...
導出原理を元とする導出の手法は、その後の[定理自動証明](https://ja.wikipedia.org/wiki/定理自動証明 "wikilink")に大きな影響を与え、また[Prolog](https://ja.wikipedia.org/wiki/Prolog "wikilink")などの[論理プログラミング](https://ja.wikipedia.org/wiki/論理プログラミング "wikilink")言語の基礎となった。

## 背景

述語論理式 *P* が[恒真であるかを証明する一般的な手続きは存在しないが](https://ja.wikipedia.org/wiki/恒真式 "wikilink")、1930年に発表された[エルブランの定理](https://ja.wikipedia.org/wiki/エルブランの定理 "wikilink")は[エルブラン領域の要素を論理式に代入して命題論理のレベルに落としその充足不能性を調べることで](https://ja.wikipedia.org/wiki/エルブランの定理#エルブラン領域 "wikilink")、*¬P* が充足不能（恒偽）であれば有限のステップで証明できることを保証している。また、エルブランの論文には[単一化](https://ja.wikipedia.org/wiki/単一化 "wikilink")アルゴリズムなど他の様々なものが含まれていた\[2\]。

1950年代以降、計算機上での定理証明の研究が活発になり、[ギルモアのアルゴリズム](https://ja.wikipedia.org/wiki/ギルモアのアルゴリズム "wikilink")(1960)や[デービス・パトナムのアルゴリズム](https://ja.wikipedia.org/wiki/デービス・パトナムのアルゴリズム "wikilink")(1958,1960) が考案された。デービス・パトナムのアルゴリズムには[連言標準形](https://ja.wikipedia.org/wiki/連言標準形 "wikilink")の使用や導出規則の考え方が含まれていた。しかし、これらはエルブランの定理の証明を直接計算機上で実現したような方法で、[エルブラン領域の要素を順次生成して変数を含まない論理式](https://ja.wikipedia.org/wiki/エルブランの定理#エルブラン領域 "wikilink")(*基礎例*)を作成し命題論理のレベルで充足不能性を調べるものだったため、不要な論理式が多数生成され、非常に効率が悪かった\[3\]。
...
*導出*とは二つの[節](https://ja.wikipedia.org/wiki/節 "wikilink")より新しい節を導き出す操作で、一方の節に含まれるリテラル \(l\) と、他方の節に含まれる否定リテラル \(\neg l\) をのぞき、その他のリテラルの[論理和](../Page/論理和.md "wikilink")をとることで、新しい節を得ることをいう。

[命題論理](https://ja.wikipedia.org/wiki/命題論理 "wikilink")での導出規則を式で表せば、 \(\frac{
l \vee P,
\quad \neg l \vee Q}
...
導出規則は[三段論法](https://ja.wikipedia.org/wiki/三段論法 "wikilink")や[前件肯定を一般化した規則となっている](https://ja.wikipedia.org/wiki/モーダスポネンス "wikilink")。 導出は[完全な証明系であることが知られている](https://ja.wikipedia.org/wiki/完全性 "wikilink")。

### 例

\(p \to q\) を節形式にすると \(\lnot p \lor q\) となる。\(C_R\) を\(C_1, C_2\) の導出節とすると、[前件肯定は以下の導出と同じである](https://ja.wikipedia.org/wiki/モーダスポネンス "wikilink")。
...
同様に、\(p \to q\)、\(q \to r\) の節形式 \(\lnot p \lor q\)、 \(\lnot q \lor r\) による[三段論法](https://ja.wikipedia.org/wiki/三段論法 "wikilink")は以下のようになる。
...
[述語論理](https://ja.wikipedia.org/wiki/述語論理 "wikilink")のリテラルには個体変数が含まれるため、リテラルと否定リテラルとを単純に比較するだけでは削除できるかどうか分からない。一階述語論理ではリテラルと否定リテラルそれぞれの[原子論理式](https://ja.wikipedia.org/wiki/原子論理式 "wikilink")が[単一化できる場合に導出を行う](https://ja.wikipedia.org/wiki/ユニフィケーション "wikilink")。

また、導出の対象となる節は、[冠頭標準形](https://ja.wikipedia.org/wiki/冠頭標準形 "wikilink")にして[存在記号](https://ja.wikipedia.org/wiki/存在記号 "wikilink")を削除した[スコーレム連言標準形の論理式である](https://ja.wikipedia.org/wiki/スコーレム標準形 "wikilink")。
...
論理式 *P* が[恒真であれば](https://ja.wikipedia.org/wiki/恒真式 "wikilink") \(\lnot P\) は充足不能（恒偽）であるため、節の集合に \(\lnot P\) を追加し導出を繰り返すことで空節 □ になれば、論理式 *P* が[恒真であることが証明できる](https://ja.wikipedia.org/wiki/恒真式 "wikilink")。

反駁により \(P\) が \(P_1, \dots ,P_n\) の論理的帰結か調べるアルゴリズムは以下のように表現できる。

1.  \(P_1, \dots ,P_n\) を[スコーレム連言標準形](https://ja.wikipedia.org/wiki/スコーレム標準形 "wikilink") \(C_1, \dots ,C_n\) に変換する。
2.  \(\lnot P\) を[スコーレム連言標準形](https://ja.wikipedia.org/wiki/スコーレム標準形 "wikilink") \(C\) に変換する。
...
<!-- end list -->
  ...
  -
    前提となる節の一方を、頂上節（）として指定した節と、頂上節から導出された節に限定する方法。導出木を書くと導出の流れが線状に一列に並ぶため、線形導出と呼ばれる。[論理プログラミング](https://ja.wikipedia.org/wiki/論理プログラミング "wikilink")言語の代表である[Prolog](https://ja.wikipedia.org/wiki/Prolog "wikilink")で用いられる*SLD導出*（）は線形導出の一種である。

...
`--posix`オプションを付けてBashを呼び出したり、スクリプトに`set -o posix`を記述すると、Bashは[POSIX](../Page/POSIX.md "wikilink") 1003.2 standardに非常に良く準拠する\[36\]。[移植を前提としたBashシェルスクリプトは](../Page/移植_\(ソフトウェア\).md "wikilink")、最低でもBashをBourne Shellで置き換えた場合を考慮しなければならない。伝統的なBourne ShellにはないがBashには搭載されている機能は以下である\[37\]:

...
**選択ソート**（）は、[ソート](../Page/ソート.md "wikilink")の[アルゴリズム](../Page/アルゴリズム.md "wikilink")の一つ。配列された要素から、最大値やまたは最小値を探索し配列最後の要素と入れ替えをおこなうこと。最悪計算時間が[O](../Page/ランダウの記号.md "wikilink")(n<sup>2</sup>)と遅いが、アルゴリズムが単純で実装が容易なため、しばしば用いられる。内部ソート。後述するように、[安定ソート](../Page/安定ソート.md "wikilink")ではない。

...
**DFFITS** は[統計学](../Page/統計学.md "wikilink")の[回帰分析](../Page/回帰分析.md "wikilink")において、ある点の影響度を示す[統計量](../Page/統計量.md "wikilink")である。1980年に出版されたベルスレー、クー、ウェルシュ共著の『回帰診断：影響の強いデータと共線形性の源泉を同定する』\[1\]で提案された。

DFFITS は 問題の点を回帰から外した場合の予測（回帰）値の変化 "DFFIT" を問題の点での当てはめの標準偏差の推定値で割って（[スチューデント化](../Page/ウィリアム・ゴセット.md "wikilink")、'S'）したものである。
...
完全に均衡のとれた実験計画、たとえば（や均衡部分因子計画）の場合、各点でのテコ値は \(p/n\) 、すなわち[母数](../Page/母数.md "wikilink")の個数を点の個数で割ったものである。これは DFFITS 値が（正規分布の場合）\(\sqrt{p \over n-p} \approx \sqrt{p \over n}\) と t 変数の積である。したがって、同書の著者は DFFITS が \(2\sqrt{p \over n}\) より大きい場合を外れ点としてチェックすることを薦めている。

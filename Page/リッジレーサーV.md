...
[キャッチコピー](../Page/キャッチコピー.md "wikilink")は、「リッジレーサーの現代（いま）がここにある」。コンシューマーゲーム\]\]の[『リッジレーサー』シリーズの第](../Page/リッジレーサーシリーズ.md "wikilink")5弾。
...
イメージガール（イメージキャラクター）は今作のみ[永瀬麗子](../Page/永瀬麗子.md "wikilink")ではなく[深水藍](https://ja.wikipedia.org/wiki/深水藍 "wikilink")が登場し、オープニングムービーなどで見ることができる。PLAYSTATION FESTIVAL 2000の体験版のパッケージにもキャッチコピーは記載されているが[深水藍](https://ja.wikipedia.org/wiki/深水藍 "wikilink")のいないデザインとなっている。
...
今作のリッジレーサーでは、マシン・エンジンの性能別にクラス分けがある。
  ...
  - エクストラエンジン
    エクストラクラスのマシンに搭載されるエンジン。各性能が向上しているが、重点的に向上される性能は車種によって異なる\[5\]。
  - オーバルエンジン
...
各セクターに制限時間を設け、時間切れで失格となるグランプリ。規定順位も全戦1位となっている。
  ...
  -
    ...
    初回ラウンド（規定順位）
      ...
      - 第2戦：SUNNY BEACH（1位）
      - 第3戦：PARK TOWN（1位）
      - 第4戦：GREEN FIELD\[7\]（1位）

    周回数：全レース順走3周

  - リバース

逆走コース\[8\]を使用して行われるグランプリ。逆走以外の特殊規定はない。
...
4.
5.  初級者向けマシンでは旋回性能重視、上級者向けマシンでは高速性能重視のものがそれぞれ搭載される。
6.  初回優勝で他モードでの使用が解禁される。
7.  初回優勝で他モードでの使用が解禁される。
8.  初回優勝で、これまでに出現させたコースの逆走バージョンが解禁される。
9.  初回優勝で他モードでの使用が解禁される。逆走コースも同時解禁。
10. これに合わせ、オープニングムービーに登場するソラーレとメルクーリオはエクストラクラス仕様になっている。
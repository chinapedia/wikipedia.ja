...
は名前呼び（、[評価戦略](../Page/評価戦略.md "wikilink")を参照）をサポートしているため （[:en:Jensen's Device](https://ja.wikipedia.org/wiki/:en:Jensen's_Device "wikilink")）を容易に実装できる。デフォルトはと異なり値呼び（）であるため、を実装する際には、名前呼びであることを明示する必要がある。

...
**メッセージキュー**（）は、[プロセス間通信](../Page/プロセス間通信.md "wikilink")や同一プロセス内の[スレッド間通信に使われる](../Page/スレッド_\(コンピュータ\).md "wikilink")[ソフトウェアコンポーネント](../Page/ソフトウェアコンポーネント.md "wikilink")である。制御やデータを伝達する[メッセージの](https://ja.wikipedia.org/wiki/メッセージ_\(コンピュータ\) "wikilink")[キューである](../Page/キュー_\(コンピュータ\).md "wikilink")。
...
多くの[通信プロトコル](../Page/通信プロトコル.md "wikilink")は、[同期型である](https://ja.wikipedia.org/wiki/同期_\(計算機科学\) "wikilink")。[World Wide Webや](../Page/World_Wide_Web.md "wikilink")[Webサービス](https://ja.wikipedia.org/wiki/Webサービス "wikilink")で使われている [HTTP](../Page/Hypertext_Transfer_Protocol.md "wikilink") などは明らかに同期型である。同期モデルでは、あるシステムが別のシステムとのコネクションを形成し、要求を送って、応答を待つ。多くの場合、これで全く問題ない。例えば、ユーザーが Web ページに要求を送り、応答を待つというような場合である。
...
アプリケーションは[同期または非同期のどちらか一方だけで実装する必要はない](https://ja.wikipedia.org/wiki/同期_\(計算機科学\) "wikilink")。対話型アプリケーションは特定の要求に対して即座に応答する必要があるだろう（顧客に対して、在庫を確認したうえで購入要求が受理されたことを知らせる場合など）。しかし一方で、キューを使って処理を遅延させることが可能な部分もある（請求金額の計算を完了させ、そのデータを中央のデータベースに登録し、関連する他のサービスを実行する）。このような場合に、非同期型のメッセージキューを使えば、システム全体の性能（特に顧客から見た応答性能）を向上させることができる。

...
さらに以下の二つの恒等式を公理として仮定することも多いが、実際には吸収律を二度使うことで導くことが可能である{{\#tag:ref|つまり、*a* ∨ *a* = *a* ∨(*a* ∧(*a* ∨ *a*)) = *a* であり、もう一本の式はその双対として得られる\[1\]。|group="\*"}}
  ...
  - [冪等](https://ja.wikipedia.org/wiki/冪等 "wikilink")律
...
で定めれば順序集合論的な意味の束が得られる。吸収律はいずれの定義に関しても同値である。そうして、この方法で定めた順序関係 ≤ が導く交わりと結びがもともとの代数的な意味での束の演算 ∨, ∧ に一致することも確かめられる。
...
二つの束の間の[射](https://ja.wikipedia.org/wiki/射_\(圏論\) "wikilink")（あるいは[準同型](../Page/準同型.md "wikilink")）としてどのようなものを考えるべきかは、代数的構造としての定義を使えば容易にわかる。二つの束 (*L*, ∨<sub>*L*</sub>, ∧<sub>*L*</sub>) および (*M*, ∨<sub>*M*</sub>, ∧<sub>*M*</sub>) が与えられたとき、**束の射**あるいは**束準同型**とは、写像 *f*: *L* → *M* で
...
任意の束準同型は付随する順序関係に関して[単調である必要があるが](https://ja.wikipedia.org/wiki/単調写像 "wikilink")、逆は真ではない。つまり、単調性は結びや交わりを保存することを保証しない。一方、[順序を保つ](https://ja.wikipedia.org/wiki/単調写像 "wikilink")[全単射](../Page/全単射.md "wikilink")が束準同型となるのは、その[逆写像](https://ja.wikipedia.org/wiki/逆写像 "wikilink")がやはり向きを保つときである。
...
**条件付き完備束** (conditionally complete lattice) とは任意の**空でない上に有界な**部分集合が結び（最小上界）を持つことをいう。このような束は[実数](../Page/実数.md "wikilink")全体の集合に対する[完備性公理](https://ja.wikipedia.org/wiki/完備性公理 "wikilink")を最も直接に一般化するものである。条件付き完備束は、完備束か、完備束から最大元 1 を除いたものか、完備束から最小元 0 を除いたものか、あるいは完備束から最大元と最小元の両方を取り除いたものかのいずれかである。
...
[領域理論](https://ja.wikipedia.org/wiki/領域理論 "wikilink")において、半順序集合の元を「より単純な」元によって近似することを考えるのは自然である。それによって、任意の元がその元の[ずっと下にある元の成す](https://ja.wikipedia.org/wiki/way-below関係 "wikilink")[有向集合](https://ja.wikipedia.org/wiki/有向集合 "wikilink")の上限として得られるような半順序集合からなる[連続的半順序集合](https://ja.wikipedia.org/wiki/連続的半順序集合 "wikilink")のクラスが導かれる。ここでさらに有向集合を得るのに使える元を[コンパクト元](https://ja.wikipedia.org/wiki/コンパクト元 "wikilink")に制限することを考えるならば、[代数的半順序集合](https://ja.wikipedia.org/wiki/代数的半順序集合 "wikilink")が得られる。これらの概念を束に対しても考えれば、
...
が成り立つことをいう。特に補元が一意に定まる場合、これを ¬*x* = *y* および ¬*y* = *x* で表す。任意の元が補元を持つ有界束は[可補束](https://ja.wikipedia.org/wiki/可補束 "wikilink") (complemented lattice) と呼ばれ、補元が一意に定まる場合、 *L* 上の[単項演算](https://ja.wikipedia.org/wiki/単項演算 "wikilink") ¬ は補演算あるいは補化 (complementation) と呼ばれる。これは[論理否定](https://ja.wikipedia.org/wiki/論理否定 "wikilink")の束論における類似物として導入された。一般に補元は一意である必要も、*L* 上で可能な全ての単項演算のなかで特別なものであるわけでもない。可補束がさらに分配的でもあるならば、それは[ブール代数](../Page/ブール代数.md "wikilink")である。分配束に対しては、補元は存在すれば一意である。
...
任意の集合 *X* に対して、それが生成する**自由半束** *FX* を考えることができる。すなわち、*FX* は *X* の有限部分集合全体に通常の[集合の和を考えて得られる半束として定義される](https://ja.wikipedia.org/wiki/合併_\(集合論\) "wikilink")。自由半束は[普遍性](https://ja.wikipedia.org/wiki/普遍性 "wikilink")を持つ。
...
任意の半順序集合に対して、互いに双対なおよび[フィルターの概念をある種の部分集合族として考えることができるが](https://ja.wikipedia.org/wiki/フィルター_\(数学\) "wikilink")、もちろん半順序集合である束の理論においてもそれはやはり重要な概念であるが、詳細はそれぞれの項に譲る。

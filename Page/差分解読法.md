...
**差分解読法**（さぶんかいどくほう、）は、主に[ブロック暗号](https://ja.wikipedia.org/wiki/ブロック暗号 "wikilink")に対して用いられる[暗号解読](https://ja.wikipedia.org/wiki/暗号解読 "wikilink")法の一種である。[ストリーム暗号](https://ja.wikipedia.org/wiki/ストリーム暗号 "wikilink")や[ハッシュ関数](../Page/ハッシュ関数.md "wikilink")の攻撃にも用いられる。広い意味では、入力差分がどのように出力差分に影響を及ぼすか考察することである。ブロック暗号においては、置換ネットワークを通してどのように差分が波及するかを追跡することによって、暗号アルゴリズムの非ランダム性を発見し、秘密鍵を復元するような性質を発見することを意味する。
...
DESは差分解読法に対して耐性を持つように設計されていたが、他の暗号は脆弱であった。最も初期の攻撃対象は、ブロック暗号[FEAL](https://ja.wikipedia.org/wiki/FEAL "wikilink")であった。最初に提案された4段のFEAL (FEAL-4) はたった8個の[選択平文で解読可能であった](https://ja.wikipedia.org/wiki/選択平文攻撃 "wikilink")。31段でさえも攻撃は可能である。
...
差分解読法は、攻撃者が自由に選択した平文に対する暗号文が入手できるという条件で可能な攻撃法であり、選択平文攻撃に分類できる。 DESを解読するためには、2<sup>47</sup>の選択平文が必要である。 しかしながら、[既知平文攻撃](https://ja.wikipedia.org/wiki/既知平文攻撃 "wikilink")や[暗号文単独攻撃](https://ja.wikipedia.org/wiki/暗号文単独攻撃 "wikilink")を適用できるような拡張も存在する。 基本的な手法においては、ある固定された差を持つ二つの平文の組を用いる。差の定義にはいくらかの変形があるが、たいていは[排他的論理和](../Page/排他的論理和.md "wikilink")が用いられる。 攻撃者は、暗号文の組の差を計算し、その分布の中に統計的パターンを発見しようとする。これらの差を差分と呼ぶ。 この統計的性質は、暗号化に用いられる[Sボックス](../Page/Sボックス.md "wikilink")の性質に由来している。 それゆえ攻撃者はこれらのSボックス \(S\) に対して、\((\Delta_X, \Delta_Y)\)（ただし \(\Delta_Y = S(X) \oplus S(X \oplus \Delta_X)\)）となるような差分を解析する。 基本的な攻撃では、ひとつの特徴的な暗号文差分が高い確率で生起することが期待される。このようにして、暗号アルゴリズムは、ランダム関数と区別される。さらに洗練された手法では、全数探索法より速く鍵を復元できる。 ほとんどの暗号では、攻撃が成功するためには、差分を注意深く選ぶ必要がある。解析はアルゴリズムの内部構造を調査する。標準的な手法では、それぞれの段の高確率の差分のパスを追跡することによって行われる。このようにして得られたものを差分特性と呼ぶ。
...
## 関連項目
  ...
  - [線形解読法](https://ja.wikipedia.org/wiki/線形解読法 "wikilink")
  - [差分確率](https://ja.wikipedia.org/wiki/差分確率 "wikilink")
  - 関連した攻撃：[切詰差分解読法](https://ja.wikipedia.org/wiki/切詰差分解読法 "wikilink") / [高階差分解読法](https://ja.wikipedia.org/wiki/高階差分解読法 "wikilink") / [丸め差分攻撃](https://ja.wikipedia.org/wiki/丸め差分攻撃 "wikilink") / [不能差分攻撃](https://ja.wikipedia.org/wiki/不能差分攻撃 "wikilink") / [ブーメラン攻撃](https://ja.wikipedia.org/wiki/ブーメラン攻撃 "wikilink")

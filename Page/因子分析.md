...
## ソフトウェア
  ...
  - [Rの基本パッケージ中の多変量解析関数一覧](http://www.okada.jp.org/RWiki/index.php?R%A4%CE%B4%F0%CB%DC%A5%D1%A5%C3%A5%B1%A1%BC%A5%B8%C3%E6%A4%CE%C2%BF%CA%D1%CE%CC%B2%F2%C0%CF%B4%D8%BF%F4%B0%EC%CD%F7)
      -
        統計解析ツール[R言語](https://ja.wikipedia.org/wiki/R言語 "wikilink")は因子分析など多変量解析を標準で行えるフリーウェア。[可視化](https://ja.wikipedia.org/wiki/可視化 "wikilink")機能に優れる。[マルチプラットフォーム](https://ja.wikipedia.org/wiki/マルチプラットフォーム "wikilink")。他統計ソフトやExcelのファイル取込やODBC接続も可能。[FDAの申請にも使用を認められ](https://ja.wikipedia.org/wiki/アメリカ食品医薬品局 "wikilink")、CRANという仕組みで世界の膨大なアプリケーションを無償で使える。

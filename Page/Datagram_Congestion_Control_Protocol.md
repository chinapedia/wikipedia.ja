...
DCCPは、アプリケーション層ではなくトランスポート層で輻輳制御機構を提供する。DCCPは、[TCPのようなフローベースの考え方を可能にするが](../Page/Transmission_Control_Protocol.md "wikilink")、信頼性や正しい順序を保証しない。また、[SCTPのような複数のストリームで順序指定の配送を提供しない](https://ja.wikipedia.org/wiki/Stream_Control_Transmission_Protocol "wikilink")。

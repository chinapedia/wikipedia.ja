...
完全なRDBMSと異なり、Accessの[データベースエンジン](https://ja.wikipedia.org/wiki/データベースエンジン "wikilink")JETには[データベーストリガ](https://ja.wikipedia.org/wiki/データベーストリガ "wikilink")および[ストアドプロシージャ](https://ja.wikipedia.org/wiki/ストアドプロシージャ "wikilink")が存在しない。Accessは基礎となるテーブルに対する変更を引き起こすようなコードをフォームに含めることを認めており、また、Accessに含まれるパススルー・クエリや他の技術を用いて、外部のRDBMSがサポートしているストアドプロシージャを実行することも一般的である。

...
**山登り法**（やまのぼりほう、）は、評価関数の[極値](../Page/極値.md "wikilink")を探索する[探索](https://ja.wikipedia.org/wiki/探索 "wikilink")[アルゴリズム](../Page/アルゴリズム.md "wikilink")。最も代表的な[局所探索法](../Page/局所探索法.md "wikilink")として知られている。[最良優先探索](../Page/最良優先探索.md "wikilink")は過去の解を管理するが、探索対象を現在の解だけに制限したものである。評価関数を使用する探索アルゴリズムとしては最も単純。
...
[擬似コード](https://ja.wikipedia.org/wiki/擬似コード "wikilink")を以下に示す。この擬似コードでは EVAL(node) の極大値を探索している。

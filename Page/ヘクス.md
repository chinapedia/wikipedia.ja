...
また、升目の距離判定が複雑になる。スクエアなら、単にX座標の差とY座標の差を足せば距離になる（[マンハッタン距離](https://ja.wikipedia.org/wiki/マンハッタン距離 "wikilink")）が、ヘクスでは[条件分岐が必要になる](https://ja.wikipedia.org/wiki/条件文 "wikilink")。

## 関連項目

  - [ZOC](https://ja.wikipedia.org/wiki/ZOC "wikilink")

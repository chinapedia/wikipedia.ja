...
[ファイルシステム](../Page/ファイルシステム.md "wikilink")においては、プロセスの[利用者識別子](../Page/ユーザー識別子.md "wikilink")（[POSIX](../Page/POSIX.md "wikilink")では実効UID）が制御の主な手段となる。 アクセス制御リスト (ACL) はデータ構造（通常は配列）をもち、特定のシステムオブジェクト（プログラム、プロセス、ファイル）への個々のユーザーやグループの権利を示す要素から構成される。これらの要素は、[Microsoft Windows](https://ja.wikipedia.org/wiki/Microsoft_Windows "wikilink")\[1\]、[OpenVMS](https://ja.wikipedia.org/wiki/OpenVMS "wikilink")、[Unix系](../Page/Unix系.md "wikilink")、[macOS](https://ja.wikipedia.org/wiki/macOS "wikilink") といった[オペレーティングシステム](../Page/オペレーティングシステム.md "wikilink") (OS) ではアクセス制御エントリ (ACE) と呼ばれている。アクセス可能なオブジェクトはACLへの識別子をもっている。特権またはパーミッションは、あるユーザーがそのオブジェクトについて何ができるか（読み、書き、実行）を決定する。実装によっては、ACEは所有者かどうか、または所有者のグループかどうかにかかわらずオブジェクトのACLを制御できる。

アクセス制御リストは概念であり、その実装はOSによって異なるが、POSIXの「標準」がある（POSIX security drafts の .1e と .2c は対象範囲を広げすぎたため標準策定が完了しないとして破棄された。しかし、ACLについてよく書かれた部分は広く実装され「POSIX ACL」として知られるようになった）。多くの[UNIX](../Page/UNIX.md "wikilink")およびUnix系OS（例えば、[Linux](../Page/Linux.md "wikilink")\[2\]、[BSD](../Page/BSD.md "wikilink")、[Solaris](../Page/Solaris.md "wikilink")）はドラフト版の POSIX.1e ACL をサポートしている。また、[AIX](https://ja.wikipedia.org/wiki/AIX "wikilink")、[FreeBSD](../Page/FreeBSD.md "wikilink")\[3\]、[Mac OS X v10.4](../Page/Mac_OS_X_v10.4.md "wikilink") ("Tiger") 以降、[Solaris](../Page/Solaris.md "wikilink")の[ZFS](../Page/ZFS.md "wikilink")\[4\] では [NFSv4](https://ja.wikipedia.org/wiki/Network_File_System "wikilink") ACL をサポートしている。Linuxでも[ext3](https://ja.wikipedia.org/wiki/ext3 "wikilink")ファイルシステム向け\[5\]と[Richaclsという](https://ja.wikipedia.org/wiki/:en:Richacls "wikilink")[ext4](https://ja.wikipedia.org/wiki/ext4 "wikilink")向け\[6\]の NFSv4 ACL の実験的実装が2つある。
...
[ネットワークでは](https://ja.wikipedia.org/wiki/コンピュータネットワーク "wikilink")、**Access Control List** (**ACL**) は[TCP/IP](https://ja.wikipedia.org/wiki/TCP/IP "wikilink")のポート番号とその[ホスト上で使える](../Page/サーバ.md "wikilink")[デーモンのリストを意味する](../Page/デーモン_\(ソフトウェア\).md "wikilink")。つまり、ホストがネットワークに対して提供しているサービスのリストである。個々の[サーバ](../Page/サーバ.md "wikilink")だけでなく[ルーター](../Page/ルーター.md "wikilink")もネットワークACLをもっている。この場合のアクセス制御リストは双方向のトラフィックを制御するもので、例えば[ファイアウォール](../Page/ファイアウォール.md "wikilink")と深い関係がある。[PCI DSS](../Page/PCIデータセキュリティスタンダード.md "wikilink") のようなセキュリティの規約や標準の対象ともなっている。

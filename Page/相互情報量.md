...
**相互情報量**（そうごじょうほうりょう、）または**伝達情報量**（でんたつじょうほうりょう、）は、[確率論](https://ja.wikipedia.org/wiki/確率論 "wikilink")および[情報理論](../Page/情報理論.md "wikilink")において、2つの[確率変数](https://ja.wikipedia.org/wiki/確率変数 "wikilink")の相互依存の尺度を表す[量](https://ja.wikipedia.org/wiki/量 "wikilink")である。最も典型的な相互情報量の[物理単位](../Page/物理単位.md "wikilink")は[ビット](../Page/ビット.md "wikilink")であり、2 を底とする対数が使われることが多い。
...
ここで、\(p(x,y)\) は \(X\) と \(Y\) の[同時分布](https://ja.wikipedia.org/wiki/同時分布 "wikilink")関数、\(p(x)\) と \(p(y)\) はそれぞれ \(X\) と \(Y\) の[周辺確率](https://ja.wikipedia.org/wiki/周辺確率 "wikilink")分布関数である。
...
直観的には、相互情報量は *X* と *Y* が共有する情報量の尺度である。一方の変数を知ることでもう一方をどれだけ推測できるようになるかを示す。例えば、*X* と *Y* が独立であれば、*X* をいくら知っても *Y* に関する情報は得られないし、逆も同様である。従って、相互情報量はゼロである。逆に、*X* と *Y* が同じであれば、*X* と *Y* は全情報を共有していると言う事ができ、*X* を知れば *Y* も知ることになり、逆も同様である。結果として、相互情報量は *Y*（または *X*）単独の不確かさ、すなわち *Y*（または *X*）の[エントロピーと同じとなる](https://ja.wikipedia.org/wiki/情報量 "wikilink")。

相互情報量は、*X* と *Y* が独立な場合の同時分布と実際の[同時分布](https://ja.wikipedia.org/wiki/同時分布 "wikilink")の距離を示す量である。また、以下のような意味で相互の依存性の尺度でもある。すなわち、*I*(*X*; *Y*) = 0 であるのは、*X* と *Y* が全く依存しない、独立な確率変数である場合だけである（[同値](https://ja.wikipedia.org/wiki/同値 "wikilink")）。これは一方向から考えると分かり易い。*X* と *Y* が独立なら、*p*(*x*,*y*) = *p*(*x*) × *p*(*y*) であるから、次が成り立つ。
...
ここで *H*(*X*) と *H*(*Y*) は周辺[エントロピー](https://ja.wikipedia.org/wiki/情報量 "wikilink")、*H*(*X*|*Y*) と *H*(*Y*|*X*) は[条件付きエントロピー](https://ja.wikipedia.org/wiki/情報量 "wikilink")、*H*(*X*,*Y*) は *X* と *Y* の[結合エントロピー](https://ja.wikipedia.org/wiki/結合エントロピー "wikilink")である。*H*(*X*) ≥ *H*(*X*|*Y*) であるため、これは上述の非負性とも一貫している。
...
相互情報量は、2つの確率変数 *X* と *Y* の[周辺分布](https://ja.wikipedia.org/wiki/周辺分布 "wikilink")の積 *p*(*x*) × *p*(*y*) と[同時分布](https://ja.wikipedia.org/wiki/同時分布 "wikilink") *p*(*x*,*y*) の[カルバック・ライブラー情報量](https://ja.wikipedia.org/wiki/カルバック・ライブラー情報量 "wikilink")で表すこともできる。
...
多くの場合、相互情報量を最大化させ（つまり相互依存性を強め）、[条件付きエントロピーを最小化させるという方向で使われる](https://ja.wikipedia.org/wiki/情報量 "wikilink")。以下のような例がある。
  ...
  - [機械学習](https://ja.wikipedia.org/wiki/機械学習 "wikilink")における[特徴選択](https://ja.wikipedia.org/wiki/特徴選択 "wikilink")や特徴変換の尺度として相互情報量が使われてきた。
  ...
  - [時系列](https://ja.wikipedia.org/wiki/時系列 "wikilink")解析におけるの検出。

  - [情報量最大化](https://ja.wikipedia.org/wiki/情報量最大化 "wikilink")[独立成分分析](https://ja.wikipedia.org/wiki/独立成分分析 "wikilink")アルゴリズムでも利用されている。

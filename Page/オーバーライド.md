...
例えば、ある[クラス](../Page/クラス_\(コンピュータ\).md "wikilink")`Base`にメソッド`print`があり、あるクラス`Derived`がクラス`Base`を[継承したとする](https://ja.wikipedia.org/wiki/継承_\(プログラミング\) "wikilink")。そのとき、クラス`Derived`はクラス`Base`にあるメソッド`print`をオーバーライドすることにより、再定義することができる。これは[オブジェクト指向プログラミング](../Page/オブジェクト指向プログラミング.md "wikilink")における[ポリモーフィズム](../Page/ポリモーフィズム.md "wikilink")（多態性）を実現する際によく使われる。

通例、オーバーライドを可能とする条件として、メソッドの名前、引数の数と型の順序、そして戻り値の型が統一されている必要がある\[1\]。

メソッドの[オーバーロード](https://ja.wikipedia.org/wiki/多重定義 "wikilink")（[多重定義](https://ja.wikipedia.org/wiki/多重定義 "wikilink")、**）と混同されることがあるが、まったく異なる概念である。

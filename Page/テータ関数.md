...
\end{align}\] 楕円テータ関数にも定義に 2 つの流儀があり、注意が必要である。この記事での定義は、フルヴィッツ・クーランの 「楕円関数論」で用いられたのと同じ定義である。添え字が  から  ではなく、  から  までの定義もある。 その場合は \(\vartheta_{1}(v , \tau)\), \(\vartheta_{2}(v, \tau)\), \(\vartheta_{3}(v, \tau)\) の定義は変わらず、 \(\vartheta_{4}(v, \tau) := \vartheta_{0}(v, \tau)\) で定義される。 文脈から  あるいは  が明らかな場合は \(\vartheta_i(v)\) あるいは \(\vartheta_i(\tau)\) と書き、更に\(\vartheta_i=\vartheta_i(0,\tau)\) と書く。[Mathematica](https://ja.wikipedia.org/wiki/Mathematica "wikilink") では、\(\pi v\) のことを  と書いている。
...
[ヤコビの三重積](https://ja.wikipedia.org/wiki/ヤコビの三重積 "wikilink")の公式により、
...
0}} のときのテータ関数の値を**テータ定数**（）あるいは**テータ零値**（）という。これは[定数](../Page/定数.md "wikilink")といいながら実は  の[関数である](https://ja.wikipedia.org/wiki/関数_\(数学\) "wikilink")。
...
\end{align}\] でなるが、[オイラーの分割恒等式](https://ja.wikipedia.org/wiki/オイラーの分割恒等式 "wikilink")により、
...
次の恒等式は[ヤコビの虚数変換式](https://ja.wikipedia.org/wiki/ヤコビの虚数変換式 "wikilink")という。
...
## 関連項目
  ...
  - [ヤコビ](https://ja.wikipedia.org/wiki/カール・グスタフ・ヤコブ・ヤコビ "wikilink")

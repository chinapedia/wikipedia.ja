...
**変分法** （へんぶんほう、）とは、[関数を取り](https://ja.wikipedia.org/wiki/関数_\(数学\) "wikilink")[値](https://ja.wikipedia.org/wiki/値 "wikilink")を返す対応である[汎関数](https://ja.wikipedia.org/wiki/汎関数 "wikilink")についての[微分](../Page/微分.md "wikilink")にあたる手法を言う。[オイラー](https://ja.wikipedia.org/wiki/オイラー "wikilink")及び[ラグランジュらによって導入された](../Page/ジョゼフ＝ルイ・ラグランジュ.md "wikilink")\[1\]。

[解析力学](https://ja.wikipedia.org/wiki/解析力学 "wikilink")における重要な方程式は[最小作用の原理](https://ja.wikipedia.org/wiki/最小作用の原理 "wikilink")を元に変分法を用いて導出される。

## 変分法を使った原理

  - [フェルマーの原理](https://ja.wikipedia.org/wiki/フェルマーの原理 "wikilink")
  - [最小作用の原理](https://ja.wikipedia.org/wiki/最小作用の原理 "wikilink")
  - [ディリクレの原理](https://ja.wikipedia.org/wiki/ディリクレの原理 "wikilink")
...
ある多体系において、規格化、直交性などの条件の下で任意に選んだ試行関数（変分関数とも言う。ここでは多体の波動関数）を  とする。試行関数はいろいろな選び方があるがここでは、 は、系を記述する厳密な[固有関数](https://ja.wikipedia.org/wiki/固有関数 "wikilink")（波動関数） の展開で記述できるとする。
...
は、質量*M* ≫ *m*と電荷 +2*e*の実質的に固定された[核の周りの](../Page/原子核.md "wikilink")、質量電荷*m*と電荷 −*e*を持つ2個の電子から構成される。[微細構造](https://ja.wikipedia.org/wiki/微細構造 "wikilink")を無視したハミルトニアンは、
...
2つの電子間の反発を表わす項 *V<sub>ee</sub>* = *e*<sup>2</sup>/(4π*ε*<sub>0</sub>|**r**<sub>1</sub> − **r**<sub>2</sub>|) が考慮されなければ、ハミルトニアンは核電荷 +2*e*を持つ2つの[水素様原子のハミルトニアンの和となる](https://ja.wikipedia.org/wiki/水素原子におけるシュレーディンガー方程式の解 "wikilink")。基底状態エネルギーはその結果8*E*<sub>1</sub> = −109 eVとなり（*E*<sub>1</sub>は[リュードベリ定数](https://ja.wikipedia.org/wiki/リュードベリ定数 "wikilink")）、その基底状態波動関数は水素様原子の基底状態に対する2つの波動関数の積となる\[2\]。

\[\psi(\mathbf{r}_1,\mathbf{r}_2) = \frac{Z^3}{\pi a_0^3} e^{-Z(r_1+r_2)/a_0}\] 上式において、*a*<sub>0</sub>は[ボーア半径](https://ja.wikipedia.org/wiki/ボーア半径 "wikilink")、Z = 2はヘリウムの核電荷である。*ψ*<sub>0</sub>によって記述されるこの状態の全ハミルトニアン*H*（項*V<sub>ee</sub>*を含む）の期待値はその基底状態エネルギーについての上界となる。\<*V<sub>ee</sub>*\> は −5*E*<sub>1</sub>/2 = 34 eV あるため、<H> は8*E*<sub>1</sub> − 5*E*<sub>1</sub>/2 = −75 eV。

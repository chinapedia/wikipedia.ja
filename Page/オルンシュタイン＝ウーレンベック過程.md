...
**オルンシュタイン＝ウーレンベック過程**（オルンシュタイン＝ウーレンベックかてい、）は、[レナード・オルンシュタイン](https://ja.wikipedia.org/wiki/レナード・オルンシュタイン "wikilink")と[ジョージ・ウーレンベック](https://ja.wikipedia.org/wiki/ジョージ・ウーレンベック "wikilink")の名にちなんだ[確率過程](https://ja.wikipedia.org/wiki/確率過程 "wikilink")である。**平均回帰過程**（へいきんかいきかてい）とも呼ばれる。
...
この方程式は[定数変化法](https://ja.wikipedia.org/wiki/定数変化法 "wikilink")を用いて解くことができる。関数\(f(r_t, t) = r_t e^{\theta t}\)に対して[伊藤の補題](https://ja.wikipedia.org/wiki/伊藤の補題 "wikilink")を適用し、以下の式を得る。  これを0からtまで積分することにより、次の式が得られる。

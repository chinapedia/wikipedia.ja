...
ヘッダで指定された[エンコーディング](https://ja.wikipedia.org/wiki/エンコーディング "wikilink")指定に基づいてメール本文を表示したり、[添付ファイル](https://ja.wikipedia.org/wiki/添付ファイル "wikilink")の処理を行う。返信されたメールの本文のうち、行頭に引用を表す記号（一般的に"**\>**"）が付加された部分の表示色を変更して表示する機能をもつものもある。元来、SMTPの制約により 7bit の文字しか使用できなかった ([RFC](../Page/Request_for_Comments.md "wikilink")821) 電子メールでは、[バイナリ](../Page/バイナリ.md "wikilink")データを添付する場合は[MIME規格に基づき符号化されるが](../Page/Multipurpose_Internet_Mail_Extensions.md "wikilink")、電子メールクライアントはこれを復号し、元のデータに復元する。

...
主な仕様は上記の2つ。1つはコンテンツを配信するための[フィード](https://ja.wikipedia.org/wiki/フィード "wikilink")のフォーマットを規定する｢Atom配信フォーマット｣ (Atom Syndication Format)、もう1つはウェブ上のコンテンツを編集するための｢Atom出版プロトコル｣ (Atom Publishing Protocol) で、通称*Atom API*または*AtomPP*とも呼ばれる事がある。

元々、[The Atom Project](http://www.intertwingly.net/wiki/pie/FrontPage/)として、有志が[ウィキ](../Page/ウィキ.md "wikilink")やメーリングリストで議論しながら草の根的に始まり、現在、活動の場所は[IETF (Internet Engineering Task Force)](../Page/Internet_Engineering_Task_Force.md "wikilink") に引き継がれてワーキンググループとして標準化活動が行なわれている。
...
### Atom APIサーバ実装
  ...
  - はてなブックマーク・はてなフォトライフ
      ...
      - [はてなフォトライフAtomAPI](http://d.hatena.ne.jp/keyword/%A4%CF%A4%C6%A4%CA%A5%D5%A5%A9%A5%C8%A5%E9%A5%A4%A5%D5AtomAPI)

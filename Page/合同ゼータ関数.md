...
と  の定義の間の関係は、多くの方法で説明することができる（例えば、以下の  の無限積の公式を参照）。実際は、（一般の代数多様体に対しても、）この方法は、 が有限体上の[楕円曲線](../Page/楕円曲線.md "wikilink")  の場合のように、 は  の[有理関数](https://ja.wikipedia.org/wiki/有理関数 "wikilink")となっている。

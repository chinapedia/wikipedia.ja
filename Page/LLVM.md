...
LLVMは言語から独立した[命令セット](../Page/命令セット.md "wikilink")と[型システム](../Page/型システム.md "wikilink")を持つ。命令の多くは[3番地コード](../Page/3番地コード.md "wikilink")形式に似ている。各命令はまた[静的単一代入](https://ja.wikipedia.org/wiki/静的単一代入 "wikilink")形でもあり、[変数](../Page/変数_\(プログラミング\).md "wikilink")（型付きレジスタ）は一回代入されるとその後は変更されない。このため、変数間の依存関係の解析が単純化される。

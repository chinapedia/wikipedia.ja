...
**Scheme**（スキーム）はコンピュータ・[プログラミング言語](../Page/プログラミング言語.md "wikilink") [Lisp](https://ja.wikipedia.org/wiki/Lisp "wikilink")の[方言のひとつで](https://ja.wikipedia.org/wiki/方言_\(プログラミング言語\) "wikilink")、[静的スコープ](https://ja.wikipedia.org/wiki/静的スコープ "wikilink")などが特徴である。仕様（2017年現在、改7版まで存在する）を指すこともあれば、実装を指すこともある。Schemeにより、Lisp方言に静的スコープが広められた。

## 概要

Schemeは、[MIT AIラボにて](https://ja.wikipedia.org/wiki/MIT人工知能研究所 "wikilink")、[ジェラルド・ジェイ・サスマン](https://ja.wikipedia.org/wiki/ジェラルド・ジェイ・サスマン "wikilink")と[ガイ・スティール・ジュニア](https://ja.wikipedia.org/wiki/ガイ・スティール・ジュニア "wikilink")によって1975年頃に基本的な設計がなされた。動機は、[カール・ヒューイット](https://ja.wikipedia.org/wiki/カール・ヒューイット "wikilink")の提案によるエレガントな並行計算モデル「[アクター](https://ja.wikipedia.org/wiki/アクターモデル "wikilink")」と、同じくその言語のPLASMA（Planner-73）を理解するためであった。

[静的スコープ](https://ja.wikipedia.org/wiki/静的スコープ "wikilink")（[ALGOL](https://ja.wikipedia.org/wiki/ALGOL "wikilink")由来とされる\[1\]）は、状態を持つデータであるアクタ（[クロージャ](https://ja.wikipedia.org/wiki/クロージャ "wikilink")\[2\]）の実現以外にも、`lambda` 構文を用いた**[λ計算](https://ja.wikipedia.org/wiki/λ計算 "wikilink")**\[3\]や**[末尾再帰](https://ja.wikipedia.org/wiki/末尾再帰 "wikilink")**\[4\]の最適化に不可欠な機構であった。

また、プログラムの制御理論から当時出てきた**[継続](https://ja.wikipedia.org/wiki/継続 "wikilink")**\[5\]及びアクタ理論におけるアクタへの**メッセージ渡し**\[6\]の概念から触発された**[継続渡し形式](https://ja.wikipedia.org/wiki/継続渡しスタイル "wikilink")**\[7\]\[8\]と呼ばれるプログラミング手法は以後の継続の研究に大きな影響を与えた。

## 歴史

[MIT人工知能研究所](https://ja.wikipedia.org/wiki/MIT人工知能研究所 "wikilink")においては以下のとおりLISPに始まるいくつかの言語が作られた。
...
この中でカール・ヒューイットが設計した規則ベースの言語 [Planner](https://ja.wikipedia.org/wiki/Planner "wikilink") はあまりに複雑な機構を持っていたため当初設計された全機能の実装は困難であり\[9\]、サスマン等はそれをサブセット言語の  として実現し、さらには、 Planner の流れを汲んだ独自言語として  を作成した。
...
マッカーシーが後に回顧で、初期のLisp（LISP 1 および LISP 1.5）に関して「In modern terminology, lexical scoping was wanted, and dynamic scoping was obtained.」と書いているように\[10\]、計算理論的にも[静的スコープ](https://ja.wikipedia.org/wiki/静的スコープ "wikilink")が本来は「正当」であり、[動的スコープ](https://ja.wikipedia.org/wiki/動的スコープ "wikilink")は、言ってしまえばある種の安易なインタプリタの実装手法が招く「バグ」である（有用なことも多いが）。
...
「FUNARG問題」（[:en:Funarg problem](https://ja.wikipedia.org/wiki/:en:Funarg_problem "wikilink")）としてLISPの初期から既に認識され議論されていたことでもあり、必ずしもSchemeから始まったとは言えないが、Scheme以後のLisp方言に静的スコープが広まったのはSchemeからの影響と言ってよく、殊に[Common Lispは特筆される](https://ja.wikipedia.org/wiki/Common_Lisp "wikilink")。
...
の言語仕様は[IEEE](https://ja.wikipedia.org/wiki/IEEE "wikilink")によって公式に定められ\[14\]、その仕様は「」と呼ばれている。2016年現在広く実装されているものは改訂第五版に当たるR5RS（1998年）である。
...
の仕様書はR5RSだと50ページにも満たないため、かなりの数の実装が存在する。
  ...
  - [](http://www.gnu.org/software/kawa/) - [{{langのひとつ](https://ja.wikipedia.org/wiki/GNUプロジェクト "wikilink")。 プログラムを  仮想機械用にコンパイル可能。

  - [](http://www.larcenists.org/) - [IA-32](https://ja.wikipedia.org/wiki/IA-32 "wikilink")、 の機械語を出力。IEEE/ANSI、R5RS、ERR5RS, R6RS準拠。
  ...
  - [](http://tinyscheme.sourceforge.net/) - 非常に小さい実装。[{{langなどでも走る](https://ja.wikipedia.org/wiki/ザウルス "wikilink")。正規表現やソケット通信もサポート。
...
はしばしば他のアプリケーションの拡張用言語として使われる。代表的なアプリケーションには以下のようなものがある。
  ...
  - [{{lang](https://ja.wikipedia.org/wiki/GNU_LilyPond "wikilink")

より専門的な応用としては、映画[ファイナルファンタジーのために](https://ja.wikipedia.org/wiki/ファイナルファンタジー_\(映画\) "wikilink")3Dレンダリングエンジンに  インタプリタを組み込んだ例\[18\]や、[リトルウイング](https://ja.wikipedia.org/wiki/リトルウイング "wikilink")のピンボールコンストラクションシステムの記述に  を使った例\[19\]がある。

用の  では、 コンパイラである  を使って[{{lang用のバイトコードを生成している](https://ja.wikipedia.org/wiki/Java仮想マシン "wikilink")。
...
## 関連項目
  ...
  - [末尾再帰](https://ja.wikipedia.org/wiki/末尾再帰 "wikilink")

  -
  - [計算機プログラムの構造と解釈](https://ja.wikipedia.org/wiki/計算機プログラムの構造と解釈 "wikilink") -  を用いた計算機科学分野の古典的な教科書。

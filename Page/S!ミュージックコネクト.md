...
## 特徴その他
  ...
  - CDからのリッピング（ノンセキュアWMA形式のみ）、ポータルサイトからの音楽購入の他、WMA（ノンセキュアWMA含む）音楽ファイルのインポートが可能。
  - MP3などはインポート（および携帯電話への転送）ができない。
  - 取得・送信されるCD情報はS\!ミュージックコネクト独自で管理している。
  - コンピレーションアルバム対策のため、曲ごとのアーティスト名とは別にアルバムアーティスト名のパラメータをもうけている。
  - CDからリッピングした場合、フォルダは、softbank￥（アルバムアーティスト名）￥（アルバム名）￥となり、ファイル名は曲名の頭にトラック番号が付く。
  - 921Pの場合、600曲まで保存可能。また、音楽ファイルは￥PRIVATE￥MYFOLDER￥My Items￥WMAudio￥（アルバムアーティスト名）￥（アルバム名）￥（ファイル名）、システムフォルダは￥PRIVATE￥MYFOLDER￥My Items￥WMSystem となる。
  - プロパティの編集・情報送信は1曲ずつしかできない。
  - ノンセキュアWMAを携帯電話に転送した場合はノンセキュアWMAのままになる（921Pの場合）。
  - S\!ミュージックコネクトソフトウェアで、携帯電話側のプレイリストの作成・編集・削除はできない。
...
#### [サムスン製](../Page/サムスン電子.md "wikilink")

  - [SoftBank 930SC](https://ja.wikipedia.org/wiki/SoftBank_930SC "wikilink")
...
## 外部リンク
  ...
  - [S\!ミュージックコネクト｜SoftBank](http://mb.softbank.jp/mb/service/3G/contents/music_connect/)

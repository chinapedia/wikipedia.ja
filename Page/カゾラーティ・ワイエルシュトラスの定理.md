...
**[カゾラーティ](https://ja.wikipedia.org/wiki/フェリーチェ・カゾラーティ_\(数学者\) "wikilink")・[ワイエルシュトラスの定理](../Page/カール・ワイエルシュトラス.md "wikilink")**（）は、[解析関数の孤立した真性](https://ja.wikipedia.org/wiki/複素解析 "wikilink")[特異点](../Page/特異点.md "wikilink")の近傍の像が稠密であることを主張する定理である。具体的には、\(\mathbb{U}_\delta:=\{z\in\mathbb{C}:\;0<|z-z_0|<\delta\}\) において \(f(z)\) が[正則であって](https://ja.wikipedia.org/wiki/正則関数 "wikilink") \((z-z_0)^{n}f(z)\) が有界となる自然数 \(n\) が存在しないとき（すなわち \(z_0\) が \(f\) の真性[特異点](../Page/特異点.md "wikilink")であるとき）に
...
\[z=\frac{1}{\log v+2{\pi}in},\quad{n\ge\frac{1}{2\pi\delta}+1}\] とすれば、\(\left|z\right|<\delta\) で \(f(z)=v\) となることが確かめられる。カゾラーティの定理は、真性特異点を持つ他の関数も同様に振る舞うことを主張する。但し、カゾラーティの定理は全ての値について「それに限りなく近い値」を取るとしか主張していない。[ピカールの定理](https://ja.wikipedia.org/wiki/ピカールの定理 "wikilink")は、「それに限りなく近い値」しか取らないという値が高々唯一の例外であることを主張する。
...
\[f(z)=\frac{1}{G(z)}+v\qquad\left(z\in\mathbb{U}_\delta\right)\] となるが、これは \(f\) が \(\mathbb{U}_\delta\) で[有理型となることを意味する](https://ja.wikipedia.org/wiki/有理型関数 "wikilink")。すなわち、 \(\forall{z\in\mathbb{U}_\delta}\) で\((z-z_0)^{n}f(z)\) が有界となる自然数 *n* が存在することになり、定理の仮定に反する。

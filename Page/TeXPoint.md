> この記事は[TeXPoint](https://ja.wikipedia.org/wiki/TeXPoint)から翻訳されています。


**TexPoint** は Microsoft [PowerPoint](https://ja.wikipedia.org/wiki/Microsoft_PowerPoint "wikilink") と [Word](../Page/Microsoft_Word.md "wikilink") の[アドイン](https://ja.wikipedia.org/wiki/アドイン "wikilink")である。これを使うと、[](../Page/LaTeX.md "wikilink") の記号や数式を PowerPoint ファイルやWord文書に簡単に挿入することができるようになる。

バージョン2.0.3までは[フリーウェア](../Page/フリーウェア.md "wikilink")であったが、それ以降のバージョンは[シェアウェア](../Page/シェアウェア.md "wikilink")として配布されている。2.0.3は現在は配布されていない。

このアドインに替わる無償サービスとして、[TeXclip](https://ja.wikipedia.org/wiki/TeXclip "wikilink") という[Webアプリケーション](https://ja.wikipedia.org/wiki/Webアプリケーション "wikilink")が公開されている。また、[OpenOffice.org](../Page/OpenOffice.org.md "wikilink") 用には類似の [OOoLatex](https://ja.wikipedia.org/wiki/OOoLatex "wikilink") という拡張も開発されている。

## 外部リンク

  - [TeXPoint本家](http://texpoint.necula.org/)
  - [TeXclip](http://maru.bonyari.jp/texclip/)
  - [OOoLatex](http://extensions.services.openoffice.org/project/ooolatex)



[Category:ソフトウェア](https://ja.wikipedia.org/wiki/Category:ソフトウェア "wikilink")
...
**ファイアウォール**とは[防火壁のことだが](https://ja.wikipedia.org/wiki/防火区画#防火壁 "wikilink")、[コンピュータネットワーク](https://ja.wikipedia.org/wiki/コンピュータネットワーク "wikilink")関連では、ネットワークの結節点となる場所に設けて、[コンピュータセキュリティ](../Page/コンピュータセキュリティ.md "wikilink")上の理由、あるいはその他\[1\]の理由により「通過させてはいけない通信」を阻止する[システム](../Page/システム.md "wikilink")を指す。通過させてはいけない通信を火にたとえている。[ルーター](../Page/ルーター.md "wikilink")にその機能を実装したものは、そのルーターを指して言うこともあるし、そういった[アプライアンス商品などもある](https://ja.wikipedia.org/wiki/ネットワーク・アプライアンス "wikilink")。近年ではネットワークの終端にあたる個々のコンピュータでも自分自身の防御のために、外部と接続するネットワーク[プロトコルスタック](https://ja.wikipedia.org/wiki/プロトコルスタック "wikilink")中に、望まない通信を防ぐ（たとえばTCPの接続要求など）フィルタなどを持っているものも多く、それらを指して言うこともある（たとえば[Windowsには](https://ja.wikipedia.org/wiki/Microsoft_Windows "wikilink")「[Windows ファイアウォール](https://ja.wikipedia.org/wiki/Windows_ファイアウォール "wikilink")」、[macOS](https://ja.wikipedia.org/wiki/macOS "wikilink")には「アプリケーションファイアウォール」がある）。

外部からの攻撃に対する防御だけではなく、内側から外部への望まない通信を制御する目的も含め運用されていることもある。利便性の制限となってしまっている場合もあるが、[標的型攻撃](https://ja.wikipedia.org/wiki/標的型攻撃 "wikilink")などで既に内部に[トロイの木馬などが入り込んでしまっている場合などに](https://ja.wikipedia.org/wiki/トロイの木馬_\(ソフトウェア\) "wikilink")、その活動を防ぐ効果を期待しての運用である場合もある。

以下では、[OSI参照モデル](../Page/OSI参照モデル.md "wikilink")に従ったレイヤによって分類しつつ説明する。

## パケットフィルタ型

[OSI参照モデル](../Page/OSI参照モデル.md "wikilink")における[ネットワーク層](https://ja.wikipedia.org/wiki/ネットワーク層 "wikilink")(レイヤ3)や[トランスポート層](https://ja.wikipedia.org/wiki/トランスポート層 "wikilink")(レイヤ4)に相当するIPから[TCP](../Page/Transmission_Control_Protocol.md "wikilink")、[UDP層の条件](../Page/User_Datagram_Protocol.md "wikilink")（[ポリシー](https://ja.wikipedia.org/wiki/ポリシー "wikilink")）で、通信の許可/不許可を判断するもの。狭義でのファイアウォールとは、このタイプのものを指す。このタイプはさらに、スタティックなものとダイナミックなものとに分類できる。

基本的にはレイヤ3で通信制御を判断するが、フィルタの種類によってはレイヤ4のヘッダも参照する。すなわち、TCP/UDPの[セッション](https://ja.wikipedia.org/wiki/セッション "wikilink")単位で管理する訳ではない。ただし、ステートフルパケットインスペクション型ではTCP/UDPセッションの一部も記憶して判断動作する。
...
レイヤ3・IPパケットではなく、TCP/IPなどのレイヤ4・[トランスポート層のレベルで通信を代替し](../Page/OSI参照モデル.md "wikilink")、制御する。内部のネットワークから外部のネットワークへ接続する場合は、サーキットレベルゲートウェイに対してTCPのコネクションを張ったり、UDPのデータグラムを投げることになる。サーキットレベルゲートウェイは、自らに向けられていたIPアドレスとポート番号を本来のものへと振り替え、自らが外部と通信した結果を返すという動作をする。代表的なソフトウェア実装としては[SOCKS](https://ja.wikipedia.org/wiki/SOCKS "wikilink")がある。また、ハードウェア実装として[レイヤ4スイッチ](https://ja.wikipedia.org/wiki/レイヤ4スイッチ "wikilink")にもこの機能を持たせる事ができる。
...
パケットではなく、レイヤ7の[HTTP](../Page/Hypertext_Transfer_Protocol.md "wikilink") や [FTP](../Page/File_Transfer_Protocol.md "wikilink") といった、アプリケーションプロトコルのレベルで外部との通信を代替し、制御するもの。一般的には[プロキシ](https://ja.wikipedia.org/wiki/プロキシ "wikilink")サーバと呼ばれている。アプリケーションゲートウェイ型ファイアウォールの内部のネットワークでは、アプリケーションはアプリケーションゲートウェイ（プロキシサーバ）と通信を行うだけであり、外部との通信はすべてプロキシサーバが仲介する。アプリケーションプロキシが用意されていないサービスについては、サーキットプロキシで対応する事が可能である。

このため、アプリケーションゲートウェイで許されている[プロトコル](../Page/プロトコル.md "wikilink")で[トンネリング](https://ja.wikipedia.org/wiki/トンネリング "wikilink")を行うソフトウェア、例えば[SoftEther](https://ja.wikipedia.org/wiki/SoftEther "wikilink")や[httptunnel](https://ja.wikipedia.org/wiki/httptunnel "wikilink")といった、運用方法によっては[セキュリティホール](../Page/セキュリティホール.md "wikilink")になりうる実装の利用を、かえって促進してしまうという事例も近年目立っている。強すぎる[セキュリティポリシーが迂回路を招いてしまっているとも言える](https://ja.wikipedia.org/wiki/情報セキュリティポリシー "wikilink")。
...
なお、アプリケーションの通信の中身も検査するため、[電気通信事業者](https://ja.wikipedia.org/wiki/電気通信事業者 "wikilink")が自らが仲介する通信の内容に立ち入ってはならない（[通信の秘密](https://ja.wikipedia.org/wiki/通信の秘密 "wikilink")）と言う原理原則に反する[検閲](https://ja.wikipedia.org/wiki/検閲 "wikilink")だと批判する向きもある。通信事業に携わる技術者や学者の間ではこういった種類のファイアウォールを設置するという発想を強く批判する向きもある。

なお実際に、[ISPの](https://ja.wikipedia.org/wiki/インターネットサービスプロバイダ "wikilink")[ぷらら](https://ja.wikipedia.org/wiki/ぷらら "wikilink")が[ファイル共有ソフト](https://ja.wikipedia.org/wiki/ファイル共有ソフト "wikilink")ウェア[Winny](../Page/Winny.md "wikilink")の通信を全て遮断する事を計画・発表し、それに対して通信の秘密を侵害する可能性があるとして[総務省](../Page/総務省.md "wikilink")から[行政指導](https://ja.wikipedia.org/wiki/行政指導 "wikilink")を受け、Winny遮断は同ISPユーザの利用者の選択に任せるとした事例もあった。

## 具体的な実装例

上述のパケットフィルタリング型や、サーキットレベルゲートウェイ型ではそれぞれ、[レイヤ3スイッチ](https://ja.wikipedia.org/wiki/レイヤ3スイッチ "wikilink")（[ルーター](../Page/ルーター.md "wikilink")）や[レイヤ4スイッチ](https://ja.wikipedia.org/wiki/レイヤ4スイッチ "wikilink")等のハードウェア機器の一部機能として組み込まれている事も多い。この場合、ある程度簡易な条件でしかパケット検査をできないため、簡易ファイアウォール、広義のファイアウォールと呼ぶこともある。レイヤー7ファイアウォール(L7FW)は通信の内容まで検査するため、L7FWが本来の（狭義の）ファイアウォールであるとすることもある。
...
[Windowsでは](https://ja.wikipedia.org/wiki/Microsoft_Windows "wikilink")[ZoneAlarm](https://ja.wikipedia.org/wiki/ZoneAlarm "wikilink")、[ノートン インターネットセキュリティ](https://ja.wikipedia.org/wiki/ノートン_インターネットセキュリティ "wikilink")、[ウイルスバスター](https://ja.wikipedia.org/wiki/ウイルスバスター "wikilink")、[NetOp Desktop Firewall等の](https://ja.wikipedia.org/wiki/NetOp_Desktop_Firewall "wikilink")[フリーウェア](../Page/フリーウェア.md "wikilink")ないし商用[アプリケーションが普及しているが](../Page/アプリケーションソフトウェア.md "wikilink")、これらはファイアウォールというよりは、[IDSに近い動作をしている](https://ja.wikipedia.org/wiki/侵入検知システム "wikilink")。しかし、一般的にファイアウォールという語が防護のイメージを喚起しやすいためか、用語は混乱して使われている。一般的には、**[パーソナルファイアウォール](https://ja.wikipedia.org/wiki/パーソナルファイアウォール "wikilink")**と呼ばれる。
...
### ソフトウェア型
  ...
  - [PF](https://ja.wikipedia.org/wiki/PF_\(ファイアウォール\) "wikilink")
  ...
  - [Symantec](https://ja.wikipedia.org/wiki/シマンテック "wikilink") [Client Security](https://ja.wikipedia.org/wiki/Client_Security "wikilink")
...
### ハードウェア型
  ...
  - [Symantec](https://ja.wikipedia.org/wiki/シマンテック "wikilink") [Gateway Security](https://ja.wikipedia.org/wiki/Gateway_Security "wikilink")
  ...
  - VSR - [バリオセキュア](https://ja.wikipedia.org/wiki/バリオセキュア "wikilink")
...
## 関連項目
  ...
  - [DMZ](https://ja.wikipedia.org/wiki/非武装地帯_\(コンピュータセキュリティ\) "wikilink")
  ...
  - [コモンクライテリア](https://ja.wikipedia.org/wiki/コモンクライテリア "wikilink")
  - [統合脅威管理](https://ja.wikipedia.org/wiki/統合脅威管理 "wikilink")（UTM: Unified threat management）

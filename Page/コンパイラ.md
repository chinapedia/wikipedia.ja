...
**コンパイラ**（）とは、コンピュータ・[プログラミング言語](../Page/プログラミング言語.md "wikilink")の[処理系](https://ja.wikipedia.org/wiki/処理系 "wikilink")（言語処理系）の一種で、[高水準言語](../Page/高水準言語.md "wikilink")による[ソースコード](../Page/ソースコード.md "wikilink")から、[機械語](../Page/機械語.md "wikilink")あるいは元のプログラムよりも低い水準のコードに変換（コンパイル）するプログラムである。

## 概説

[プログラミング言語](../Page/プログラミング言語.md "wikilink")（人間が理解しやすい言語や数式でプログラムを記述可能な人工言語）で書かれたプログラムを、コンピュータのプロセッサ\[1\]が直接的に実行できる機械語（[バイナリコード](https://ja.wikipedia.org/wiki/バイナリコード "wikilink")）、あるいは元のプログラムよりも低いレベルのコード（例えば[バイトコード](../Page/バイトコード.md "wikilink")などの[仮想機械](../Page/仮想機械.md "wikilink")向け[中間言語](../Page/中間言語.md "wikilink")あるいは[中間表現](../Page/中間表現.md "wikilink")）に変換することを、英語の[動詞](../Page/動詞.md "wikilink")で""（**コンパイル**）と呼ぶ。コンパイラとは、コンパイルを行なう[コンピュータプログラム](https://ja.wikipedia.org/wiki/コンピュータプログラム "wikilink")のことである。

コンパイラによって変換される前のプログラムを「[ソースコード](../Page/ソースコード.md "wikilink")」と呼び、変換後の機械語あるいは中間言語のプログラムなどを「[オブジェクトコード](../Page/オブジェクトファイル.md "wikilink")」と呼ぶ。なお、開発環境や処理系によっては、変換することを""（[ビルド](https://ja.wikipedia.org/wiki/ビルド_\(ソフトウェア\) "wikilink")）と呼んでいるものもある。ただし、厳密には「コンパイル」は「ビルド」の一工程である。直接バイナリコード を出力するものもあるが、コンパイラ自体は[アセンブリ言語](../Page/アセンブリ言語.md "wikilink")によるコードを出力するにとどまり、その先、それをバイナリに変換する作業（アセンブル）は[アセンブラ](https://ja.wikipedia.org/wiki/アセンブラ "wikilink")に任せているものも多い。

「compile」はもともと「[編集](../Page/編集.md "wikilink")する」「[編纂](https://ja.wikipedia.org/wiki/編纂 "wikilink")する」という意味の英語であり\[2\]\[3\]、「compiler」というのは「編集者」という意味の英語である\[4\]\[5\]\[6\]。なお、日本語による解説書などには「」などという解説がよくある。

のコンパイラでは、ひとつのプログラムとして動作する全てのコードをいっぺんにコンパイルするのではなく、モジュール毎などに分けてコンパイルし（「分割コンパイル」）、[ライブラリ](../Page/ライブラリ.md "wikilink")などはあらかじめコンパイルされているものと合わせて、実行するようにすることが多い。この場合、コンパイラは[リロケータブルバイナリ](../Page/リロケータブルバイナリ.md "wikilink")を出力し、実行可能ファイルの生成には[リンケージエディタ](../Page/リンケージエディタ.md "wikilink")が必要であり、さらに[動的リンク](../Page/動的リンク.md "wikilink")で実行する場合はダイナミックリンカ／ローダ（[ローダ](../Page/ローダ.md "wikilink")の一種）も必要である。

の開発環境などでは、コンパイル（ビルド）して実行、というような手続きを1命令で行えるものも増えている。そして、インタプリタでも[実行時コンパイラ](../Page/実行時コンパイラ.md "wikilink")などの技術の利用がさかんになってきており、古典的な意味での「コンパイラ」と「インタプリタ」の中間的な性質のツール（プログラム）も増えてきているので、「コンパイラ言語 / インタプリタ言語」という、以前よく行われた対比は、あまり意味を持たない場合も増えてきている。
...
一回でコンパイルが可能なものを**ワンパスコンパイラ**と呼び、一般に**マルチパスコンパイラ**よりも高速で扱いやすい。多くの言語はワンパスでコンパイルできるよう意図して設計されている（たとえば、[Pascal](../Page/Pascal.md "wikilink")）。
...
字句規則から字句解析器を生成する[lex](https://ja.wikipedia.org/wiki/lex "wikilink")、構文規則から構文解析器を生成する[パーサジェネレータ](../Page/パーサジェネレータ.md "wikilink")というプログラムがあり、広く実用に使われている。コンパイラ全体を生成する[コンパイラジェネレータ](https://ja.wikipedia.org/wiki/コンパイラジェネレータ "wikilink")も研究されているものの、広く実用に使われるには至っていない。
...
2.  **[字句解析](../Page/字句解析.md "wikilink")** - ソースコードを「トークン」と呼ばれる断片に分割する。各トークンは言語の最小構成要素であり、キーワード、[識別子](../Page/識別子.md "wikilink")、シンボル名などである。トークンは一般に[正規言語](../Page/正規言語.md "wikilink")に従うため、[正規表現](../Page/正規表現.md "wikilink")を解釈する[有限オートマトン](../Page/有限オートマトン.md "wikilink")で認識できる。字句解析を行うソフトウェアを[字句解析器](https://ja.wikipedia.org/wiki/字句解析器 "wikilink")（lexical analyzer）と呼ぶ。
3.  **[プリプロセッサ](../Page/プリプロセッサ.md "wikilink")** - C言語などでは、[マクロによる文字列置き換えや条件付きコンパイルなどの処理をするフェーズが必要である](../Page/マクロ_\(コンピュータ用語\).md "wikilink")。一般にこのフェーズは構文解析や意味解析の前に行われる。C言語の場合、プリプロセッサはトークンを操作するものであって、構文を考慮しない。しかし、[Scheme](../Page/Scheme.md "wikilink")のような言語では構文解析後に[マクロによる置き換えが行われる](https://ja.wikipedia.org/wiki/マクロ_\(コンピュータ用語\)#Lisp_のマクロ "wikilink")。
4.  **[構文解析](../Page/構文解析.md "wikilink")** - トークン列を解析し、プログラムの構造を明らかにする。このフェーズで[構文木](https://ja.wikipedia.org/wiki/構文木 "wikilink")が構築され、単なるトークンの列だったプログラムにその言語の文法を定義した[形式文法](../Page/形式文法.md "wikilink")の規則を適用することで[木構造を生成する](../Page/木構造_\(データ構造\).md "wikilink")。構文木は、この後の工程で解析され、強化され、変換される。
5.  **** - [構文木](https://ja.wikipedia.org/wiki/構文木 "wikilink")に意味論的情報を追加し、シンボルテーブルを作成する。[型チェック](../Page/型システム.md "wikilink")（データ型などを間違っていないかのチェック）や、変数や関数の定義と参照箇所を結びつける処理、既定値代入（自動変数の初期化）、意味的に不正なプログラムを検出して通知するなどの処理が行われる。意味解析には完全な構文木が必要であり、理論上[構文解析](../Page/構文解析.md "wikilink")と[コード生成](https://ja.wikipedia.org/wiki/コード生成 "wikilink")の間に行わなければならない。もちろんコンパイラの実装によってはこれらを渾然一体に行うこともある。
...
2.  **[最適化](../Page/最適化_\(情報工学\).md "wikilink")** - 中間表現を機能的には等価だがより高速な（小さい）形式に変換する。主な最適化手法として[インライン展開](../Page/インライン展開.md "wikilink")、[デッドコード削除](../Page/デッドコード削除.md "wikilink")、[定数伝播](https://ja.wikipedia.org/wiki/定数伝播 "wikilink")、ループ変換、[レジスタ割り当て](https://ja.wikipedia.org/wiki/レジスタ割り当て "wikilink")、自動並列化などがある。
...
1.  例えば[CPU](../Page/CPU.md "wikilink")や[GPUなど](../Page/Graphics_Processing_Unit.md "wikilink")。

...
データ転送には以下の4つのデータ表現が利用できる\[11\]\[12\]\[13\]。
  ...
  - [ASCII](https://ja.wikipedia.org/wiki/ASCII "wikilink")モード: テキストデータに使用される。必要に応じて、送信側で送信ホストの文字表現から[拡張ASCII](../Page/拡張ASCII.md "wikilink")に変換され、受信側では受信ホストの文字表現に変換される。そのため、このモードはプレーンテキスト以外のデータを含むファイルには不適切である。

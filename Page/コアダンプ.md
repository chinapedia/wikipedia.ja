...
[オペレーティングシステム](../Page/オペレーティングシステム.md "wikilink") (OS) が生まれ、大きなデータファイルを扱えるようになる以前は、コアダンプはメモリの内容を紙にプリントアウトしたものであり、一般に[8進数か](https://ja.wikipedia.org/wiki/八進法 "wikilink")[16進数](../Page/十六進法.md "wikilink")（ヘックスダンプと呼ばれる）で記され、場合によっては、その横に何らかの形式に翻訳したものも表示された（機械語命令として翻訳したり、[ASCII](https://ja.wikipedia.org/wiki/ASCII "wikilink")文字列として翻訳したり、十進数や浮動小数点数として翻訳したりする）。その後、コアダンプは特定の[プロセス](../Page/プロセス.md "wikilink")の[アドレス空間](https://ja.wikipedia.org/wiki/アドレス空間 "wikilink")の一部または全部のメモリイメージを格納した[ファイルとなり](https://ja.wikipedia.org/wiki/ファイル_\(コンピュータ\) "wikilink")、同時に[レジスタの値などの情報も格納するようになった](../Page/レジスタ_\(コンピュータ\).md "wikilink")。このようなファイルは適当なツールを使えば、かつての紙のコアダンプと同じように、人間が読める形式で参照可能となる。
...
最近の[Unix系](../Page/Unix系.md "wikilink")OSでは、コアダンプファイルを[GNU Binutilsの](https://ja.wikipedia.org/wiki/GNU_Binutils "wikilink")[BFDライブラリ](https://ja.wikipedia.org/wiki/BFDライブラリ "wikilink")を使って読むことができ、[GNUデバッガ](../Page/GNUデバッガ.md "wikilink") (gdb) や[objdump](https://ja.wikipedia.org/wiki/objdump "wikilink")はこのライブラリを使っている。このライブラリはメモリアドレスを指定すると、そのアドレスに相当する位置のコアダンプ内の生のデータを返すだけで、変数やデータ構造に関する知識は持たない。従って、このライブラリを利用する側が変数のアドレスやデータ構造を把握する必要があり、対象プログラムの[シンボルテーブル](../Page/シンボルテーブル.md "wikilink")などを使ってデバッグすることになる。

コアダンプをある時点で（`gcore`コマンドなどで）セーブしておき、後でその状態に戻すのに使うこともできる。[高可用性システムによっては](https://ja.wikipedia.org/wiki/High_availability "wikilink")、このようなコアダンプファイルをプロセッサ間で転送することで高可用性を実現することもある。この手法は[GNU Emacsや](https://ja.wikipedia.org/wiki/GNU_Emacs "wikilink")[Perl](../Page/Perl.md "wikilink")といった、起動時のコストが非常に高いソフトウエアでも利用されている。また[プリコンパイルヘッダー](https://ja.wikipedia.org/wiki/プリコンパイルヘッダー "wikilink")と呼ばれる技術では[コンパイラ](../Page/コンパイラ.md "wikilink")が[ヘッダファイル](../Page/ヘッダファイル.md "wikilink")を処理した時点でコアダンプを作成し、ヘッダファイル処理時間を節約するといった利用方法もある。
...
Unix系システムでは、コアダンプは一般に[実行ファイル](https://ja.wikipedia.org/wiki/実行ファイル "wikilink")の[ファイルフォーマット](../Page/ファイルフォーマット.md "wikilink")を使っている。古い[UNIX](../Page/UNIX.md "wikilink")では、[a.outフォーマット](https://ja.wikipedia.org/wiki/a.outフォーマット "wikilink")が使われていた。[Linux](../Page/Linux.md "wikilink")、[System V](https://ja.wikipedia.org/wiki/UNIX_System_V "wikilink")、[Solaris](../Page/Solaris.md "wikilink")、[BSD](../Page/BSD.md "wikilink")などでは、[ELF](https://ja.wikipedia.org/wiki/Executable_and_Linkable_Format "wikilink") 形式が使われている。[macOS](https://ja.wikipedia.org/wiki/macOS "wikilink")では[Mach-O](https://ja.wikipedia.org/wiki/Mach-O "wikilink") 形式が使われている。

## 関連項目

  - [バグ](../Page/バグ.md "wikilink")
  - [デバッガ](../Page/デバッガ.md "wikilink")
  - [本物のプログラマはPascalを使わない](https://ja.wikipedia.org/wiki/本物のプログラマはPascalを使わない "wikilink") - むしろFORTRANとコアダンプを用いるという冗談

...
行列Aが[対称行列](../Page/対称行列.md "wikilink")である場合、 相似変換後に得られる行列A<sub>m+1</sub>は [三重対角行列となる](https://ja.wikipedia.org/wiki/対角行列#三重対角行列 "wikilink")。 三重対角行列の固有値は対角線上に並ぶため、固有値の特定が容易である。

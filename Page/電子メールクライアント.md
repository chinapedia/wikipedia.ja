...
メールユーザーエージェントとしての電子メールクライアントは通常、受信サーバ、送信サーバの[ホスト名](https://ja.wikipedia.org/wiki/ホスト名 "wikilink") ([FQDN](https://ja.wikipedia.org/wiki/Fully_Qualified_Domain_Name "wikilink")) と[アカウント](https://ja.wikipedia.org/wiki/アカウント "wikilink")（[メールアドレス](https://ja.wikipedia.org/wiki/メールアドレス "wikilink")やログイン[ID](https://ja.wikipedia.org/wiki/ID "wikilink")、[パスワード](https://ja.wikipedia.org/wiki/パスワード "wikilink")）、使用する通信プロトコルなどをあらかじめ設定しておく。

電子メールを扱う機能のほか、[ネットニュース](../Page/ネットニュース.md "wikilink")（[ニュースグループ](https://ja.wikipedia.org/wiki/ニュースグループ "wikilink")）の閲覧・投稿を行うニュースリーダ機能、[RSS](https://ja.wikipedia.org/wiki/RSS "wikilink")などを閲覧する[フィードリーダー](https://ja.wikipedia.org/wiki/フィードリーダー "wikilink")機能、[スケジュール](https://ja.wikipedia.org/wiki/スケジュール "wikilink")管理機能などが統合された電子メールクライアントソフトウェアもある。
...
受信メールサーバ上のユーザーのメールボックスまで届いた受信メールが、最後にそこから電子メールクライアントまで転送される方式は、プッシュ型とプル型に分類される。
  ...
  - プッシュ型
    [プッシュ型電子メール](https://ja.wikipedia.org/wiki/プッシュ型電子メール "wikilink")（プッシュメール、プッシュ型メール）とは、受信メールサーバが能動的に電子メールクライアントへ、即時に転送する方式を指す。
    電源が入った状態では常に[無線](https://ja.wikipedia.org/wiki/無線 "wikilink")でネットワーク接続される[携帯機器](../Page/携帯機器.md "wikilink")は、プッシュ型が有効である。日本では、1999年から [iモードメール](https://ja.wikipedia.org/wiki/iモードメール "wikilink")といった[携帯電話](../Page/携帯電話.md "wikilink")の電子メールにプッシュ型を使っている。
  - プル型
    **電子メールクライアント**が受信メールサーバに[ログイン](https://ja.wikipedia.org/wiki/ログイン "wikilink")時とその後一定間隔で[ポーリングし](https://ja.wikipedia.org/wiki/ポーリング_\(情報\) "wikilink")、新たな受信メールがあるかどうかを調べ、あればそれをユーザーのコンピュータ上のメールボックスにダウンロードしたりする。
    電子メールクライアントがネットワークに[常時接続](https://ja.wikipedia.org/wiki/常時接続 "wikilink")でない場合、[ネットワークアドレス](https://ja.wikipedia.org/wiki/ネットワークアドレス "wikilink")が頻繁に変更されるため、プッシュ型は適さず、通常はポーリングを行う。例えば、[Wi-Fi](../Page/Wi-Fi.md "wikilink")接続するノートパソコンは [DHCPサーバから](https://ja.wikipedia.org/wiki/Dynamic_Host_Configuration_Protocol "wikilink")1回きりのアドレスを付与され、ネットワーク名も一定しない。したがって受信メールサーバに新たな電子メールが到着しても、その転送先となるクライアントのその時点のアドレスは不明である。
...
ヘッダで指定された[エンコーディング](https://ja.wikipedia.org/wiki/エンコーディング "wikilink")指定に基づいてメール本文を表示したり、[添付ファイル](https://ja.wikipedia.org/wiki/添付ファイル "wikilink")の処理を行う。返信されたメールの本文のうち、行頭に引用を表す記号（一般的に"**\>**"）が付加された部分の表示色を変更して表示する機能をもつものもある。元来、SMTPの制約により 7bit の文字しか使用できなかった ([RFC](../Page/Request_for_Comments.md "wikilink")821) 電子メールでは、[バイナリ](../Page/バイナリ.md "wikilink")データを添付する場合は[MIME規格に基づき符号化されるが](https://ja.wikipedia.org/wiki/Multipurpose_Internet_Mail_Extensions "wikilink")、電子メールクライアントはこれを復号し、元のデータに復元する。
...
電子メールクライアントには、受信した電子メールの一覧を作成し並べかえて表示する[ソート](../Page/ソート.md "wikilink")機能がある。ローカルのメールボックスを複数作成して電子メールをまとまりごとに管理するほか、表示色を変更したり、[フラグの概念を用いてメールを分類して保存することもよく行われる](https://ja.wikipedia.org/wiki/フラグ_\(コンピュータ\) "wikilink")。電子メールのヘッダにある情報に基づき、返信したメールを[スレッド](../Page/スレッド.md "wikilink")表示する機能をもつ電子メールクライアントもある。メールヘッダや本文などに含まれている特定の文字列に応じ自動的に分類を行ったり、[ベイジアンフィルタ](https://ja.wikipedia.org/wiki/ベイジアンフィルタ "wikilink")などを用いて[スパム](../Page/スパム_\(メール\).md "wikilink")（いわゆる迷惑メール）を抽出する機能をもつものもある。

### 電子メールの作成

電子メールを作成する場合、SMTP (RFC 5321) に沿ったフォーマットで作成しなければならない。通常、電子メールは7[ビット](../Page/ビット.md "wikilink")文字のみを用いた[テキストファイル](https://ja.wikipedia.org/wiki/テキストファイル "wikilink")として作成される。[バイナリファイル](https://ja.wikipedia.org/wiki/バイナリファイル "wikilink")を添付するためには、電子メールクライアントにより[符号化](https://ja.wikipedia.org/wiki/符号化 "wikilink")が行われる。送信先（`To`、`Cc`、`Bcc`）や、題名（`Subject`）をはじめとしたヘッダを付加するのも電子メールクライアントの機能である。返信したメールの引用部分への引用記号の付加や、アドレスリスト、[署名](../Page/署名.md "wikilink")の管理、[テンプレート](../Page/テンプレート.md "wikilink")の適用などの機能を持つものもある。
...
## 関連項目
  ...
  - [Outbound Port 25 Blocking](https://ja.wikipedia.org/wiki/Outbound_Port_25_Blocking "wikilink")(OP25B)

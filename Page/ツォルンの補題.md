...
[4x4_grid_spanning_tree.svg](https://ja.wikipedia.org/wiki/File:4x4_grid_spanning_tree.svg "fig:4x4_grid_spanning_tree.svg")が[全域木](https://ja.wikipedia.org/wiki/全域木 "wikilink")を持つことが分かる。部分グラフのうち、木であるものからなる集合は包含関係により順序付けられ、鎖の和集合は上界となる。ツォルンの補題により極大の木が存在する。グラフが連結であるため、これは全域木である。この図のような有限グラフについてはツォルンの補題は不要である。\]\]

[集合論](https://ja.wikipedia.org/wiki/集合論 "wikilink")において**ツォルンの補題**（ツォルンのほだい、）または**クラトフスキ・ツォルンの補題**（クラトフスキ・ツォルンのほだい）とは次の定理をいう。

  - 命題 (Zorn の補題)
    [半順序集合](https://ja.wikipedia.org/wiki/半順序集合 "wikilink")*P*は、その全ての鎖(つまり、[全順序](https://ja.wikipedia.org/wiki/全順序 "wikilink")[部分集合](https://ja.wikipedia.org/wiki/部分集合 "wikilink"))が*P*に[上界](https://ja.wikipedia.org/wiki/上界 "wikilink")を持つとする。このとき、*P*は少なくともひとつの[極大元](https://ja.wikipedia.org/wiki/極大元 "wikilink")を持つ。

この定理は数学者[マックス・ツォルン](https://ja.wikipedia.org/wiki/マックス・ツォルン "wikilink")と[カジミェシュ・クラトフスキ](https://ja.wikipedia.org/wiki/カジミェシュ・クラトフスキ "wikilink")に因む。
...
[ZF集合論において](https://ja.wikipedia.org/wiki/公理的集合論 "wikilink")、ツォルンの補題は[整列可能定理](https://ja.wikipedia.org/wiki/整列可能定理 "wikilink")や[選択公理](https://ja.wikipedia.org/wiki/選択公理 "wikilink")と同値である。すなわち、ひとつを仮定すると残りを証明することができる。この補題は[関数解析学](https://ja.wikipedia.org/wiki/関数解析学 "wikilink")においては[ハーン・バナッハの定理](https://ja.wikipedia.org/wiki/ハーン・バナッハの定理 "wikilink")を、[線型代数](https://ja.wikipedia.org/wiki/線型代数 "wikilink")においては[基底の存在を](https://ja.wikipedia.org/wiki/基底_\(線型代数学\) "wikilink")、[位相空間論](https://ja.wikipedia.org/wiki/位相空間論 "wikilink")においては「任意の[コンパクト集合](https://ja.wikipedia.org/wiki/コンパクト集合 "wikilink")の直積はまたコンパクトである」という[チコノフの定理](https://ja.wikipedia.org/wiki/チコノフの定理 "wikilink")を、そして[代数学](../Page/代数学.md "wikilink")においては全てのゼロでない[環は](https://ja.wikipedia.org/wiki/環_\(数学\) "wikilink")[極大イデアル](https://ja.wikipedia.org/wiki/極大イデアル "wikilink")を持ち、任意の[体における](https://ja.wikipedia.org/wiki/可換体 "wikilink")[代数的閉包](https://ja.wikipedia.org/wiki/代数的閉包 "wikilink")の存在をそれぞれ証明する際に使われる。

## 例

ツォルンの補題を使って、単位元を持つ自明でない全ての環 *R* が極大イデアルを持つことを示すことができる。上記の用語でいうと、*P* は *R* の(両側)[イデアルのうち](https://ja.wikipedia.org/wiki/イデアル_\(環論\) "wikilink") *R* 自身以外からなる集合とする。これは自明なイデアル {0} を含むので空ではない。この集合は[包含関係](https://ja.wikipedia.org/wiki/包含関係 "wikilink")により半順序集合である。極大イデアルを見つけることは *P* の極大元を見つけることと同じである。ここで、*R* を取り除いたのは極大イデアルの定義には、*R* に等しくないことが入っているからである。
...
この証明は環 *R* が[乗法単位元](https://ja.wikipedia.org/wiki/乗法単位元 "wikilink") [1](https://ja.wikipedia.org/wiki/1 "wikilink") を持っていることに依存していることに注意しよう。これ無しではこの証明は無効であり、さらにこの言明は偽になりうる。例えば、 に[通常の加法と自明な乗法](https://ja.wikipedia.org/wiki/加法 "wikilink")(つねに ab = 0)を入れた環は極大イデアルも 1 も持たない: この環のイデアルは加法による部分群そのものである。真部分群  による商群  は[可除群](https://ja.wikipedia.org/wiki/可除群 "wikilink")である。よって、有限生成にはならず、 を真に含む自明でない部分群が存在する。
...
この関数 *b* を使うことで、*P* の元の列 *a*<sub>0</sub> \< *a*<sub>1</sub> \< *a*<sub>2</sub> \< *a*<sub>3</sub> \< ... を定めることができる。この列は**本当に長い**、添え字の範囲は単なる[自然数](../Page/自然数.md "wikilink")ではなく、全ての[順序数](https://ja.wikipedia.org/wiki/順序数 "wikilink")を動く。実は *P* と比較しても長すぎる。順序数の全体は[真クラス](https://ja.wikipedia.org/wiki/真クラス "wikilink")を成すほど大きすぎて、普通の集合より大きくなる。そして、この長さにより集合 *P* の元を使い尽くすことで矛盾を得る。

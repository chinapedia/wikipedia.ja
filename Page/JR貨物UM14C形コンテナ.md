...
無蓋コンテナ以外の他形式では、形式を表す1 - 2桁の数字は全てコンテナの内容積で決定されているが、無蓋コンテナのみ床面積で付与されている。 これによりm<sup>3</sup>の算出は、厳密には端数四捨五入計算の為に、内容積13.5 - 14.4m<sup>3</sup>の間に属するコンテナが対象となる。 また形式によっては、ドライコンテナと同等の高さがあったり、逆に1／8サイズの板型の様に8段**（12ftの場合）**又は、1／4サイズの薄型の様に4段**（20ftの場合）**に積み上げてやっとドライコンテナと同等の高さに達するなど、他形式ではタンクコンテナ以外では見られない様に、見た目が多種多彩に入り乱れているのも無蓋コンテナ形式の特徴となっている。
...
## 参考文献
  ...
  - 無蓋[1](http://container.pro.tok2.com/MUTON.html) コンテナの絵本。

      - ※リンク先の実写画像一覧表内で、【UM14C】欄を参照のこと。

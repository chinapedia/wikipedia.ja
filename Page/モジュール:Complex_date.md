...
`       p.Error = string.format('`<span style="background-color:red;">`Error in `[`Module:Complex``   ``date`](https://ja.wikipedia.org/wiki/Module:Complex_date "wikilink")`: input parameter "%s" is not recognized.`</span>`', operation or 'nil')`
`       return ''`
`   end`
...
`       out = a[1]`
`   else`
`       p.Error = string.format('`<span style="background-color:red;">`Error in `[`Module:Complex``   ``date`](https://ja.wikipedia.org/wiki/Module:Complex_date "wikilink")`: %s is not recognized.`</span>`', str1)`
`   end`
...
`   --if not cd then`
`   --  return ' `
`error: ' .. d1.." / " .. d2.." / ".. (cd or '') .." / ".. string.sub(d1,2,5).." / " .. string.sub(d2,2,5).." / " .. tostring(k)`
`   --end`
...
[Category:Pages_using_Complex_date_template_with_incorrect_parameter](https://ja.wikipedia.org/wiki/Category:Pages_using_Complex_date_template_with_incorrect_parameter "wikilink")
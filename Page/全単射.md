...
\[\forall\ b \in B,\,\exists\ !\ a \in A \text{ s.t. } b=f(a)\] が成り立つことと等価である。実際、全射と単射の定義を合わせれば、全射の定義における[存在記号](https://ja.wikipedia.org/wiki/存在記号 "wikilink") \(\exists\) を唯一存在記号 \(\exists\ !\) に置き換えればよいことがすぐに分かる。
...
## 性質
  ...
  - 2つの全単射が合成できるならば、その合成写像も全単射である。
      - 集合 *X* 上の全単射全体の成す集合を *S* とすると、*S* は写像の合成に関して[群を成す](https://ja.wikipedia.org/wiki/群論 "wikilink")。これを *X* 上の置換群あるいは[対称群](https://ja.wikipedia.org/wiki/対称群 "wikilink")と呼ぶ。
  - 集合全体のつくるクラス（類）において、「2つの集合の間に全単射が存在する」 という関係は[同値関係](https://ja.wikipedia.org/wiki/同値関係 "wikilink")を定める。この同値関係により集合全体の成すクラスを類別して[濃度の概念が定義される](https://ja.wikipedia.org/wiki/濃度_\(数学\) "wikilink")。すなわち、集合間で全単射が定義可能な場合、それらの集合は[基数](https://ja.wikipedia.org/wiki/基数 "wikilink")が等しい。

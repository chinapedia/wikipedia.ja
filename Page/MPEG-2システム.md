...
符号化された画像・音声データのそれぞれを、**エレメンタリストリーム**（ES：Elementary Stream）と呼ぶ。MPEG-2システムファイルにおける各ES（格納可能なメディアデータ・コーデックの種類）を以下に説明する。
  ...
  - ビデオ
    [MPEG-1](../Page/MPEG-1.md "wikilink")、[MPEG-2](../Page/MPEG-2.md "wikilink")（拡張規格：[MPEG-4](../Page/MPEG-4.md "wikilink")、[MPEG-4 AVC](https://ja.wikipedia.org/wiki/H.264 "wikilink")、[VC-1](https://ja.wikipedia.org/wiki/VC-1 "wikilink")）
  - オーディオ
...
TSでは複数のプログラムを取り扱うことが可能であるがそのTSにどのようなプログラムが存在し、TSに含まれる各ESがどのプログラムに属しているかを記した情報を**PSI**（Program Specific Information）という。PSIは 動画や音声とは別のPIDを持つTSパケットとして、TSに挿入される。PSIは、以下のようなものが存在する。特にPID=0のものはPATと呼ばれるほかPMT、CATなどが存在する。
  ...
  - PAT（Program Association Table）
    あるTS内に含まれるプログラム一覧を、PMTのPID一覧で格納したのがPATである。PATのPIDは必ず0と決まっている。なお[ワンセグ](https://ja.wikipedia.org/wiki/ワンセグ "wikilink")では帯域削減の為含まれず、PMTは0x1FC8もしくは0x1FC9といった特定の値を持つ。
...
SIはPSIの一部であると見なして、PSI/SI情報としてまとめて扱う場合が多い。またSIには[EPGのほぼ全てが含まれるので](https://ja.wikipedia.org/wiki/電子番組ガイド "wikilink")、SI/EPGとまとめて扱う場合も多い

[ARIBがARIB](https://ja.wikipedia.org/wiki/電波産業会 "wikilink") STD-B10で規定するSIには、以下のようなものが存在する。

  - NIT（Network Information Table）
    チャンネル番号や[変調方式](../Page/変調方式.md "wikilink")、[ガードインターバル](https://ja.wikipedia.org/wiki/ガードインターバル "wikilink")など、送信するネットワークに関する情報が含まれる。NITのPIDは必ず0x10と決まっている。
...
1.  現在ではMPEG-2システムの拡張が行われており、MPEG-2誕生以後に開発された[H.264](https://ja.wikipedia.org/wiki/H.264 "wikilink")や[VC-1](https://ja.wikipedia.org/wiki/VC-1 "wikilink")の映像コーデックを多重化することも可能となっている。

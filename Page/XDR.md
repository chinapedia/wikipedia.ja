...
その反面、マシンの内部フォーマットと正規化形式と違う場合、同じアーキテクチャ同士のデータ交換でも常に変換オーバーヘッドが発生する。
  ...
  - 例）　[IA-32](https://ja.wikipedia.org/wiki/IA-32 "wikilink")/[64系](https://ja.wikipedia.org/wiki/IA-64 "wikilink")（リトルエンディアン）同士の通信では、常にエンディアン変換が発生する。（Sun Workstation同士の通信の場合、ビッグエンディアンのため、エンディアン変換は発生しない）

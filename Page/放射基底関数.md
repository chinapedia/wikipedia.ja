...
放射基底関数は次の形式のの構築に使われることが多い。  ここで、この近似関数 *y*(***x***) は *N* 個の放射基底関数の総和で表され、個々の放射基底関数はそれぞれ異なる中心点 ***c***<sub>*i*</sub> を持ち、それぞれ固有の係数 *w*<sub>*i*</sub> で重み付けされている。この種の近似手法は、十分に単純な[カオス的振る舞いを示す](../Page/カオス理論.md "wikilink")[時系列](https://ja.wikipedia.org/wiki/時系列 "wikilink")の予測や[非線形系の](https://ja.wikipedia.org/wiki/非線型システム論 "wikilink")[制御に使われる](https://ja.wikipedia.org/wiki/制御理論 "wikilink")。

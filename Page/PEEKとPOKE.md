...
1980年代初期のベンダーであるは、独自のBASIC方言を持つNSDOSオペレーティングシステムを使用していたが、法的問題の懸念から、コマンド名を`EXAM`と`FILL`に変更していた。他に、MEMWとMEMRを代わりに使用するBASIC方言もあった。

[BBC Microやその他の](https://ja.wikipedia.org/wiki/BBC_Micro "wikilink")[エイコーン・コンピュータ](https://ja.wikipedia.org/wiki/エイコーン・コンピュータ "wikilink")のマシンで使用されたでは、PEEKやPOKEというキーワードはなく、*query*と呼ばれる[疑問符](../Page/疑問符.md "wikilink")(?)を関数とコマンドの両方の操作に使用していた。以下にその例を示す（BBC BASICでは日本語は使えないが、以下の例ではREMの後のコメント内の説明を日本語で表記している）。

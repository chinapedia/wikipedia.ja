...
tarの[GNU](../Page/GNU.md "wikilink")実装のバージョン1.22以降ではtarballのlzmaやxzへの透過的な圧縮をサポートしており、xz圧縮については`--xz`または`-J`、LZMA圧縮では`--lzma`[スイッチが使用できる](https://ja.wikipedia.org/wiki/:en:Command-line_interface#Command-line_option "wikilink")。

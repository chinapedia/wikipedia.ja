...
ST-506は、[S-100バス](https://ja.wikipedia.org/wiki/S-100バス "wikilink")のコントローラカードを使用することにより、4台まで接続する事ができた。各ドライブは34pinの制御線と20pinのデータ線の2本で接続する。制御信号、データ信号共々5[MHz](https://ja.wikipedia.org/wiki/MHz "wikilink")で駆動され、最大625Kバイト/秒のデータ転送速度を発揮できた。

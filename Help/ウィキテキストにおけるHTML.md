> この記事は[Help:ウィキテキストにおけるHTML](https://ja.wikipedia.org/wiki/Help:ウィキテキストにおけるHTML)から翻訳されています。
...
以下は、使用できますが[HTML5](../Page/HTML5.md "wikilink")で廃止された要素であり、ウィキペディアは最終的にHTML5で出力されるため使うべきではありません。代替方法は[:en:Wikipedia:HTML5](https://ja.wikipedia.org/wiki/:en:Wikipedia:HTML5 "wikilink")を参照。
...
`This word is `<b>`bold`</b>`.    `[`ウィキペディア`](../Page/ウィキペディア.md "wikilink")の` URL は`[`1`](https://ja.wikipedia.org)` です。`
...
は右のように生成されます。これは、 `infobox` クラスが [Mediawiki:Common.css](../MediaWiki/Common.css.md "wikilink") の記述によって右端揃えの浮動ボックスとして定義されているからです。
...
楽譜を描画します。[Help:Scoreを参照してください](https://ja.wikipedia.org/wiki/Help:Score "wikilink")。

### math, math chem, chem

数式や化学式を描画します。[:en:Help:Displaying a formula](https://ja.wikipedia.org/wiki/:en:Help:Displaying_a_formula "wikilink")あるいは[m:ヘルプ:数式の書き方を参照してください](https://ja.wikipedia.org/wiki/m:Help:Displaying_a_formula/ja "wikilink")。かつて化学式の要素には<ce>が使われていましたが、この要素名は短すぎて意味が分かりづらいという指摘があったため、2017年3月から<chem>が使用可能になりました\[7\]\[8\]。
...
[GeSHi 構文ハイライト機能](http://qbnz.com/highlighter/)を利用してプログラミング言語などをみやすく表示する要素です。長らく `<source>` が使われてきましたが、このタグを使用するプログラミング言語を表示する際に不都合なため、同じ意味を持つ要素 `<syntaxhighlight>` が追加されました\[9\]。 `lang=` 言語を指定して使います。
...
7.  [phab:T153606](https://ja.wikipedia.org/wiki/phab:T153606 "wikilink")
8.  [Wikipedia:お知らせ/過去ログ/2017年01月\#Tech News: 2017-03](https://ja.wikipedia.org/wiki/Wikipedia:お知らせ/過去ログ/2017年01月#Tech_News:_2017-03 "wikilink")
9.  [rev:50696](https://ja.wikipedia.org/wiki/rev:50696 "wikilink")(MediaWiki)
...
**グリーン関数** () とは、[微分方程式](../Page/微分方程式.md "wikilink")や偏微分方程式の解法の一つである[グリーン関数法](https://ja.wikipedia.org/wiki/グリーン関数法 "wikilink")に現れる[関数である](https://ja.wikipedia.org/wiki/関数_\(数学\) "wikilink")。グリーン関数法は、[英国の](https://ja.wikipedia.org/wiki/イギリス "wikilink")[数学者](../Page/数学者.md "wikilink")[ジョージ・グリーン](https://ja.wikipedia.org/wiki/ジョージ・グリーン "wikilink")によって考案された。

[物理学](../Page/物理学.md "wikilink")、[数学](../Page/数学.md "wikilink")、[工学](https://ja.wikipedia.org/wiki/工学 "wikilink")各分野において非常に重要な関数であり、広い用途で使用される。物理学におけるグリーン関数は**[プロパゲーター](https://ja.wikipedia.org/wiki/プロパゲーター "wikilink")**（**伝播関数**）とも呼ばれる。
...
下の[偏微分方程式](https://ja.wikipedia.org/wiki/偏微分方程式 "wikilink")の（初期値）[境界値問題](https://ja.wikipedia.org/wiki/境界値問題 "wikilink")を例に考える。
...
ここで、L は微分作用素、Ω は領域であり、領域の境界 Γ は、\(\bar{y_1}\) が規定されている境界 Γ<sub>1</sub> と、\(\partial y/\partial n\) が規定されている境界 Γ<sub>2</sub> からなり、Γ<sub>1</sub> ∪ Γ<sub>2</sub> = Γ、Γ<sub>1</sub> ∩ Γ<sub>2</sub> = ∅ であるものとする。また、*n* は境界での外向き法線方向を示す。
...
グリーン関数はもともと微分方程式の境界値問題に現れる関数である。物理学においても微分方程式を解くためにグリーン関数を用いることも多いが、[量子物理学](https://ja.wikipedia.org/wiki/量子物理学 "wikilink")ではこれを拡張して使っている\[1\]。つまり物理学においてグリーン関数は2通りの意味で扱われている。\[2\]
  ...
  - 境界値問題における微分方程式の主要解を意味し、与えられた全ての境界条件・初期条件を満足する。物理学では、微分方程式を直接解く代わりに、まず単純な点源問題の解であるグリーン関数を求めた後、[重ね合わせの原理](https://ja.wikipedia.org/wiki/重ね合わせの原理 "wikilink")によって微分方程式の解をグリーン関数を用いて表す。
...
[電磁気学](../Page/電磁気学.md "wikilink")における[ポアソン方程式](https://ja.wikipedia.org/wiki/ポアソン方程式 "wikilink")\(\Delta \varphi(\boldsymbol{r})=-\rho(\boldsymbol{r})\)の解\(\varphi(\boldsymbol{r})\)を求めたい。この方程式の解として[積分方程式](https://ja.wikipedia.org/wiki/積分方程式 "wikilink")\(\varphi(\boldsymbol{r})=\int G(\boldsymbol{r},\boldsymbol{r'})\rho(\boldsymbol{r'})d\boldsymbol{r'}\)を仮定し、ポアソン方程式に代入するとグリーン関数\(G(\boldsymbol{r},\boldsymbol{r'})\)の満たすべき式が得られる。

\[\Delta G(\boldsymbol{r},\boldsymbol{r'}) = -\delta(\boldsymbol{r}-\boldsymbol{r'})\] これを解くために両辺を[フーリエ変換](https://ja.wikipedia.org/wiki/フーリエ変換 "wikilink")すると、\(G(\boldsymbol{r},\boldsymbol{r'})\)のフーリエ変換\(g(\boldsymbol{k},\boldsymbol{r'})=\frac{e^{-i\boldsymbol{k \cdot r'}}}{\boldsymbol{k}^2}\)が得られる。これを[逆フーリエ変換](https://ja.wikipedia.org/wiki/逆フーリエ変換 "wikilink")するとグリーン関数\(G(\boldsymbol{r},\boldsymbol{r'})\)が求まる。

\[G(\boldsymbol{r},\boldsymbol{r'})=\frac{1}{4\pi|\boldsymbol{r}-\boldsymbol{r'}|}\] よってポアソン方程式の解は次のように求まる。

\[\varphi(\boldsymbol{r})=\int \frac{\rho(\boldsymbol{r'})}{4\pi|\boldsymbol{r}-\boldsymbol{r'}|}d\boldsymbol{r'}\] 以上のことから、位置\(\boldsymbol{r}\)の点電荷が別の位置\(\boldsymbol{r'}\)に作る静電ポテンシャルを表したものがグリーン関数であり、これを[重ね合わせ](https://ja.wikipedia.org/wiki/重ね合わせ "wikilink")たものが[電荷分布](https://ja.wikipedia.org/wiki/電荷分布 "wikilink")\(\rho(\boldsymbol{r})\)の作る[静電ポテンシャル](https://ja.wikipedia.org/wiki/静電ポテンシャル "wikilink")\(\varphi(\boldsymbol{r})\)であることがわかる。

### ダランベール演算子

## グリーン演算子と形式論

微分演算子を[線型演算子](https://ja.wikipedia.org/wiki/線型写像 "wikilink")  と見て、微分方程式  −*ρ*}} を解きたいとき、一種の逆演算子  を求めることができれば、 −*L**ρ*}} というように微分方程式を解くことができる。これは[線型代数](https://ja.wikipedia.org/wiki/線型代数 "wikilink")における連立方程式において、係数行列の逆行列を求めることができれば連立方程式を解くことができることと対応している。このような  を**グリーン演算子**  という。グリーン演算子を[行列表示](https://ja.wikipedia.org/wiki/行列表示 "wikilink")したときの[行列要素](https://ja.wikipedia.org/wiki/行列要素 "wikilink")を**グリーン関数**という。

このようにグリーン関数を抽象的な演算子と考えて取り扱うことには次のような利点がある。

  - 微分演算子や積分演算子だけでなく、[第二量子化](https://ja.wikipedia.org/wiki/第二量子化 "wikilink")のような抽象的な演算子を用いた理論に対してもそのまま用いられる。それは定常状態の[シュレーディンガー方程式](../Page/シュレーディンガー方程式.md "wikilink")において[ハミルトニアン](https://ja.wikipedia.org/wiki/ハミルトニアン "wikilink")を第二量子化における演算子で書かれていると考えるだけである。
...
= \frac{1}{4\pi|\boldsymbol{r}-\boldsymbol{r}'|} e^{\pm ik|\boldsymbol{r}-\boldsymbol{r}'|}\] ここで \(G_+^0\) は外向き、\(G_-^0\) は内向きの[球面波](https://ja.wikipedia.org/wiki/球面波 "wikilink")で、波が }} から  へ伝播する様子を示すものである。

### 散乱理論

[散乱理論](https://ja.wikipedia.org/wiki/散乱理論 "wikilink")の形式論では、グリーン関数が用いられる。その基本方程式にもグリーン関数が含まれ、[リップマン‐シュウィンガー方程式](https://ja.wikipedia.org/wiki/リップマン‐シュウィンガー方程式 "wikilink")と呼ばれる。

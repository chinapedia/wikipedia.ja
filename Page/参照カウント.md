...
**参照カウント**（さんしょうカウント、）は、メモリオブジェクトのライフサイクル（寿命）管理に使用される方式のひとつ。[ガベージコレクション](../Page/ガベージコレクション.md "wikilink")の実装方法および[ガベージコレクタ](https://ja.wikipedia.org/wiki/ガベージコレクタ "wikilink")の動作方法のひとつとしても利用される。また、[コピーオンライト](https://ja.wikipedia.org/wiki/コピーオンライト "wikilink")の実装方法としても多用される。

## 処理の概要

  - すべてのオブジェクト（メモリ上に置かれているデータの単位）に対して、参照カウントと呼ばれる整数値を付加しておく。これは、このオブジェクトへの**[参照](https://ja.wikipedia.org/wiki/参照_\(情報工学\) "wikilink")**（あるいは[ポインタ](https://ja.wikipedia.org/wiki/ポインタ_\(プログラミング\) "wikilink")）がシステム全体にいくつ存在しているかを数えるものである。
...
[文字列](https://ja.wikipedia.org/wiki/文字列 "wikilink")オブジェクトの実装手段としては適しており、多くのシステムで採用されている。 これは、文字列であれば内部で他のオブジェクトを参照しないので、短所の多くには影響がないためである。

単純なビット列などのデータも同様に適している。

## 循環参照の問題点

参照カウントには、[循環参照](https://ja.wikipedia.org/wiki/循環参照 "wikilink")によりデータを解放できなくなる（[メモリリーク](https://ja.wikipedia.org/wiki/メモリリーク "wikilink")が発生する）という問題がある。

例えば[C++](../Page/C++.md "wikilink")向けの[Boost C++ライブラリあるいは](https://ja.wikipedia.org/wiki/Boost_C++ライブラリ "wikilink")[C++11](https://ja.wikipedia.org/wiki/C++11 "wikilink")規格以降の[標準C++ライブラリ](https://ja.wikipedia.org/wiki/標準C++ライブラリ "wikilink")には、参照カウントベースの[スマートポインタ](https://ja.wikipedia.org/wiki/スマートポインタ "wikilink")を実現するクラステンプレートとして、それぞれ`boost::shared_ptr`あるいは`std::shared_ptr`が用意されているが、これを使って自己参照あるいは相互参照を持つクラスを定義してしまうと、参照カウントが適切に減らされずに互いのインスタンスが解放されなくなってしまう。
...
この問題を解消するために、多くの[プログラミング言語](../Page/プログラミング言語.md "wikilink")やソフトウェアライブラリあるいはシステムで[弱い参照](https://ja.wikipedia.org/wiki/弱い参照 "wikilink") (ウィークリファレンス、) が導入されている。弱い参照とは、参照カウントを増加させない参照である。例えばC++では`boost::weak_ptr`あるいは`std::weak_ptr`などが該当する。

一方、[マーク・アンド・スイープ](https://ja.wikipedia.org/wiki/マーク・アンド・スイープ "wikilink")や[コピーGC](https://ja.wikipedia.org/wiki/コピーGC "wikilink")によるガベージコレクションでは、少なくとも循環参照によるメモリリークの問題は発生しない。

なお、[Java](https://ja.wikipedia.org/wiki/Java "wikilink")や[.NET Framework](https://ja.wikipedia.org/wiki/.NET_Framework "wikilink") ([.NET Compact Frameworkや](https://ja.wikipedia.org/wiki/.NET_Compact_Framework "wikilink")[Monoも含む](https://ja.wikipedia.org/wiki/Mono_\(ソフトウェア\) "wikilink")) では、いずれも参照カウントベースではないガベージコレクション方式を採用しており\[1\]\[2\]\[3\]\[4\]、循環参照によるメモリリークは発生しない。例えば以下の[Java](https://ja.wikipedia.org/wiki/Java "wikilink")コードは合法であり、循環参照していたとしてもガベージコレクションの回収対象となる。
...
[ウィキペディア](https://ja.wikipedia.org/wiki/ウィキペディア "wikilink")の「[孤立した記事](https://ja.wikipedia.org/wiki/特別:Lonelypages "wikilink")」は、参照カウントが0のものを表示しているだけなので、孤立した記事だけから参照されている記事は孤立した記事と見なされていない（これも循環参照の例である）。

## 実用例

  - [マイクロソフト](../Page/マイクロソフト.md "wikilink")の[Component Object Model](https://ja.wikipedia.org/wiki/Component_Object_Model "wikilink") (COM) におけるCOMオブジェクトは参照カウント方式で管理される。
...
<!-- end list -->
  ...
  - [GLib](https://ja.wikipedia.org/wiki/GLib "wikilink")に含まれるオブジェクトシステム*[GObject](https://ja.wikipedia.org/wiki/GObject "wikilink")*は参照カウント方式で管理を行う。
  - [ファイルシステム](../Page/ファイルシステム.md "wikilink")における[ハードリンク](https://ja.wikipedia.org/wiki/ハードリンク "wikilink")

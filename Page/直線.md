...
[1D_line.svg](https://ja.wikipedia.org/wiki/File:1D_line.svg "fig:1D_line.svg") [300pxである為](https://ja.wikipedia.org/wiki/画像:Empty.png "wikilink")、厳密に正しく表示した場合、視覚では確認不能となる）\]\] [300px](https://ja.wikipedia.org/wiki/画像:ABC_Line_1.svg "wikilink") **直線**（ちょくせん、line）は、太さを持たない[幾何学](../Page/幾何学.md "wikilink")的な対象である[曲線](https://ja.wikipedia.org/wiki/曲線 "wikilink")の一種で、その上にある点について一様に横たわる面である。まっすぐ無限に伸びて端点を持たない。まっすぐな線には直線の他に、有限の長さと両端を持つ**[線分](https://ja.wikipedia.org/wiki/線分 "wikilink")**（せんぶん、line segment、segment）と、一つの端点を始点として無限にまっすぐ伸びた**半直線**（はんちょくせん、ray、half-line）がある。表記の場合は可視化のために太さを持たせている。

## 概要

[ユークリッドの](https://ja.wikipedia.org/wiki/エウクレイデス "wikilink")[幾何学では](https://ja.wikipedia.org/wiki/ユークリッド幾何学 "wikilink")、直線は本質的に[無定義述語](https://ja.wikipedia.org/wiki/無定義述語 "wikilink")である。つまり、「直線とは何か」を直接定義せずに、ただある関係（[公理](../Page/公理.md "wikilink")・公準）を満たすものであるとして理論を展開していくのである。ユークリッド幾何学においては以下のようなことである：

1.  二つの異なる点を与えれば、それを通る直線は一つに決まる。
2.  一つの直線とその上にない一つの点が与えられたとき、与えられた点を通り与えられた直線に平行な直線を、ただ一つ引くことができる。

また、このような公理から例えば以下のようなことが導かれる：二つの異なる直線は高々一つの点を共有する。二つの異なる[平面](https://ja.wikipedia.org/wiki/平面 "wikilink")は、高々一つの直線を共有する。
...
[ユークリッド空間](../Page/ユークリッド空間.md "wikilink")内の有向線分を、その位置のみの違いを除くことにより類別して、幾何学的[ベクトル](https://ja.wikipedia.org/wiki/ベクトル "wikilink")（いわゆる矢印ベクトル）の概念を考えることができる。逆にベクトルを用いてユークリッド空間やその中の線分・直線を定式化することもできるが、これについては後述する。

ユークリッド幾何学のように、無定義述語と公理によって構築される幾何学では、直線が「まっすぐ」であるなどのイメージは本質を持たない。曲がった空間の幾何学である[非ユークリッド幾何学](https://ja.wikipedia.org/wiki/非ユークリッド幾何学 "wikilink")での直線（[測地線](https://ja.wikipedia.org/wiki/測地線 "wikilink")）はユークリッド幾何学の中で見ると曲がって見えるのである。

## 1 次元アフィン空間

[アフィン空間](https://ja.wikipedia.org/wiki/アフィン空間 "wikilink")（ベクトル）の理論を持ち出すと、次のようにして直線を定義することが出来る： [ユークリッド空間](../Page/ユークリッド空間.md "wikilink") *E*<sup>*n*</sup> に対して、任意の一点 *P* と 0 でない一つの[ベクトル](https://ja.wikipedia.org/wiki/ベクトル "wikilink") **a** が与えられたとき、
...
直線上の点に[実数](../Page/実数.md "wikilink")を対応させることで[数直線](https://ja.wikipedia.org/wiki/数直線 "wikilink")を考えることができる。具体的には、直線上に原点 *O* と単位点 *E* を指定し、任意の実数 *x* に対し、直線上にあり、一方の端点を原点とし、原点から単位点までを結ぶ有向線分との（向きまで込めた）線分[比](https://ja.wikipedia.org/wiki/比 "wikilink")が *x* となるような線分の、原点ではない側の端点と *x* とを対応付けたもののことをいう。
...
形式的には、点[集合](../Page/集合.md "wikilink") *V* が与えられたとき、[直積](https://ja.wikipedia.org/wiki/直積 "wikilink")集合 *V* × *V* の元を**有向線分** とし、さらに[同値関係](https://ja.wikipedia.org/wiki/同値関係 "wikilink") \~ を 任意の *a*, *b* ∈ *V* に対し (*a*, *b*) \~ (*b*, *a*) と定めたときの集合 *E* = *V* × *V* / \~ の元（同値類） \[(*a*, *b*)\] (*a*, *b* ∈ *V*, *a* ≠ *b*) のこと（これをしばしば {*a*, *b*} と記す）を *a* と *b* を結んだ線分と呼ぶ。
...
## 関連項目
  ...
  - [曲線](https://ja.wikipedia.org/wiki/曲線 "wikilink")
      - [放物線](https://ja.wikipedia.org/wiki/放物線 "wikilink")
  - [平面](https://ja.wikipedia.org/wiki/平面 "wikilink")

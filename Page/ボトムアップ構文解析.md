...
[プログラミング言語](../Page/プログラミング言語.md "wikilink")の[コンパイラ](../Page/コンパイラ.md "wikilink")におけるボトムアップ構文解析は、最初に[終端記号](https://ja.wikipedia.org/wiki/終端記号 "wikilink")を識別し、それらを徐々に組み合わせていって[非終端記号](https://ja.wikipedia.org/wiki/非終端記号 "wikilink")を生成する。この過程で人間の読める[ソースコード](../Page/ソースコード.md "wikilink")で書かれたプログラムの[構文木](https://ja.wikipedia.org/wiki/構文木 "wikilink")が構築され、そこから[アセンブリ言語](../Page/アセンブリ言語.md "wikilink")や[擬似コード](https://ja.wikipedia.org/wiki/擬似コード "wikilink")にコンパイルされる。

言語が異なれば構文解析技法も異なるが、実際に必要とされるより強力な構文解析技法を適用することも珍しくない。

ボトムアップ構文解析器による汎用[構文解析器](../Page/構文解析器.md "wikilink")もあり、特定の[プログラミング言語](../Page/プログラミング言語.md "wikilink")向けの構文解析器を生成するのに使われる（[パーサジェネレータ](https://ja.wikipedia.org/wiki/パーサジェネレータ "wikilink")）。例えば、[yacc](https://ja.wikipedia.org/wiki/yacc "wikilink")などがある。
...
トップダウン構文解析のように力尽くの方法でも機能する。つまり、パターンマッチする生成規則が見つかるまで次々に探索していく方法である。このような単純な例では示せないが、力任せの方法では不適切な規則の適用もあり、さらに規則を適用として後戻り（[バックトラッキング](https://ja.wikipedia.org/wiki/バックトラッキング "wikilink")）することになる。これは効率的ではない。そのような後戻りを防ぐために入力を先読みして不適切な規則を適用しないという方法もある。
...
以下に主なボトムアップ構文解析手法を挙げる。
  ...
  - [LR法](../Page/LR法.md "wikilink")
      ...
      - [SLR(1)](https://ja.wikipedia.org/wiki/単純LR法 "wikilink") - 単純で1つだけ先読みする方法

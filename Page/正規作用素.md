...
正規作用素の概念は[対合線型環への一般化される](https://ja.wikipedia.org/wiki/*-代数 "wikilink")。つまり、対合線型環の元  が**正規**であるとは、  *x*<sup>∗</sup> *x*}} を満たすときに言う。最も重要な場合は、対合線型環が[{{mathであるときである](https://ja.wikipedia.org/wiki/C*-環 "wikilink")。は正規元の例である。

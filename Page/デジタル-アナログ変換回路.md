...
[8_bit_DAC.svg](https://ja.wikipedia.org/wiki/File:8_bit_DAC.svg "fig:8_bit_DAC.svg") **デジタル-アナログ変換回路**（デジタル-アナログへんかんかいろ、**D/A変換回路**　digital to analog converter）は、[デジタル](../Page/デジタル.md "wikilink")電気信号を[アナログ](https://ja.wikipedia.org/wiki/アナログ "wikilink")電気信号に変換する[電子回路](../Page/電子回路.md "wikilink")である。**D/Aコンバーター**（**DAC**（ダック））とも呼ばれる。 また、**デジタル-アナログ変換**（デジタル-アナログへんかん、**D/A変換**）は、デジタル信号をアナログ信号に変換することをいう。

逆は[アナログ-デジタル変換回路](https://ja.wikipedia.org/wiki/アナログ-デジタル変換回路 "wikilink")である。[集積回路](../Page/集積回路.md "wikilink")化されている。 [DA_conversion_(ja).png](https://ja.wikipedia.org/wiki/File:DA_conversion_\(ja\).png "fig:DA_conversion_(ja).png")
**下**：復元された連続量を持つアナログ信号\]\]
...
<tr class="even">
<td><p><a href="https://ja.wikipedia.org/wiki/ΔΣ変調" title="wikilink">デルタシグマ形</a></p></td>
...
2進数データを[パルス幅変調](https://ja.wikipedia.org/wiki/パルス幅変調 "wikilink")データに変換し、その出力を[ローパスフィルタ](https://ja.wikipedia.org/wiki/ローパスフィルタ "wikilink")に通してパルス周波数による高周波成分を除去する。
...
デジタル入力を時間方向に補完してサンプリング周波数を数十倍にする（オーバーサンプリング）。この出力を[デルタ・シグマ・モジュレータを通すことで低ビットのオーバーサンプリングデータにする](https://ja.wikipedia.org/wiki/デルタシグマ変調 "wikilink")。デルタ・シグマ・モジュレータの目的はAD変換の場合と同じだが、高ビットのデジタル入力を、デジタル処理によって低ビットの「[ディザ](https://ja.wikipedia.org/wiki/ディザ "wikilink")化」されたデジタル出力にする。1bit出力であればパルス幅変調と似た出力になるが、デルタ・シグマ・モジュレータにより、より良いパルス波形になる。

この低ビット出力をDA変換し（1bitの場合は出力電力が足りればそのままでもよい）、パルス幅変調型と同様に[ローパスフィルタ](https://ja.wikipedia.org/wiki/ローパスフィルタ "wikilink")に通すことで[折り返し雑音](https://ja.wikipedia.org/wiki/折り返し雑音 "wikilink")（エイリアス）成分や[量子化誤差](https://ja.wikipedia.org/wiki/量子化誤差 "wikilink")成分を除去して、アナログ出力とする。

### 抵抗ストリング型

\(n\) bitの場合は、\(2^n\)個の抵抗を直列に接続して基準電圧を[分圧する事で](https://ja.wikipedia.org/wiki/分圧回路 "wikilink")、対応する電圧\(2^n\)個を全て得られる。この中から、デジタル入力に対応する電圧点を[アナログスイッチ](https://ja.wikipedia.org/wiki/アナログスイッチ "wikilink")で接続して出力する。

２点間の抵抗値をそのまま用いれば、デジタル[ポテンショメータ](https://ja.wikipedia.org/wiki/ポテンショメータ "wikilink")として用いる事もできる。
...
ビットに応じて重み付けをした[コンデンサ](https://ja.wikipedia.org/wiki/コンデンサ "wikilink")を充電し、全体の電圧を測定する。

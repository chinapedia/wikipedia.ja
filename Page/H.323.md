...
### 多地点間通信装置
  ...
  - Multipoint Control Unit ([MCU](https://ja.wikipedia.org/wiki/MCU "wikilink"))：パケットを端末に配分し、電話会議などの多地点間通信を行う機器。MCU は会議の資源を管理し、端末間の交渉をおこない、場合によってはメディア・ストリームを扱う。MCU はゲートウェイ、ゲートキーパーと同一のデバイス上に共存させることも可能である。

...
現在のパソコンに使われるCPUは[CISC](../Page/CISC.md "wikilink")と呼ばれるものが主流で、多くは一つのコアにコードを読み込み（ロード）、解釈（デコード）し、処理をスケジュールする装置を一つと、それを処理するパイプラインと呼ばれる演算装置を複数持っている。しかしスケジュールの能力に限りがあるため、例えば整数を処理するパイプラインが働いていても、浮動小数点や[MMX](https://ja.wikipedia.org/wiki/MMX "wikilink")などのマルチメディア計算をするパイプラインが空いていることがある。
...
一方苦手とするのは、例えばWindowsやLinuxのように雑多なDLLやライブラリー類が頻繁かつ非同期的にコールされ、またコードやデータが頻繁に更新される、いわゆるローカリティが低い場合である。この場合、複数のスレッドでCPUのデータ幅やL2キャッシュが細分化され（[スラッシング](https://ja.wikipedia.org/wiki/スラッシング "wikilink")）、ハイパースレッディングによって逆に効率が低下することもある。
...
すなわちハイパースレッディングはソフトウェアを改変せず利用できる[透過性があることが特徴である](https://ja.wikipedia.org/wiki/透過性_\(情報工学\) "wikilink")。もちろんハイパースレッディングを意識した処理として実装しているかどうかによって効率は変化する。
...
パソコン向けのCPUでは、2013年現在ハイパースレッディング搭載製品ではすべて物理コア1つ当たり論理コア2つである一方、[HPC用途や](https://ja.wikipedia.org/wiki/高性能計算 "wikilink")[メインフレーム](../Page/メインフレーム.md "wikilink")用途などの高価なCPUでは、例えば[IBM](../Page/IBM.md "wikilink")の[POWER8](https://ja.wikipedia.org/wiki/POWER8 "wikilink")や、[オラクルの](https://ja.wikipedia.org/wiki/オラクル_\(企業\) "wikilink")[Sparc T3は物理コア](https://ja.wikipedia.org/wiki/Sparc_T3 "wikilink")1つ当たり論理コア8つである。インテルの[Xeon PhiはCPU用途ではなく](https://ja.wikipedia.org/wiki/Xeon_Phi "wikilink")[GPGPU](https://ja.wikipedia.org/wiki/GPGPU "wikilink")のような使い方を想定しているが、これは物理コア1つが論理コア4つになっている。

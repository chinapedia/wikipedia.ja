...
**計算木論理**（けいさんきろんり、Computational Tree Logic、**CTL**）は、分岐[時相論理](https://ja.wikipedia.org/wiki/時相論理 "wikilink")の一種である。その時間モデルでは未来は決定されておらず[木構造のように分岐している](../Page/木構造_\(データ構造\).md "wikilink")。未来の複数の経路のうちの1つが実際に現実の経路となる。
...
CTL には作用素の最小セットがある。全てのCTLの論理式は、この最小セットだけを使った論理式に書き換え可能である。これは[モデル検査](https://ja.wikipedia.org/wiki/モデル検査 "wikilink")の際に便利である。最小セットの例として {false, \(\or, \neg\), **EG**, **EU**, **EX**} がある。

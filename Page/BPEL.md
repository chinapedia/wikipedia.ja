...
1.  外部の実体に対して[WSDL](https://ja.wikipedia.org/wiki/WSDL "wikilink") 1.1を用いて定義された[Webサービス](https://ja.wikipedia.org/wiki/Webサービス "wikilink")の操作を介してやりとりし、自身を WSDL 1.1を用いて定義された Webサービスとして宣言するビジネスプロセスを定義すること。やりとりは、portType の定義に依存しポートの定義には依存しないという意味で "抽象的" であること。
...
BPELの現代的な[ビジネスプロセス](../Page/ビジネスプロセス.md "wikilink")に対する焦点、さらに[WSFL](https://ja.wikipedia.org/wiki/WSFL "wikilink")および[XLANG](https://ja.wikipedia.org/wiki/XLANG "wikilink")の歴史から、BPELは[Webサービス](https://ja.wikipedia.org/wiki/Webサービス "wikilink")を外部の通信メカニズムとして採用することになった。 そのため、BPEL のメッセージング能力は、入出力される[メッセージ](../Page/メッセージ.md "wikilink")を記述するための[WSDL](https://ja.wikipedia.org/wiki/Web_Services_Description_Language "wikilink")1.1の使い方に依存する。

メッセージの送信受信を行える能力を提供することに加えて、BPELプログラミング言語は下記の項目をサポートする：

  - プロパティに基づくメッセージ間関連機構
  - XML および WSDL の型に基づく変数
  - 表現や問い合わせを複数の言語で書くことができるようにするための、拡張可能な言語プラグインモデル：BPELは[XPath](https://ja.wikipedia.org/wiki/XML_Path_Language "wikilink")1.0をデフォルトでサポートしている
  - 連接\[2\]、分岐\[3\]、反復\[4\]、並列\[5\]を含む[構造化プログラミング](../Page/構造化プログラミング.md "wikilink")要素
  - [ローカル変数](https://ja.wikipedia.org/wiki/ローカル変数 "wikilink")、障害[ハンドラー](https://ja.wikipedia.org/wiki/ハンドラー "wikilink")、補償ハンドラー、[イベントハンドラー](https://ja.wikipedia.org/wiki/イベントハンドラー "wikilink")を用いたロジックのカプセル化を可能とするスコープシステム
  - [変数への並行的なアクセスを制限するためのシリアライズされたスコープ](https://ja.wikipedia.org/wiki/変数_\(プログラミング\) "wikilink")

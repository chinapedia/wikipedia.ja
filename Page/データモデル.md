...
データモデルは、データまたは*構造化データ*の構造を明示的に決める。データモデルの代表的な応用は、[データベース・モデル](https://ja.wikipedia.org/wiki/データベース・モデル "wikilink")、[情報システム](https://ja.wikipedia.org/wiki/情報システム "wikilink")の設計、およびデータの交換を可能にすることを含む。通常データモデルは、[データモデリング](https://ja.wikipedia.org/wiki/データモデリング "wikilink")言語によって規定する\[3\]。
...
大量の構造化されたデータや構造化されないデータを管理することは、[情報システム](https://ja.wikipedia.org/wiki/情報システム "wikilink")の主要な機能である。データモデルは、リレーショナル・データベースのようなデータ管理システムでの[記憶装置](../Page/記憶装置.md "wikilink")のための構造化された[データ](../Page/データ.md "wikilink")を記述する。それらは、典型的に、[ワードプロセッサ](../Page/ワードプロセッサ.md "wikilink")文書、[電子メール](../Page/電子メール.md "wikilink")、ピクチャ、音声あるいはビデオのような、非構造化データを記述しない。

### データモデルの役割

[3-4_Data_model_roles.svg](https://ja.wikipedia.org/wiki/File:3-4_Data_model_roles.svg "fig:3-4_Data_model_roles.svg")

データモデルの主な目的は、データの定義とフォーマットを提供することによって、[情報システム](https://ja.wikipedia.org/wiki/情報システム "wikilink")の開発を支援することである。WestとFowler（1999）によれば、「もしこれがシステムを通して一貫して行われたら、そこでデータの互換性が達成されうる。もし同じデータ構造がデータの格納やアクセスに使われるなら、そこで異なるアプリケーションがデータを共有できる。これの結果は上で示される。しかしながら、システムとインタフェースは、しばしば、構築し、運用し、そして維持するため、それらがあるべきより多くのコストを費やす。それらは、事業を支援するよりむしろ制約するかもしれない。1つの大きな原因は、システムとインタフェースに実装されるデータモデルの品質が貧弱だったことである。」\[5\]
...
1960年代にデータモデリングは、[経営情報システム](https://ja.wikipedia.org/wiki/経営情報システム "wikilink")概念の導入と共に更にその重要性を増大させた。Leondes（2002）によれば、「必要なとき、情報システムは、管理目的のためデータと情報を提供する。このと呼ばれる、第一世代データベースシステムが、GEの[チャールズ・バックマン](../Page/チャールズ・バックマン.md "wikilink")によって設計された。2つの有名なデータベース・モデル:[ネットワーク型データモデル](https://ja.wikipedia.org/wiki/ネットワーク型データモデル "wikilink")、および[階層型データモデル](https://ja.wikipedia.org/wiki/階層型データモデル "wikilink")がこの期間中に提案された。」\[17\]1960年代の終わりに向けて、[エドガー・F・コッド](../Page/エドガー・F・コッド.md "wikilink")は、彼のデータ編成の理論を練り、[一階述語論理](../Page/一階述語論理.md "wikilink")に基づいたデータベース管理のための[リレーショナル・モデルを提案した](../Page/関係モデル.md "wikilink")\[18\]。

1970年代に[実体関連モデル](https://ja.wikipedia.org/wiki/実体関連モデル "wikilink")が、1976年に[ピーター・チェン](https://ja.wikipedia.org/wiki/ピーター・チェン "wikilink")によって初めて提案され、概念データモデルの新しいタイプとして出現した。実体関連モデルは、[データベース](../Page/データベース.md "wikilink")に格納される情報ニーズや[情報](https://ja.wikipedia.org/wiki/情報 "wikilink")のタイプを記述するための、[要求分析](../Page/要求分析.md "wikilink")中の[情報システム](https://ja.wikipedia.org/wiki/情報システム "wikilink")設計の最初のステージで使われた。この技術は、あらゆる[概念体系](https://ja.wikipedia.org/wiki/オントロジー_\(情報科学\) "wikilink")、すなわち、一定の[関心の領域のための](https://ja.wikipedia.org/wiki/議論領域 "wikilink")、概念の全貌と分類とそれらの関連、を記述できる。
...
<!-- end list -->
  ...
  - [ネットワーク型データモデル](https://ja.wikipedia.org/wiki/ネットワーク型データモデル "wikilink")：このモデルは、レコードとセットと呼ばれる、2つの基本的概念を使うデータを組織化する。レコードはフィールドを含み、セットはレコード間の、1は所有者、多はメンバーである、1対多の関連を定義する。
...
[地理情報システム](../Page/地理情報システム.md "wikilink")における[データモデル](https://ja.wikipedia.org/wiki/データモデル_\(GIS\) "wikilink")（[data model](https://ja.wikipedia.org/wiki/:en:Data_model_\(GIS\) "wikilink")）は、データとして地理的オブジェクトまたは地表を表現するための数学的概念である。たとえば、以下のような例がある。
  ...
  - [ラスターデータモデルは](https://ja.wikipedia.org/wiki/ビットマップ画像 "wikilink")、数値を格納するセル・マトリックスとして地形を表現する
...
## 関連項目
  ...
  - [ダイアグラム](https://ja.wikipedia.org/wiki/ダイアグラム "wikilink")

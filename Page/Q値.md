...
{P} </math>}} ここで\(\nu\) は共振周波数、 \(\mathcal{E}\) はキャビティに蓄えられるエネルギー、 \(P=-\frac{dE}{dt}\) は散逸率である。光学的Q値は、共振周波数をその共振器の半価幅で割ったものに等しい。キャビティ内の光子の寿命は、このQ値に比例する。 レーザーの技術の一つとして、キャビティのQ値を切り替えることによって、高出力を得ることができる。この技術は[Qスイッチ](../Page/Qスイッチ.md "wikilink")と呼ばれている。

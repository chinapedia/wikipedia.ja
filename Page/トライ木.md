...
キーが[文字列](https://ja.wikipedia.org/wiki/文字列 "wikilink")である[連想配列](../Page/連想配列.md "wikilink")の実装構造としても使われる。右図の例では、ノードを表す丸の中にキーが書かれ、連想される値がその下に書かれている。値が書かれていないノードはキー文字列の途中までにしか対応していない。各英単語には整数の値が対応している。

トライ木は一種の[決定性有限オートマトン](https://ja.wikipedia.org/wiki/決定性有限オートマトン "wikilink")と見ることもできるが、エッジ（枝）に対応する記号（文字）はその順序が暗黙のうちに決定される（辞書順など）。
...
トライ木は基本的にキーとして文字列を必要とするが、様々なデータ型を文字列に変換することもできる。例えば、整数を単にビットの列と見れば、文字列と何ら変わらない。[ルーティング](https://ja.wikipedia.org/wiki/ルーティング "wikilink")テーブルや[ページテーブル](https://ja.wikipedia.org/wiki/ページテーブル "wikilink")ではその性質から、プレフィックスが共通の整数がキーとしてよく使われる（つまり偏りがある）。
...
以下の[擬似コード](https://ja.wikipedia.org/wiki/擬似コード "wikilink")は与えられた文字列がトライ木にあるかどうかを判定する汎用のアルゴリズムを示したものである。ここで、`children` は子ノード群の配列であり、terminal ノードとは格納された単語に対応するノードを意味する。
...
1989年に青江順一がダブル配列を利用する方法を発表した\[1\]。トライ木は[決定性有限オートマトン](https://ja.wikipedia.org/wiki/決定性有限オートマトン "wikilink")として解釈する事が出来るが、決定性有限オートマトンはデフォルト（default）・ベース（base）・次（next） ・チェック（check）の4つの配列で表現できる\[2\]\[3\]。トライ木の場合はデフォルトは不要である。その上で、s から t へ c により遷移する場合は、以下の関係が成立すれば良い。
...
## 関連項目
  ...
  - [基数木](https://ja.wikipedia.org/wiki/基数木 "wikilink")（パトリシア木）
  - [DAWG](https://ja.wikipedia.org/wiki/DAWG "wikilink")
  - [三分探索木](https://ja.wikipedia.org/wiki/三分探索木 "wikilink")
  - [決定性有限オートマトン](https://ja.wikipedia.org/wiki/決定性有限オートマトン "wikilink")

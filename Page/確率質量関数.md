...
[thumbは](https://ja.wikipedia.org/wiki/画像:Discrete_probability_distrib.svg "wikilink")、確率質量がはたらく点に丸を付け、支柱を付けて表す。\]\] **確率質量関数**（かくりつしつりょうかんすう、, PMF）とは、[確率論](https://ja.wikipedia.org/wiki/確率論 "wikilink")および[統計学](../Page/統計学.md "wikilink")において、[離散型確率変数にその値をとる確率を対応させる関数のことである](../Page/離散確率分布.md "wikilink")\[1\]（単に確率関数ということもある）。
...
\[\sum_{x\in A} f_X(x) = 1\] であると考えることができる。 離散型確率変数には[順序を与えておくことでその](https://ja.wikipedia.org/wiki/順序集合 "wikilink")[離散確率分布](../Page/離散確率分布.md "wikilink")が[グラフ](https://ja.wikipedia.org/wiki/グラフ "wikilink")で表せる。確率変数ベクトルなどの[確率要素](https://ja.wikipedia.org/wiki/確率要素 "wikilink")に対しても同様である。離散型確率変数の[像以外では確率質量関数値は](https://ja.wikipedia.org/wiki/像_\(数学\) "wikilink") 、すなわち全ての \(x\notin X(S)\) に対して  0}} である。

すると、 の像は高々[可算集合](../Page/可算集合.md "wikilink")であるので、確率質量関数  は可算個の点を除いて全領域で  となる。確率質量関数の不連続性は、離散確率変数の[累積分布関数](https://ja.wikipedia.org/wiki/累積分布関数 "wikilink")もまた不連続であることを示す。微分可能な範囲では、微分値は 0 であり、その範囲では確率質量関数もまた  である。

## 測度論的定式化

離散型確率変数  の確率質量関数は 2 つのより一般的な測度論的構成の特別な場合と見ることができる。すなわち、[数え上げ測度](https://ja.wikipedia.org/wiki/数え上げ測度 "wikilink")に関して、 の[確率分布](../Page/確率分布.md "wikilink")と  の[確率密度関数](https://ja.wikipedia.org/wiki/確率密度関数 "wikilink")である。以下詳述する。
...
\frac{1}{2}, &x \in \{0, 1\},\\
0, &x \notin \{0, 1\}.
\end{cases}\] であり、これは[二項分布](https://ja.wikipedia.org/wiki/二項分布 "wikilink")の特別な場合に相当する。

多値を採る離散分布および確率質量関数の例は[多項分布](https://ja.wikipedia.org/wiki/多項分布 "wikilink")を参照。

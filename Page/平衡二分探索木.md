...
**平衡二分探索木**（へいこうにぶんたんさくぎ、）とは、[計算機科学](../Page/計算機科学.md "wikilink")において[二分探索木](https://ja.wikipedia.org/wiki/二分探索木 "wikilink")のうち木の高さ（根からの階層の数）を自動的にできるだけ小さく維持しようとするもの（[平衡木](https://ja.wikipedia.org/wiki/平衡木 "wikilink")）である。平衡二分探索木は[連想配列](../Page/連想配列.md "wikilink")や[集合](../Page/集合.md "wikilink")その他の[抽象データ型](../Page/抽象データ型.md "wikilink")を実装する最も効率のよい[データ構造](../Page/データ構造.md "wikilink")の1つである。

## 概要

[二分探索木](https://ja.wikipedia.org/wiki/二分探索木 "wikilink")上の大半の操作にかかるコストは木の高さに比例するので木の高さは低く保つのが望ましい。通常の[二分探索木](https://ja.wikipedia.org/wiki/二分探索木 "wikilink")の主要な欠点は、キーが辞書順に挿入されるような普通の状況で木の高さが大きくなってしまうということである。結果として[連結リスト](../Page/連結リスト.md "wikilink")同様のデータ構造になってしまい、全ての操作が高くつく結果となる。もしあらかじめ全てのデータが分かっているならば、値をランダムに追加することで木の高さを平均的に小さく保つことができるが、そのような贅沢はいつもできるわけではない。特に入力が一括して与えられることのない[オンラインアルゴリズム](../Page/オンラインアルゴリズム.md "wikilink")([online algorithm](https://ja.wikipedia.org/wiki/:en:Online_algorithm "wikilink"))の場合はそうである。
...
なお、2分ではない、平衡探索木としては[B木](../Page/B木.md "wikilink")、[2-3木](https://ja.wikipedia.org/wiki/2-3木 "wikilink")、[2-3-4木](https://ja.wikipedia.org/wiki/2-3-4木 "wikilink")などがある。木構造ではないが、同じような用途に使えるものとして[スキップリスト](https://ja.wikipedia.org/wiki/スキップリスト "wikilink")がある。treapやスキップリストは[乱択アルゴリズム](https://ja.wikipedia.org/wiki/乱択アルゴリズム "wikilink")。
...
多くのアルゴリズムで最悪ケースでの性能を、ほんの少しの手間で良好にするために、平衡二分探索木を利用することができる。例えば、[2分探索](https://ja.wikipedia.org/wiki/2分探索 "wikilink")を平衡二分探索木で行った場合、最適なO(*n* log *n*)のソートアルゴリズムを簡単に記述することができる(実際には、このようなアルゴリズムはキャッシュ効率が悪いという問題を抱える)。また、[計算幾何学](https://ja.wikipedia.org/wiki/計算幾何学 "wikilink")の多くのアルゴリズムは平衡二分探索木のバリエーションを利用して[線分の交差判定問題](https://ja.wikipedia.org/wiki/線分の交差判定問題 "wikilink")([line segment intersection](https://ja.wikipedia.org/wiki/:en:line_segment_intersection "wikilink"))や[点位置決定問題](https://ja.wikipedia.org/wiki/点位置決定問題 "wikilink")([point location problem](https://ja.wikipedia.org/wiki/:en:point_location_problem "wikilink"))を効率よく解決している。
...
## 関連項目
  ...
  - [スキップリスト](https://ja.wikipedia.org/wiki/スキップリスト "wikilink")

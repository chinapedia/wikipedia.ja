...
[thumb](https://ja.wikipedia.org/wiki/ファイル:Gamma.png "wikilink") [thumb](https://ja.wikipedia.org/wiki/ファイル:Gamma_abs.png "wikilink") 数学において**ガンマ関数**（ガンマかんすう、）とは、[階乗](../Page/階乗.md "wikilink")の概念を複素数全体に拡張した[特殊関数](https://ja.wikipedia.org/wiki/特殊関数 "wikilink")である。互いに同値となるいくつかの定義が存在するが、1729年、数学者[レオンハルト・オイラー](https://ja.wikipedia.org/wiki/レオンハルト・オイラー "wikilink")が[無限乗積](https://ja.wikipedia.org/wiki/無限乗積 "wikilink")の形で、最初に導入した\[1\]。

## 定義

実部が正となる複素数  について、次の積分で定義される関数  をガンマ関数と呼ぶ\[2\]。この積分は、[ルジャンドルの定義にしたがって](https://ja.wikipedia.org/wiki/アドリアン＝マリ・ルジャンドル "wikilink")、第二種[オイラー積分](https://ja.wikipedia.org/wiki/オイラー積分 "wikilink")とも呼ばれる。元は階乗の一般化としてオイラーが得たもので、 という記号は、1814年にルジャンドルが導入したものである\[3\]。それ以前は  などと表記していた（ただし ）。

一般の複素数  については、[解析接続](https://ja.wikipedia.org/wiki/解析接続 "wikilink")もしくは次の[無限乗積](https://ja.wikipedia.org/wiki/無限乗積 "wikilink")で定義される。 . </math>}}
...
\[\begin{align}
    \Gamma (n+1)=n!
\end{align}\] が成り立ち、その意味でガンマ関数は[階乗](../Page/階乗.md "wikilink")の定義域を[複素平面](https://ja.wikipedia.org/wiki/複素平面 "wikilink")に拡張したものとなっている。そもそもガンマ関数は「階乗の複素数への拡張となるもの」の実例としてオイラーが考案したのである。実際には、そのような関数は無数に存在するが、正の実軸上で[対数凸である解析関数という条件を付ければ](https://ja.wikipedia.org/wiki/凸関数 "wikilink")、それは一意に定まりガンマ関数に他ならない（→**[ボーア・モレルップの定理](https://ja.wikipedia.org/wiki/ボーア・モレルップの定理 "wikilink")**）。右半平面においてオイラー積分で定義されたガンマ関数は全平面に[有理型に](https://ja.wikipedia.org/wiki/有理型関数 "wikilink")[解析接続](https://ja.wikipedia.org/wiki/解析接続 "wikilink")する。ガンマ関数は[零点](https://ja.wikipedia.org/wiki/零点 "wikilink")を持たず、原点と負の整数に一位の極を持つ。その[留数](https://ja.wikipedia.org/wiki/留数 "wikilink")は、  である。また、非整数でのガンマ関数の値のうちでおそらく最も有名なのは、[ガウス積分](https://ja.wikipedia.org/wiki/ガウス積分 "wikilink")になる以下の場合である。  これより、自然数  について

\[\Gamma\left(\frac{1}{2}+n\right) = \frac{(2n-1)!!}{2^{n}}\sqrt{\pi}\] が成立することがわかる。ここで  は[二重階乗](https://ja.wikipedia.org/wiki/二重階乗 "wikilink")を表す。この性質を利用して高[次元](../Page/次元.md "wikilink")[球](https://ja.wikipedia.org/wiki/球 "wikilink")の体積と表面積を求めることができる。また、
...
\[\gamma=\lim_{n\to\infty}\left(\sum_{k=1}^{n}\frac{1}{k}-\log{n}\right)\] を括り出すと**ワイエルシュトラスの乗積表示**が得られる。[ワイエルシュトラスはガンマ関数が負の整数に](../Page/カール・ワイエルシュトラス.md "wikilink")[極を持つことを嫌って逆数を用いた](https://ja.wikipedia.org/wiki/特異点_\(数学\) "wikilink")。ガンマ関数の逆数は複素平面全体で[正則](https://ja.wikipedia.org/wiki/正則 "wikilink")である。 {n^zn\!}=\\lim_{n\\to\\infty}zn^{-z}\\left(\\prod_{k=1}^{n}{e^{z/k}}\\right)\\left(\\prod_{m=1}^{n}{\\frac{z+m}{m}}e^{-z/m}\\right)=ze^{{\\gamma}z}\\prod_{m=1}^{\\infty}\\left(1+\\frac{z}{m}\\right)e^{-z/m}</math>}}

## ハンケルの積分表示

ガンマ関数は次の周回積分で表される\[4\]。積分経路は正の無限大から実軸の上側に沿って原点に至り、原点を正の向きに回り、実軸の下側に沿って無限大に戻るものとする。但し、その偏角は\(-\pi\le\arg(-t)\le\pi,0\le\arg(s)\le2\pi\)とする。  これを**ハンケルの積分表示**と呼ぶ。このハンケルの積分表示は、積分経路を適当に変形し、数値積分でガンマ関数の値を求めるために使われることがある\[5\]。
...
\(z \to \infty\)での漸近展開として、ガンマ関数は[スターリングの公式で近似される](https://ja.wikipedia.org/wiki/スターリングの近似 "wikilink")。この漸近近似は複素平面全体(負の実数を除く)で成立するが、\(|{\arg z}|={\pi}\)に近づくにつれ近似の誤差が大きくなる(極限の収束が遅くなる)ため、応用上は[相反公式などを用いて](https://ja.wikipedia.org/wiki/#相反公式 "wikilink")\(|{\arg z}|\le{\pi}/2\)程度に制限することもある。

## 相反公式

次の恒等式を**相反公式**(reflection formula)という\[6\]。 </math>}} **相補公式**とも呼ばれる。 この恒等式はオイラーの乗積表示から得られる。 \\right)\\left(\\lim_{n\\to\\infty}\\frac{n^{-z}n\!}{\\prod_{k=0}^{n}{(-z+k)}}\\right)\\\\ &=\\frac{1}{z}\\prod_{k=1}^{\\infty}\\frac{k^2}{k^2-z^2}\\\\ &=\\frac{\\pi}{{\\pi}z\\displaystyle\\prod_{k=1}^{\\infty}\\displaystyle\\frac{k^2-z^2}{k^2}}\\\\ \\end{align}</math>}} この分母は[正弦関数の無限乗積展開であるから](https://ja.wikipedia.org/wiki/三角関数の無限乗積展開 "wikilink")、 </math>}} である。相反公式に\(z=\frac{1}{2}\)を代入すれば }=\\pi</math>}} となり  を得る。
...
\((x,\ y,\ y_1,\ \ldots ,\ y_n)\)を変数とする多項式\(F(x,\ y,\ y_1,\ \ldots ,\ y_n)\)に対し、  の形で表される微分方程式を代数的微分方程式という。ガンマ関数はいかなる代数的微分方程式も満たさないことが知られている\[7\]。[ヘルダーが](https://ja.wikipedia.org/wiki/オットー・ヘルダー "wikilink")1887年に最初に証明を与えた後 \[8\]、\[9\]、\[10\] \[11\]、\[12\]、[ハウスドルフ](https://ja.wikipedia.org/wiki/フェリックス・ハウスドルフ "wikilink")\[13\]により、別証明や一般化がなされた。

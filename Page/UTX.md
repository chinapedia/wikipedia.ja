...
**UTX**（Universal Terminology eXchange）とは、AAMT（[アジア太平洋機械翻訳協会](https://ja.wikipedia.org/wiki/アジア太平洋機械翻訳協会 "wikilink")）が策定した、シンプルな用語集形式である。[翻訳ソフト](../Page/翻訳ソフト.md "wikilink")の対訳[ユーザー辞書](https://ja.wikipedia.org/wiki/ユーザー辞書 "wikilink")の標準仕様であり、人間翻訳の用語集としても使用できる。既存仕様[UPFの後継として仕様策定がされた](https://ja.wikipedia.org/wiki/UPF_\(規格\) "wikilink")\[1\]。仕様は、公開されており、無償で利用できる。仕様策定はボランティアにより行われる。

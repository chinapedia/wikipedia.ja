...
[ドン・ランカスター](../Page/ドン・ランカスター.md "wikilink") は、KIM-1 用の低価格なビデオディスプレイを開発した。追加基板でTVまたはモニターに最大4000文字を表示できる装置であった。一般的な設定は、16行×32桁表示で、大文字だけというものであった。その基板上には10個の低価格なICが使われているだけで、KIM-1 のメモリを表示用に利用していた。
...
メモリ上の1バイトはテープ上では[ASCII](../Page/ASCII.md "wikilink")文字2文字で格納される。例えば、[十六進法](../Page/十六進法.md "wikilink")でB5（十進では181）という値があるとき、テープ上には "B" と "5"（十六進では42と35）が書き込まれる\[10\]。

...
\[\Delta G(\boldsymbol{r},\boldsymbol{r'}) = -\delta(\boldsymbol{r}-\boldsymbol{r'})\] これを解くために両辺を[フーリエ変換](../Page/フーリエ変換.md "wikilink")すると、\(G(\boldsymbol{r},\boldsymbol{r'})\)のフーリエ変換\(g(\boldsymbol{k},\boldsymbol{r'})=\frac{e^{-i\boldsymbol{k \cdot r'}}}{\boldsymbol{k}^2}\)が得られる。これを[逆フーリエ変換](https://ja.wikipedia.org/wiki/逆フーリエ変換 "wikilink")するとグリーン関数\(G(\boldsymbol{r},\boldsymbol{r'})\)が求まる。

...
**引数** （ひきすう）、**パラメータ**（**ー**）（[英語](../Page/英語.md "wikilink"):parameter）は、[数学における関数や](https://ja.wikipedia.org/wiki/関数_\(数学\) "wikilink")[コンピュータプログラムにおける](../Page/プログラム_\(コンピュータ\).md "wikilink")[手続きにおいて](https://ja.wikipedia.org/wiki/プロシージャ "wikilink")、その外部と値をやりとりするための特別な変数、あるいはその変数の値のことである。
...
[関数](https://ja.wikipedia.org/wiki/サブルーチン "wikilink")・[サブルーチン](https://ja.wikipedia.org/wiki/サブルーチン "wikilink")・[メソッド等を定義する時に](https://ja.wikipedia.org/wiki/メソッド_\(計算機科学\) "wikilink")、外部から値を渡される特別な変数として指定されるのが**仮引数**。関数（等）を呼出す式において、仮引数に対応する式（あるいはその値）が**実引数**である。実行時には、実引数の値を仮引数が受け取る。
...
**仮引数**（かりひきすう、かびきすう、、）とは、[手続き(プロシージャー)で定義される](https://ja.wikipedia.org/wiki/プロシージャ "wikilink")[変数のうち](https://ja.wikipedia.org/wiki/変数_\(プログラミング\) "wikilink")、実行時に呼び出し元から渡される（実引数の）値を受けるものをいう。例として[C言語](../Page/C言語.md "wikilink")系言語における定義を挙げる:
...
2つを伴った[関数](https://ja.wikipedia.org/wiki/サブルーチン "wikilink") `sum` を定義している。定義の中で `addend1` と `addend2` が変数のように使用されていることに注目されたい。

## 実引数

**実引数**（じつひきすう、、）とは、プロシージャーを呼び出す際に渡す値のことで、プロシージャーの挙動（動作や結果）に作用する。変数や[リテラルを含む](https://ja.wikipedia.org/wiki/リテラル#コンピュータ "wikilink")[式を指定できる](https://ja.wikipedia.org/wiki/式_\(プログラミング\) "wikilink")。[C言語](../Page/C言語.md "wikilink")系言語において前に示した例中の関数 `sum` を用いた例を挙げる:
...
**値渡し**（あたいわたし、）は[右辺値](https://ja.wikipedia.org/wiki/右辺値 "wikilink")を渡す方法で、実引数として変数を渡したとしても、その値のみが渡される。もちろん即値や複雑な式を渡すこともでき、式の評価結果が渡される。その仕組みとしては、独立した新たな[変数が関数内に用意され](https://ja.wikipedia.org/wiki/変数_\(プログラミング\) "wikilink")、元の値がコピーされる。そのため変数を渡したとしても、元の変数が変更されるという事はない。
...
**参照渡し**（さんしょうわたし、）はその実装手段の一つ（と見ることもできる\[5\]）。変数に対する[参照](https://ja.wikipedia.org/wiki/参照_\(情報工学\) "wikilink")（アドレス情報）を渡す方法である（これは言語側が勝手に行う。C言語のように明示的にアドレス演算子を使うものは参照渡しとは呼ばない）。
...
C言語は値渡しのみをサポートするが、変数のポインタ（[メモリアドレス](https://ja.wikipedia.org/wiki/メモリアドレス "wikilink")）を取得することが可能であるため、変数へのポインタを値で渡す事で元の変数を変更できる。オフセット計算により配列や構造体の一部分を参照するコードも容易に記述できる。
...
## 関連項目
  ...
  - [評価戦略](https://ja.wikipedia.org/wiki/評価戦略 "wikilink")
...
3.  [Javascript練習 09](http://www.cc.kyoto-su.ac.jp/~shimizu/MAKE_HTML/js_smp11.html) [京都産業大学](https://ja.wikipedia.org/wiki/京都産業大学 "wikilink")

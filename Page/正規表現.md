...
**正規表現**（せいきひょうげん、）とは、[文字列](https://ja.wikipedia.org/wiki/文字列 "wikilink")の集合を一つの文字列で表現する方法の一つである。**正則表現**（せいそくひょうげん）とも呼ばれ、[形式言語](../Page/形式言語.md "wikilink")理論の分野では比較的こちらの訳語の方が使われる。まれに**正規式**と呼ばれることもある。

もともと正規表現は形式言語理論において[正規言語](../Page/正規言語.md "wikilink")を表すための手段として導入された。形式言語理論では、形式言語が正規言語であることと正規表現によって表せることは[同値](https://ja.wikipedia.org/wiki/同値 "wikilink")である。

その後正規表現は[テキストエディタ](https://ja.wikipedia.org/wiki/テキストエディタ "wikilink")、[ワードプロセッサ](../Page/ワードプロセッサ.md "wikilink")などの[アプリケーションで](https://ja.wikipedia.org/wiki/アプリケーションソフトウェア "wikilink")（ないし、そもそもそれ以前に単機能の[文字列探索](https://ja.wikipedia.org/wiki/文字列探索 "wikilink")ツールの）、マッチさせるべき対象を表すために使用されるようになり、表せるパターンの種類を増やすために本来の正規表現にはないさまざまな記法が新たに付け加えられた。このような拡張された正規表現には正規言語ではない文字列も表せるものも多く、ゆえに正規表現という名前は実態に即していない面もあるが、伝統的に正規表現と呼ばれ続けている。

この記事では主にこのような正規表現を用いた[パターンマッチング](https://ja.wikipedia.org/wiki/パターンマッチング "wikilink")について説明している。以下、誤解のない限り、アプリケーションや[プログラミングにおいて正規表現を用いた文字列のパターンマッチングを行う機能のことを](https://ja.wikipedia.org/wiki/プログラミング_\(コンピュータ\) "wikilink")、単に正規表現という。

ほとんどの[プログラミング言語](../Page/プログラミング言語.md "wikilink")では、[ライブラリ](../Page/ライブラリ.md "wikilink")によって正規表現を使うことができる他、一部の言語では正規表現の[リテラル](https://ja.wikipedia.org/wiki/リテラル "wikilink")もある。「正規表現によるマッチ」を意味する（専用の）演算子がある言語なども一部ある。具体例として、[grep](https://ja.wikipedia.org/wiki/grep "wikilink")、[AWK](../Page/AWK.md "wikilink")、[sed](https://ja.wikipedia.org/wiki/Sed_\(コンピュータ\) "wikilink")、[Perl](../Page/Perl.md "wikilink")、[Tcl](https://ja.wikipedia.org/wiki/Tcl/Tk "wikilink")、[lex](https://ja.wikipedia.org/wiki/lex "wikilink")などがある。

それぞれの言語やアプリケーションで細部の仕様が異なっている、といったように思われることも多いが（また、古い実装では実際にそういうことも多いが）、近年は同じライブラリを使っていれば同じということも多い。また[POSIX](https://ja.wikipedia.org/wiki/POSIX "wikilink")など標準もある。
...
例えば、「`Handel`」、「`H`**`e`**`ndel`」、「`Ha`**`e`**`ndel`」 という三つの文字列を含む集合は 「`H(e|ae?)ndel`」 というパターンで表現できる（あるいは、パターンは個々の三つの文字列に**マッチする**と言われる）。ほとんどの形式では、もし特定の集合にマッチする何らかの正規表現が存在すれば、無限の数のそのような表現がある。ほとんどの形式では正規表現を構築するのに次の[演算子](https://ja.wikipedia.org/wiki/演算子 "wikilink")を提供している。

  - 選言
    縦棒は選択肢を区切る。例えば「`gray|grey`」は「`gray`」または「`grey`」にマッチし、これは普通 「`gr(a|e)y`」 に短縮される。
  - グループ分け
    丸括弧はスコープと[演算子](https://ja.wikipedia.org/wiki/演算子 "wikilink")の優先順位を定義するのに用いられる。例えば、「`gr(a|e)y`」 では 「`(a|e)`」 の部分で 「`a`」 または 「`e`」 を示し、全体で 「`gray`」 または 「`grey`」 にマッチする。
  - 量化
    文字やグループの後ろの量化子は、直前の表現が何回現れることが許されるかを指定する。非常によく使われる量化子は 「`?`」 と 「`*`」 と 「`+`」 である。
      - `?`
        [疑問符](https://ja.wikipedia.org/wiki/疑問符 "wikilink")は直前の表現が<u>0個か1個ある</u>ことを示す。例えば、「`colou?r`」 は　「`color`」 と 「`colour`」 にマッチする。
      - `*`
        [アスタリスク](https://ja.wikipedia.org/wiki/アスタリスク "wikilink")は直前の表現が<u>0個以上ある</u>ことを示す。例えば、「`go*gle`」 は 「`ggle`」、「`gogle`」、「`google`」 などにマッチする。
      - `+`
...
正規表現の起源は、[言語学](../Page/言語学.md "wikilink")と、[理論計算機科学](https://ja.wikipedia.org/wiki/理論計算機科学 "wikilink")の一分野である[オートマトン](https://ja.wikipedia.org/wiki/オートマトン "wikilink")理論や[形式言語](../Page/形式言語.md "wikilink")理論にみることができる。20世紀の言語学では数理的に言語を扱う数理言語学が発展しその過程の一部として、また後者は計算のモデル化（オートマトン）や形式言語の分類方法などを扱う学術分野である。数学者の[スティーヴン・クリーネは](https://ja.wikipedia.org/wiki/スティーヴン・コール・クリーネ "wikilink")1950年代に[正規集合](https://ja.wikipedia.org/wiki/正規集合 "wikilink")と呼ばれる独自の数学的表記法を用い、これらの分野のモデルを記述した。

Unix系のツールに広まったのは、[ケン・トンプソン](https://ja.wikipedia.org/wiki/ケン・トンプソン "wikilink")がテキストファイル中のパターンにマッチさせる手段として、この表記法をエディタ[QEDに導入したことなどに始まる](https://ja.wikipedia.org/wiki/QED_\(テキストエディタ\) "wikilink")。彼はこの機能をのエディタにも追加し、後に一般的な検索ツールである[grep](https://ja.wikipedia.org/wiki/grep "wikilink")の正規表現へと受け継がれていった。これ以降、トンプソンの正規表現の適用にならい、多くのUnix系のツールがこの方法を採用した（例えば 、、、、、 など）。

との正規表現はヘンリー・スペンサーによって書かれたものから派生している（Perlは後にスペンサーの正規表現を拡張し、多くの機能を追加した）。フィリップ・ヘーゼルはの正規表現とほぼ互換のものを実装する試みとして (PCRE) を開発した。これは[PHPや](https://ja.wikipedia.org/wiki/PHP_\(プログラミング言語\) "wikilink")[{{langなどといった新しいツールで使用されている](https://ja.wikipedia.org/wiki/Apache_HTTP_Server "wikilink")。
...
[UNIX](../Page/UNIX.md "wikilink")の標準である[POSIX](https://ja.wikipedia.org/wiki/POSIX "wikilink")では、**単純正規表現**、**基本正規表現**、**拡張正規表現**の3種類の記法が示されている。このうち、単純正規表現は「歴史的\[1\]」また「[レガシー](https://ja.wikipedia.org/wiki/レガシーシステム "wikilink")\[2\]」と書かれており、後方互換性を提供するものとされ、標準の将来の版では廃止され得る\[3\]と注意されている。
...
古いバージョンの grepは選言[演算子](https://ja.wikipedia.org/wiki/演算子 "wikilink")「`\|`」をサポートしていない。
...
[符号点](https://ja.wikipedia.org/wiki/符号点 "wikilink")の範囲によってたとえば「アルファベット大文字」などを表現しようとすることは、時に問題をひきおこす。たとえばロケールに依存する例として、[エストニア語](https://ja.wikipedia.org/wiki/エストニア語 "wikilink")のアルファベット順では、文字「s」の後に「z」があり、その後は「t」、「u」、「v」、「w」、「x」、「y」と続くので、正規表現「`[a-z]`」ではすべての言語のすべてのアルファベット小文字にマッチするわけではない<ref>これは正規表現として「`[a-z]`」を使用していたことが原因である。
...
| `[[:space:\|:space:]]`   | `[ \t\n\r\f\v]`           | （半角の）空白文字                                             |
| `[[:cntrl:\|:cntrl:]]`   |                           | [制御文字](https://ja.wikipedia.org/wiki/制御文字 "wikilink") |
| `[[:graph:\|:graph:]]`   | `[^ \t\n\r\f\v[:cntrl:]]` | 印字文字                                                  |
| `[[:print:\|:print:]]`   | `[^\t\n\r\f\v[:cntrl:]]`  | 印字文字とスペース                                             |

例:正規表現「`[[:upper:]ab]`」は英語の大文字「`A`」～「`Z`」と「`a`」と「`b`」のうち一文字のみにマッチする。

いくつかのツールで使用できる、 にないクラスとして「`[:word:]`」がある。「`[:word:]`」は通常「`[:alnum:]`」と[アンダースコア](https://ja.wikipedia.org/wiki/アンダースコア "wikilink")からなる。これらが多くの[プログラミング言語](../Page/プログラミング言語.md "wikilink")で[識別子](https://ja.wikipedia.org/wiki/識別子 "wikilink")として使用できる文字であることを反映している。
...
すぐれた機能をもつPerlの拡張正規表現は、多くの[プログラミング言語](../Page/プログラミング言語.md "wikilink")やソフトウェアで採りいれられている。例えば、[Java](https://ja.wikipedia.org/wiki/Java "wikilink")のPattern[クラス](https://ja.wikipedia.org/wiki/クラス_\(コンピュータ\) "wikilink")、[Python](../Page/Python.md "wikilink")、[Rubyなどがそうである](https://ja.wikipedia.org/wiki/Ruby_\(代表的なトピック\) "wikilink")。しかし、これらがPerlの正規表現と完全に互換である訳ではない。また、[Perl Compatible Regular Expressions](https://ja.wikipedia.org/wiki/Perl_Compatible_Regular_Expressions "wikilink") (PCRE) と呼ばれる汎用の正規表現ライブラリはアプリケーションに組み込まれ、Perlの正規表現とほぼ互換の機能を提供する。
...
言語処理系やアプリケーションが正規表現をサポートしていない場合にでも、正規表現に必要な処理を提供する正規表現ライブラリを導入することで正規表現を使うことができる。以下にその一例を挙げておく。
  ...
  - [PCRE](https://ja.wikipedia.org/wiki/PCRE "wikilink")
    Perl互換のライブラリ。[Exim](https://ja.wikipedia.org/wiki/Exim "wikilink")のために開発され、[Apacheや](https://ja.wikipedia.org/wiki/Apache_HTTP_Server "wikilink")[Postfix](https://ja.wikipedia.org/wiki/Postfix "wikilink")をはじめ、さまざまなソフトウェアに組み込まれている\[10\]。
  -
    正規表現オブジェクトごとに異なる文字エンコーディングを指定できる特徴をもつ。Rubyの1.9系列や[PHPの](https://ja.wikipedia.org/wiki/PHP_\(プログラミング言語\) "wikilink")5系列に採用されている。[macOS](https://ja.wikipedia.org/wiki/macOS "wikilink")用に検索ウィンドウを追加して移植したOgreKit（Oniguruma Regular Expression Framework for Cocoa）が存在する\[11\]。
  - [鬼雲](https://ja.wikipedia.org/wiki/鬼雲 "wikilink")
    鬼車から派生した正規表現ライブラリで、Perl 5.10以降で導入された機能をサポートする。Ruby 2.0以降の標準ライブラリとして利用されている\[12\]。
  -
    PCREなどのバックトラック式ではなく[オートマトン](https://ja.wikipedia.org/wiki/オートマトン "wikilink")を用いることで、省メモリでマッチングを行うことができる。[Google](https://ja.wikipedia.org/wiki/Google "wikilink")が内部で利用している。
  - GNU Regex
    [GNU Cライブラリに含まれているため](https://ja.wikipedia.org/wiki/GNU_Cライブラリ "wikilink")、[Unix系](https://ja.wikipedia.org/wiki/Unix系 "wikilink")では標準で利用できる。
  - Boost.Regex
    [Boost C++ライブラリによる実装](https://ja.wikipedia.org/wiki/Boost "wikilink")。[C++11](https://ja.wikipedia.org/wiki/C++11 "wikilink")規格にて標準化された。POSIXや[ECMAScript](https://ja.wikipedia.org/wiki/ECMAScript "wikilink")などの文法プロファイルを任意に選択することができる\[13\] \[14\]。

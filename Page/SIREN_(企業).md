> この記事は[SIREN \(企業\)](https://ja.wikipedia.org/wiki/SIREN_\(企業\))から翻訳されています。


**SIREN**（さいれん）とは、アメリカ西海岸で設立された会社で、[デジタルオーディオプレーヤー](../Page/デジタルオーディオプレーヤー.md "wikilink")及び[GPS](../Page/グローバル・ポジショニング・システム.md "wikilink")、デジタルフォトフレーム等を販売しているブランドの名称である。

## 概要

2004年12月設立した会社で、かつて[Rioに在籍していたメンバーが中心となって設立した会社である](../Page/Rio_\(オーディオ\).md "wikilink")。設立当初は、GoVideo社のRave-MPシリーズやiloのMP3プレイヤーの開発、製造を行っていたが、2006年、自社ブランドで展開するようになった。日本では、2006年6月にA-MAX Technology Japan社の[SIGNEO](../Page/SIGNEO.md "wikilink")ブランドの後釜として設立された。このため、本国の製品は殆ど発売されていない。2007年5月25日には[iriver japanに営業権を譲渡し日本法人は解散となった](../Page/アイリバー.md "wikilink")。

## 日本での主な製品一覧

  - MP3プレイヤー

| 商品名     | 容量            | OEM先の製品名 | 開発(OEM先)         |
| ------- | ------------- | -------- | ---------------- |
| DP100   | 512MB,1GB,2GB | \-       | A-MAX Technology |
| DP150   | 1GB,2GB       | \-       | A-MAX Technology |
| DP200   | 512MB,1GB,2GB | M-20     | M-CODY           |
| DP250   | 1GB,2GB       | \-       | A-MAX Technology |
| DP300   | 1GB,2GB       | MX-700   | M-CODY           |
| DP350   | 2GB,4GB       | \-       | A-MAX Technology |
| DP10T   | 1GB,2GB       | \-       | A-MAX Technology |
| SIREN V | 4GB           | \-       | SIREN USA        |

  - ポータブルワンセグTV

| 商品名    | OEM先の製品名 | 開発(OEM先)         |
| ------ | -------- | ---------------- |
| DTV100 | \-       | A-MAX Technology |

  - デジタルフォトフレーム

| 商品名   | OEM先の製品名 | 開発(OEM先)         |
| ----- | -------- | ---------------- |
| DF70  | \-       | A-MAX Technology |
| DF100 | \-       | A-MAX Technology |
| DF150 | \-       | A-MAX Technology |

  - カーナビ

| 商品名   | OEM先の製品名 | 開発(OEM先)         |
| ----- | -------- | ---------------- |
| PN100 | \-       | A-MAX Technology |

  - USBメモリ

| 商品名   | 容量                  | OEM先の製品名 | 開発(OEM先)         |
| ----- | ------------------- | -------- | ---------------- |
| UM100 | 256MB,512MB,1GB,2GB | \-       | A-MAX Technology |

## 関連項目

  - [Rio](../Page/Rio_\(オーディオ\).md "wikilink")
  - [SIGNEO](../Page/SIGNEO.md "wikilink")
  - [iriver](../Page/アイリバー.md "wikilink")

## 外部リンク

  - [サイレン(日本法人)](http://www.sirenjp.com/index.html)
  - [SIREN(米国法人)](http://www.sirenplayers.com/)

[Category:携帯型プレーヤー](https://ja.wikipedia.org/wiki/Category:携帯型プレーヤー "wikilink")
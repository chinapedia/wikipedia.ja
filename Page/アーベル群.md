...
[数学](../Page/数学.md "wikilink")、とくに[抽象代数学](https://ja.wikipedia.org/wiki/抽象代数学 "wikilink")における**アーベル群**（アーベルぐん、）または**可換群**（かかんぐん、）は、[群演算が](https://ja.wikipedia.org/wiki/二項演算 "wikilink")[可換な](https://ja.wikipedia.org/wiki/交換法則 "wikilink")[群](https://ja.wikipedia.org/wiki/群_\(数学\) "wikilink")、すなわちどの二つの元の積も掛ける順番に依らず定まる群を言う。名称は、ノルウェーの数学者[ニールス・アーベル](https://ja.wikipedia.org/wiki/ニールス・アーベル "wikilink")に因む。
...
[集合](https://ja.wikipedia.org/wiki/集合 "wikilink") *G* に[二項演算](https://ja.wikipedia.org/wiki/二項演算 "wikilink")（"\*" と書くことにする）が定義されていて、以下の条件

1.  [結合法則](https://ja.wikipedia.org/wiki/結合法則 "wikilink"): \(a * (b * c) = (a * b) * c\).
...
4.  [交換法則](https://ja.wikipedia.org/wiki/交換法則 "wikilink"): \(a * b = b * a\).

（ただし、*a*, *b*, *c* は *G* の任意の元）を全て満たすとき、*G* と演算 "\*" の組 (*G*, \*) を**アーベル群**という。考えている演算があきらかなときは省略して単に *G* をアーベル群と呼ぶ。

アーベル群ではしばしば演算子を "+" と記す。このとき単位元を[零元](https://ja.wikipedia.org/wiki/零元 "wikilink")と呼んで 0 などで表し、逆元も −*a* のように[負符号を用いて表して](https://ja.wikipedia.org/wiki/プラス記号とマイナス記号 "wikilink")**マイナス元**あるいは**反数**ともよぶ。また、*a* + (−*b*) は *a* − *b* と書かれ、*a* から *b* を引くという[減法](https://ja.wikipedia.org/wiki/減法 "wikilink")が定義される。このような記法を**加法的な記法**と呼び、対して先に述べたような通常の群でよく使われる記法を**乗法的な記法**ということがある。アーベル群の定義を加法的に記せば
...
## 例
  ...
  - [整数](../Page/整数.md "wikilink")の全体 **Z**、[有理数](https://ja.wikipedia.org/wiki/有理数 "wikilink")の全体 **Q**、[実数](../Page/実数.md "wikilink")の全体 **R**、[複素数](../Page/複素数.md "wikilink")の全体 **C** は全て通常の[加法](https://ja.wikipedia.org/wiki/加法 "wikilink")に関してアーベル群である。一方 [自然数](../Page/自然数.md "wikilink")の全体 **N** は加法（の逆演算としての[減法](https://ja.wikipedia.org/wiki/減法 "wikilink")）に関して閉じていないのでアーベル群ではない。
  ...
  - [楕円曲線](https://ja.wikipedia.org/wiki/楕円曲線 "wikilink") *y*<sup>2</sup> = *x*<sup>3</sup> + *ax* + *b* の解集合には、加法を定義することができ、アーベル群になる。
...
素数位数の任意の群は巡回群に同型であり、ゆえにアーベル群である。また、位数が素数の平方であるような任意の群はアーベル群となる\[2\]。実は任意の素数  に対して位数  の群は、[同型を除いて](https://ja.wikipedia.org/wiki/同型を除いて "wikilink")  と  のちょうど二種類しかない。
  ...
  - 有限アーベル群の基本定理
    任意の有限アーベル群  は[素冪位数の巡回群の](https://ja.wikipedia.org/wiki/素数 "wikilink")[直和に表される](https://ja.wikipedia.org/wiki/群の直和 "wikilink")。

これは[有限生成アーベル群の基本定理](https://ja.wikipedia.org/wiki/有限生成アーベル群の基本定理 "wikilink")の特別の場合（階数  の場合）である。位数  の巡回群  が  と  の直和に同型となるための必要十分条件は  と  が[互いに素](https://ja.wikipedia.org/wiki/互いに素 "wikilink")となることである（[中国の剰余定理](https://ja.wikipedia.org/wiki/中国の剰余定理 "wikilink")）。これにより任意の有限アーベル群  が

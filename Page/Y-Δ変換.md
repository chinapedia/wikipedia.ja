...
[グラフ理論](../Page/グラフ理論.md "wikilink")ではY-Δ変換は、グラフ中のYのサブグラフを同等のΔのサブグラフに置き換えることである。この変換はグラフのエッジの数を増加させることは無いが、グラフのノードの数を増加させる。2つのグラフは、Y-Δ変換と逆変換を行い、もとに戻るのであるなら、Y-Δ等価であるという。例えば、[ピーターセングラフ](https://ja.wikipedia.org/wiki/ピーターセングラフ "wikilink")はY-Δ等価なクラスである。

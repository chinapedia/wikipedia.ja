...
**ホスト名**（ホストめい）あるいは**サイト名**（サイトめい）とは、コンピュータ、ファイルサーバ、ネットワークストレージデバイス、FAX、コピー機、ケーブルモデム、ルータなど[ネットワークに接続された機器](https://ja.wikipedia.org/wiki/コンピュータネットワーク "wikilink")（[ホスト](https://ja.wikipedia.org/wiki/ホスト_\(ネットワーク\) "wikilink")）に付けられた名前。ホスト名はWorld Wide Web、電子メールあるいはUsenetのようなさまざまな電子通信の形式において特定のホストを識別するために使われる。
...
[インターネット](https://ja.wikipedia.org/wiki/インターネット "wikilink")では、ホスト名はホストに割り当てられる[ドメイン名](../Page/ドメイン名.md "wikilink")の一種である。 これは、通常、親ドメイン名とホストのローカル名との組み合わせで表現される。

> この記事は[ParaParaParadise](https://ja.wikipedia.org/wiki/ParaParaParadise)から翻訳されています。
...
前のモニターには譜面などの、ゲームに直接関係のある表示がされる。画面の下には選択用の左右ボタンと決定用のスタートボタンがある。この横に[パラパラオールスターズ](https://ja.wikipedia.org/wiki/パラパラオールスターズ "wikilink")による振り付けが再生されるサブモニター(v1.1まではダンサーがパラパラオールスターズでない曲があったが、1stMIX Plusの時にそれらの映像は全て再録された)があることが多い。
...
2.  ゲームモードを選ぶ。
      -
        [ツインスター](https://ja.wikipedia.org/wiki/ツインスター "wikilink")公式振り付けで踊るパラパラモードと、コナミオリジナルの譜面のフリースタイルモードから選択できる。
...
4.  プレイする。
      -
        画面下方から矢印マークが出てくるので、タイミングを合わせて矢印マークが指示する方向のセンサーに手をかざす。線のつながった矢印マークの場合はその2点の間手をかざした状態を保たなければならない。矢印マークの流れる後ろには「[DanceDanceRevolution](https://ja.wikipedia.org/wiki/DanceDanceRevolution "wikilink")」のように3Dモデルのキャラクターが踊っており、パラパラモードについてはこの画面上のキャラクターか、サブモニターで流される[パラパラオールスターズ](https://ja.wikipedia.org/wiki/パラパラオールスターズ "wikilink")の映像をお手本にプレイすることも可能。キャラクター及びパラパラオールスターズのメンバーは楽曲によって変化する。
5.  結果表示
      -

...
<!-- end list -->
  ...
  - Painter 2021
    写真などのソースをベースとして、様々なスタイルの人工知能（AI）が自動でアートに変換してくれる「人工知能スタイル」機能が搭載された。AIでも手動でも簡単に写真をアートに変換できる「フォトアートクイックパネル」を搭載。Apple製品との連携が強化され、[iPad](https://ja.wikipedia.org/wiki/iPad "wikilink")をMacのサブディスプレイとして使用するApple Sidecar、Apple Pencilの傾き検知、MacBook Proに搭載されているTouchbarおよびTrackpadのマルチタッチ機能、Appleの提供する機械学習フレームワークであるCore MLなどに対応。クローンを作成して元の画像と比較しながら着色できる「クローン着色」機能が搭載され、「クローンティント」のブラシカテゴリが新設された。厚塗りペイントのワークフローが改善され、厚塗りレイヤーおよび水彩レイヤーのコントロールが強化された。他は、厚塗り系ブラシ16本の搭載、溶かし＆液体ブラシ技術の機能拡張、ブラシとツールの切り替えの高速化などの機能強化がなされた。

<!-- end list -->

  -
    なお、このバージョンよりPainterに[サブスクリプション](https://ja.wikipedia.org/wiki/サブスクリプション "wikilink")が導入された。

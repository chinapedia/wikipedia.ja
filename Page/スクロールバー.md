...
**スクロールバー**（）は、主に[ウィンドウシステム](../Page/ウィンドウシステム.md "wikilink")において単一の[ウインドウ](https://ja.wikipedia.org/wiki/ウインドウ "wikilink")内で収まりきらない情報の部分領域だけ表示し、必要に応じて表示領域を移動するための[GUIパーツ](https://ja.wikipedia.org/wiki/グラフィカルユーザインタフェース "wikilink")（[ウィジェット](https://ja.wikipedia.org/wiki/ウィジェット_\(GUI\) "wikilink")）のことをいう。縦（垂直）方向と横（水平）方向の二種類があり、前者を単にスクロールバー（或いは垂直スクロールバー）、後者を横スクロールバー（水平スクロールバー）などと呼ぶ。なおスクロールバーはユーザ側からみた呼び名で、[プログラム上ではスクローラー](../Page/プログラム_\(コンピュータ\).md "wikilink")（）と呼ばれる場合がある\[1\]。
...
スクロールバーは複数のパーツからなる複合[コンポーネント](https://ja.wikipedia.org/wiki/コンポーネント "wikilink")である。
  ...
  - [矢印](../Page/矢印.md "wikilink")（アロー）
    上下（左右）の矢印で、[クリック](../Page/クリック.md "wikilink")するとその方向にノブが1単位移動する。移動量はおおむねテキスト1行である。多くはボタンを押し続けると連続移動を行なう（リピートボタン）。アローの配置には2つの流儀があり、スクロールバーの両端に別れて配置される場合と、場合に分かれる。前者の配置は「ノブを上下左右に移動させたい場合は上下左右それぞれの端にあるアローを操作する」という表現となっており、直感的で自然であるが、後者の集中配置型のほうがアロー間の距離が近くなるため、マウス移動の効率は良い。
  - 本体
...
スクロールバー本体はウインドウ内の右と下にそれぞれ垂直、水平のバーが置かれるケースが一般的である。この配置は[Xerox Starで最初に採用され](../Page/Xerox_Star.md "wikilink")\[9\]、。対象となるウィンドウに対して常にスクロールバーが表示されるモードだけでなく、コンテンツのサイズがウィンドウのサイズを超えた場合にのみスクロールバーが自動で表示されるモードもある\[10\]。

他にも次のような配置や挙動のパターンがある。

#### 垂直スクロールバーを左側に置くパターン

本記事が対象とするウィジェットとしてのスクロールバー\[11\]自体は[アラン・ケイ](../Page/アラン・ケイ.md "wikilink")らが開発した[暫定ダイナブック環境](https://ja.wikipedia.org/wiki/ダイナブック#暫定ダイナブック "wikilink")（[Alto](../Page/Alto.md "wikilink")で動作する[Smalltalk](../Page/Smalltalk.md "wikilink")）においてウィジェットとしての“ペイン”（ウインドウ表示域を分割する区画）に付属するものとして初めて考案・実装されたものである\[12\](1976)が、この時点では左側に配置される（ただし後述のとおりウインドウやペインの表示域外に描かれる ）方式が採用された\[13\]。

左側に配置することのメリットは、グラフィカルインターフェイス上の表現では重要な機能が左側に集中する事が多く、ポインタや視線の移動効率が良い点が挙げられる\[14\]。Smalltalk-76\[15\](1976)、-80\[16\](1981)等の古典的Smalltalk処理系のGUIの他にも初期のLispマシン（[Symbolics](https://ja.wikipedia.org/wiki/Symbolics "wikilink") [Genera OS](https://ja.wikipedia.org/wiki/:en:Genera_\(operating_system\) "wikilink")(1982)、[Xerox](https://ja.wikipedia.org/wiki/Xerox "wikilink") [Interlisp](../Page/Interlisp.md "wikilink")-D\[17\](1983)）やカーネギーメロン大学の[Andrewシステム](https://ja.wikipedia.org/wiki/:en:Andrew_Project "wikilink")\[18\](1982)、Unix初のグラフィックターミナル（[Blit](../Page/Blit.md "wikilink")(1982)）、[NeXTSTEP](https://ja.wikipedia.org/wiki/:en:NeXTSTEP "wikilink")(1988)のGUI等においてスクロールバーの左配置が採用されている。
...
ウェブブラウザの中には、[CSS](../Page/CSS.md "wikilink")によってスクロールバーの色や形状を変更することができるものが存在し、これを用いてウェブサイト側で独自にスクロールバーのデザインを設定している場合がある。
...
マウスによっては中央にホイール（回転する円盤）を配したものがあり（ホイールマウス）、1996年に発売されたのが最初で\[19\]、2015年時点ではホイールマウスが主流になっている\[20\]。ホイールを上下に回すことでページをスクロールできるほか、ホイールを中ボタンとしてクリックすることで、アプリケーションによってはオートスクロールモードに切り替わるものもある\[21\]。
...
9.
10. [WPFのSystem](../Page/Windows_Presentation_Foundation.md "wikilink").Windows.Controls.ScrollBarVisibility.Autoなど。
11. 「スクロールバー」と称した機構はAltoで動作する初の[WYSIWYG](../Page/WYSIWYG.md "wikilink")[ワープロ](https://ja.wikipedia.org/wiki/ワープロ "wikilink")アプリとして知られるBravo(1974)にすでに存在したが、これはタブなどのギミックを有さず、単にマウスクリックによるスクロール操作（右クリックで下方向、左で上方向、中ボタンでジャンプ）を行なうための画面上の矩形区画に過ぎなかった。
12.
13.
14. ただし、Smalltalkの外付けスクロールバーに限っては、右側に配置すると右に隣接するペインの主要な情報を隠してしまうため左側に配置せざるをえなかったという制約がまず存在した。
15.
16.
17.
18.
19.
20. [価格.com - マウスの選び方](http://kakaku.com/pc/mouse/guide_0160/)
21. [オート・スクロール・モードが利用できない場合の対処方法 － ＠IT](http://www.atmarkit.co.jp/fwin2k/win2ktips/759autoscroll/autoscroll.html)
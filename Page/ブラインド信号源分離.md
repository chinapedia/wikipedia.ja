...
**ブラインド信号源分離**（ブラインドしんごうげんぶんり、blind source separation、**BSS**）は、複数の未知の信号系列を未知の[線形混合系で混合した複数の測定値系列から](https://ja.wikipedia.org/wiki/線型性 "wikilink")、それぞれの信号を分離することである。
...
で表される。ここで、\(\mathbf{x}\) はある時刻の複数の信号、\(\mathbf{y}\) はある時刻の複数の測定値を、それぞれ列[ベクトル](https://ja.wikipedia.org/wiki/ベクトル "wikilink")で表したもので、[添数](https://ja.wikipedia.org/wiki/添数 "wikilink") \(t\) は[離散](https://ja.wikipedia.org/wiki/離散 "wikilink")化された[時刻](../Page/時刻.md "wikilink")を表す。\(M\) は混合系を表す[行列](https://ja.wikipedia.org/wiki/行列 "wikilink")で、混合行列と呼ばれる。時間に対しては変化しない。
...
一般のBSSは自由度が高すぎ、一般的な解法はない。しかし、いくつかの条件を仮定すれば、次のような手法で解くことができる。
  ...
  - [主成分分析](https://ja.wikipedia.org/wiki/主成分分析 "wikilink") (PCA)

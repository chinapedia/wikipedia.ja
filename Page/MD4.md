> この記事は[MD4](https://ja.wikipedia.org/wiki/MD4)から翻訳されています。


**MD4**は、1990年に[マサチューセッツ工科大学](../Page/マサチューセッツ工科大学.md "wikilink")の[ロナルド・リベスト](../Page/ロナルド・リベスト.md "wikilink")によって開発された[ハッシュ関数](../Page/ハッシュ関数.md "wikilink")の一種。[暗号ハッシュ関数](https://ja.wikipedia.org/wiki/暗号ハッシュ関数 "wikilink")を実装する。[SHA](../Page/Secure_Hash_Algorithm.md "wikilink")・[RIPEMD](https://ja.wikipedia.org/wiki/RIPEMD "wikilink")などに影響を及ぼした。

[MD4.svg](https://ja.wikipedia.org/wiki/File:MD4.svg "fig:MD4.svg")

与えられた入力に対して、[128ビット](https://ja.wikipedia.org/wiki/128ビット "wikilink")の[ハッシュ](https://ja.wikipedia.org/wiki/ハッシュ "wikilink")値を出力する。

MD4に対する[脆弱性](../Page/脆弱性.md "wikilink")は[1991年](../Page/1991年.md "wikilink")に実証された。[2004年](../Page/2004年.md "wikilink")には、MD4における[ハッシュ衝突](https://ja.wikipedia.org/wiki/ハッシュ衝突 "wikilink")を作成することが可能であることが報告された。

## 関連項目

  - [ハッシュ関数](../Page/ハッシュ関数.md "wikilink")
  - [MD2](../Page/MD2.md "wikilink")
  - [MD5](../Page/MD5.md "wikilink")
  - [Secure Hash Algorithm](../Page/Secure_Hash_Algorithm.md "wikilink")

## 脚注

<references/>

## 外部リンク

  - RFC 1320
  - [Collisions for Hash Functions MD4, MD5, HAVAL-128 and RIPEMD](http://eprint.iacr.org/2004/199) MD4その他においてハッシュ衝突を作成することが可能であることを主張している論文。
  - [MD2, MD4, MD5 Online Calculator](http://www.webutils.pl/MD5_Calculator) Calculate file hashes using an on-line web form.

[Category:ハッシュ関数](https://ja.wikipedia.org/wiki/Category:ハッシュ関数 "wikilink") [Category:RFC](https://ja.wikipedia.org/wiki/Category:RFC "wikilink")
...
母集団を完全に調査するのは不可能な場合、標本から母集団の特性を推定する必要がある。この[標本抽出](https://ja.wikipedia.org/wiki/標本抽出 "wikilink")には[作為抽出](https://ja.wikipedia.org/wiki/作為抽出 "wikilink")法と[無作為抽出](https://ja.wikipedia.org/wiki/無作為抽出 "wikilink")法の２つの抽出方法がある。統計学では[無作為抽出](https://ja.wikipedia.org/wiki/無作為抽出 "wikilink")法だけを議論するわけではない。無作為抽出法では、母集団のどの要素も等しい確率で標本に選ぶことが必要である。しかし、実際に母集団がわかっていない状態で、抽出した方法が無作為であることを保証できない。そのため、理想的には無作為抽出を仮定するものの、抽出方法の妥当性を確認する必要があり、統計学の対象を無作為抽出に限定することは不可能である。
...
母集団を表現する数値を**[母数](../Page/母数.md "wikilink")**というのに対し、標本を表現する数値を**[統計量](https://ja.wikipedia.org/wiki/統計量 "wikilink")**という。統計量は標本から算出できる数値である。また統計量で特に母数を推定するために用いるものを[推定量](../Page/推定量.md "wikilink")という。

## 形式的定義

母集団分布を *F* とするとき、母集団から抽出された（**無作為**）**標本**  とは分布 *F* に従う[独立同分布](https://ja.wikipedia.org/wiki/独立同分布 "wikilink")[確率変数](https://ja.wikipedia.org/wiki/確率変数 "wikilink")列 *x*<sub>1</sub>, *x*<sub>2</sub>, ..., *x*<sub>*n*</sub> のことである。この確率変数列の長さ*n* を標本のサイズ（サンプルサイズ）という。とりうる標本の全体が成す集合 Ω, 確率を定めうる集合の全体 **M** (⊂ 2^Ω), 分布を表す確率測度 *P* からなる[確率空間](../Page/確率空間.md "wikilink") (Ω, **M**, *P*) を**標本空間**という。
...
標本から適当な操作を行って新たに作り出される確率変数を[統計量](https://ja.wikipedia.org/wiki/統計量 "wikilink")と呼ぶ。統計量は（同じ量でも）標本の採り方に依存して定まり、一般に母集団の分布とは異なる分布に従う。統計量の従う分布を[標本分布](https://ja.wikipedia.org/wiki/標本分布 "wikilink")と呼ぶ。

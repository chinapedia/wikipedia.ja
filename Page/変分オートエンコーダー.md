...
本章で想定しているセッティングにおいて、既知の手法に対する本手法の優位性は以下の通りである<sup>[\#原論文](https://ja.wikipedia.org/wiki/#原論文 "wikilink")</sup>：

  - [最尤法](https://ja.wikipedia.org/wiki/最尤法 "wikilink")は本章のセッティングでは\(p_{\theta}(\mathbf{x})=\int p_{\theta}(\mathbf{x}|\mathbf{z})p_{\theta}(\mathbf{z})\mathrm{d}\mathbf{z}\)が容易に計算できるケースでないと使えないが、本手法はそうでないケースにも使える
  - [EMアルゴリズム](https://ja.wikipedia.org/wiki/EMアルゴリズム "wikilink")は\(p_{\theta}(\mathbf{z}|\mathbf{x})\)の計算が容易であるケースでないと使えないが、本手法はそうでないケースにも使える。
  - [変分ベイズ法](https://ja.wikipedia.org/wiki/変分ベイズ法 "wikilink")は\(p_{\theta}(\mathbf{z}|\mathbf{x})\)が[平均場近似](../Page/平均場近似.md "wikilink")できる場合にしか使えないが、本手法はそうでないケースにも使える。
  - [モンテカルロEMアルゴリズム](https://ja.wikipedia.org/wiki/モンテカルロEMアルゴリズム "wikilink")は実行速度が遅い為大きなデータ集合に対しては使えないが本手法は大きなデータ集合に対しても使える。

...
**リテラル**・**直値**\[1\]\[2\]\[3\]（）は、「文字どおり」「字義どおり」を意味する語で、 と同じく[ラテン語](../Page/ラテン語.md "wikilink")の （[文字](../Page/文字.md "wikilink")）に由来する。[数理論理学](../Page/数理論理学.md "wikilink")とコンピュータプログラミング\[4\]で異なる意味の[専門用語](https://ja.wikipedia.org/wiki/専門用語 "wikilink")として使われる\[5\]。
...
[コンピュータ](../Page/コンピュータ.md "wikilink")[プログラミング言語](../Page/プログラミング言語.md "wikilink")においてリテラルは、[ソースコード](../Page/ソースコード.md "wikilink")内に[値を直接表記したものをいう](../Page/値_\(情報工学\).md "wikilink")。言語によってリテラルとして表記できる[型の種類や表記方法は異なる](https://ja.wikipedia.org/wiki/データ型 "wikilink")。簡略に記述できることが好まれる[スクリプティング言語はリテラルの種類が多い傾向にある](../Page/スクリプト言語.md "wikilink")。リテラル表記ができるかどうかはその型が[第一級オブジェクト](../Page/第一級オブジェクト.md "wikilink")かどうかを検討する材料である。静的に構文解析が可能なことが多いため[シンタックスハイライト](https://ja.wikipedia.org/wiki/シンタックスハイライト "wikilink")ではたいてい色分けされる。なお言語によっては（標準規格等で）このリテラルのことを指して「定数」という用語を使っている場合もあるが、「リテラル」という語と使い分ける場合は、「[定数](https://ja.wikipedia.org/wiki/定数_\(プログラミング\) "wikilink")」とは「初期化できるだけで、その後は値を変えられない変数」というようなものを指す。
  ...
  - [文字型](https://ja.wikipedia.org/wiki/文字型 "wikilink")
    `'A'`のように[シングルクォート](https://ja.wikipedia.org/wiki/シングルクォート "wikilink")で文字を挟み、[エスケープシーケンス](https://ja.wikipedia.org/wiki/エスケープシーケンス "wikilink")が使える場合が多い。[ASCII](https://ja.wikipedia.org/wiki/ASCII "wikilink")や[Unicode](../Page/Unicode.md "wikilink")の1文字を表すものがある。[Scheme](../Page/Scheme.md "wikilink")では `#\A`、`#\newline` などのような記法である。 などでは `λ`、`ዀ` などのような記法である。
    文字コードを表すと考えて、数値型に含める場合もある。
  - [文字列](https://ja.wikipedia.org/wiki/文字列 "wikilink")型
  ...
  - オブジェクトリテラル
    JavaScriptではオブジェクトが属性名と属性値のペアの集まりであり、（また[クラスベース](https://ja.wikipedia.org/wiki/クラスベース "wikilink")ではなく[プロトタイプベース](https://ja.wikipedia.org/wiki/プロトタイプベース "wikilink")の[オブジェクト指向](../Page/オブジェクト指向.md "wikilink")であるためもあり、）前述の連想配列のようにしてオブジェクトをリテラルとして書ける。
    JavaScriptにおける、オブジェクトリテラルと、その他の基本的なデータ型（文字列と数値）の記法の組合せは、ちょっとした構造のあるデータを記述するのに便利であり、JavaScriptの構文と意味からそれらの部分だけを取り出したサブセットである[データ記述言語](https://ja.wikipedia.org/wiki/データ記述言語 "wikilink")の[JSONがある](https://ja.wikipedia.org/wiki/JavaScript_Object_Notation "wikilink")。
  - 構造体

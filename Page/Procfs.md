...
通常、procfs は立ち上げ時に `/proc` ディレクトリにマウントされる。[Solaris](../Page/Solaris.md "wikilink")、[BSD](../Page/BSD.md "wikilink")、[Linux](../Page/Linux.md "wikilink")、[AIX](https://ja.wikipedia.org/wiki/AIX "wikilink")、[QNX](../Page/QNX.md "wikilink")などでサポートされ、特にLinuxではプロセス関連以外のデータにも拡張されている（AIXのprocfsはLinuxベース）。procfs は機能を[カーネルモード](https://ja.wikipedia.org/wiki/カーネルモード "wikilink")から[ユーザーモード](https://ja.wikipedia.org/wiki/ユーザーモード "wikilink")に移すことに重要な役割を果たしている。例えば [GNUプロジェクト](../Page/GNUプロジェクト.md "wikilink")版の `ps`コマンドは全てのデータを procfs から得ており、特別なシステムコールを使用しない。

## 歴史

Tom J. Killian は [Version 8 Unix](https://ja.wikipedia.org/wiki/Research_Unix "wikilink") 上に `/proc` を実装し\[1\]、[1984年](../Page/1984年.md "wikilink")6月、[USENIX](https://ja.wikipedia.org/wiki/USENIX "wikilink")で *Processes as Files* という題の論文を発表した。これは、プロセスのトレースを行う *[ptrace](https://ja.wikipedia.org/wiki/ptrace "wikilink")* システムコールを置き換える目的で設計された。
...
Linux では、procfs は動作中プロセスに関する情報を `/proc/`*[`PID`](../Page/プロセス識別子.md "wikilink")* というディレクトリで提供し、以下のような情報を提供する。
  ...
  - `/proc/`*`PID`*`/exe` - 元々の[実行ファイル](https://ja.wikipedia.org/wiki/実行ファイル "wikilink")へのシンボリックリンク（もしあれば。プロセスは元々の実行ファイルが削除/移動された後も動作し続けることがある）
  ...
  - `/proc/`*`PID`*`/task` - そのプロセスを親プロセスとしている全プロセスへの[ハードリンク](https://ja.wikipedia.org/wiki/ハードリンク "wikilink")を格納したディレクトリ
...
procfs では個々のプロセスとは直接関係しないシステム情報も提供するが、[Linuxカーネル](../Page/Linuxカーネル.md "wikilink") 2.6 では大部分が別の擬似ファイルシステム *[sysfs](https://ja.wikipedia.org/wiki/sysfs "wikilink")* に分離移動されている（`/sys` にマウントされる）。
  ...
  - `/proc/cpuinfo` - [CPU](../Page/CPU.md "wikilink")に関する情報（ベンダー、CPUファミリ、機種、クロック周波数、キャッシュサイズ、コア数など）\[3\]。[BogoMips](https://ja.wikipedia.org/wiki/BogoMips "wikilink")の値もあるが、実際の性能を反映しているとは言えない。

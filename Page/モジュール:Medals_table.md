> この記事は[モジュール:Medals table](https://ja.wikipedia.org/wiki/モジュール:Medals_table)から翻訳されています。
...
`       leading = frame:expandTemplate{title = 'legend', args = {'#E9D66B', "`**`本競技を主導`**`"}}`
`   end`
`   `
`   if legendpos == 't' then`
...
`                   :css('text-align','left')`
`                   :wikitext(args['remaining_link'] or args['remaining_text'] or 'その他')`
`               :tag('td')`
...
`           :addClass('sortbottom')`
`           :tag('th')`
`               :wikitext('計 ('..team..'数: '..remainingEnd..')')`
`               :attr('scope', 'row')`
`               :css('background-color', '#eaebef')`

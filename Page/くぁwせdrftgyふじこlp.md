...
[thumb](https://ja.wikipedia.org/wiki/ファイル:KB_Japanese.svg "wikilink")。キーボードの「Q」に右手の中指、「A」に右手の人差し指を置いて左から右へ滑らせると、「くぁwせdrftgyふじこlp」と表示される。\]\] [QWERTY配列](../Page/QWERTY配列.md "wikilink")の[キーボードを用い](../Page/キーボード_\(コンピュータ\).md "wikilink")、フルキーの3段目と2段目を交互に入力すると文字列「**qawsedrftgyhujikolp**」が得られるが、ローマ字かな変換モードの日本語入力でこう入力したときに変換されて表示される文字列が、「**くぁwせdrftgyふじこlp**」である。

一般にはフルキーの左端、「Q」のキーに右手の中指、「A」のキーに右手の人差し指を置き、そのまま1段目→2段目→1段目→2段目と右に指を滑らせることで入力できると紹介される。ただし、使用する[日本語入力システム](../Page/日本語入力システム.md "wikilink")のローマ字テーブル（ローマ字とひらがなの対応表）によっては、必ずしもこの文字列になるとは限らない。いずれの場合でも、大抵は「ふじこ」（←hujiko）というひらがな表記は入る。
